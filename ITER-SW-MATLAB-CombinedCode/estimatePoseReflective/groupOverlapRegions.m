...
regionPixelListUniqueSerial = int32(zeros(0,2));
coder.varsize('regionPixelListUniqueSerial');
regionPixelListUnique = [];

indexes = [];
count = 0;
sizes = zeros(1,numel(unique(groups)));
for group=1:numel(unique(groups))
    ...
    if(leastDarkInRegion>darkestInNeighbourhood) %Keep only bright on dark polarity
        ...
        regionPixelListUniqueSerial = cat(1,regionPixelListUniqueSerial(1:end-1,:),regionPixelListGroup');
