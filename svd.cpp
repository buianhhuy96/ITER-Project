...
void b_svd(const double A[3][3], double U[3][3], double s[3], double V[3][3])
{
  ...
  for (q = 0; q < 2; q++) {
    ...
    for (kase = qp1; kase < 4; kase++) {
      ...
      if (apply_transform) {
        blas::xaxpy(3 - q,
                    -(blas::xdotc(3 - q, b_A, qq, b_A, qjj + 1) / b_A[q][q]),
                    qq, b_A, qjj + 1);
      }
...
void c_svd(const double A[9][9], double U[9][9], double s[9], double V[9][9])
{
  ...
  for (q = 0; q < 8; q++) {
    ...
    for (k = qp1; k < 10; k++) {
      ...
      if (apply_transform) {
        blas::b_xaxpy(
            9 - q, -(blas::b_xdotc(9 - q, b_A, qq, b_A, qjj + 1) / b_A[q][q]),
            qq, b_A, qjj + 1);
      }
      e[k - 1] = (&b_A[0][0])[qjj];
    }
    ...
    if ((q + 1) <= 7) {
      ...
      if (nrm == 0.0) {
        ...
        for (k = qp1; k < 10; k++) {
          blas::c_xaxpy(8 - q, e[k - 1], b_A, (q + (9 * (k - 1))) + 2, work,
                        q + 2);
        }
        for (k = qp1; k < 10; k++) {
          blas::d_xaxpy(8 - q, (-e[k - 1]) / e[q + 1], work, q + 2, b_A,
                        (q + (9 * (k - 1))) + 2);
        }
      }
      for (k = qp1; k < 10; k++) {
