               ...
               ::coder::array<double, 2U> &p)
{
  ...
  int b_i;
  int c_x;
  V.set_size(b_x.size(1), static_cast<int>(static_cast<double>(g_n + 1.0)));
  if ((b_x.size(1) != 0) &&
      ((static_cast<int>(static_cast<double>(g_n + 1.0))) != 0)) {
    int k;
    b_i = b_x.size(1);
    for (k = 0; k < b_i; k++) {
      V[k + (V.size(0) *
             ((static_cast<int>(static_cast<double>(g_n + 1.0))) - 1))] = 1.0;
    }
    if (!(g_n < 1.0)) {
      b_i = b_x.size(1);
      for (k = 0; k < b_i; k++) {
        V[k + (V.size(0) * ((static_cast<int>(g_n)) - 1))] = b_x[k];
      }
      b_i =
          static_cast<int>(static_cast<double>(-((-1.0 - (g_n - 1.0)) + 1.0)));
      if (0 <= ((static_cast<int>(
                    static_cast<double>(-((-1.0 - (g_n - 1.0)) + 1.0)))) -
                1)) {
        c_x = b_x.size(1);
      }
      for (int j{0}; j < b_i; j++) {
        double b_j;
        b_j = (g_n - 1.0) + (-(static_cast<double>(j)));
        for (k = 0; k < c_x; k++) {
             ...
             ::coder::array<double, 2U> &p)
{
  ...
  int b_i;
  int rr;
  V.set_size(b_x.size(0), static_cast<int>(static_cast<double>(g_n + 1.0)));
  if ((b_x.size(0) != 0) &&
      ((static_cast<int>(static_cast<double>(g_n + 1.0))) != 0)) {
    int k;
    b_i = b_x.size(0);
