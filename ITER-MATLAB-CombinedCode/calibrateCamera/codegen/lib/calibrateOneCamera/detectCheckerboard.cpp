...
static void poly2RectMask(double d_X[4], double d_Y[4], double height,
                          ...
                          double width, ::coder::array<bool, 2U> &mask)
{
  ...
  if (d_Y[1] > d_Y[2]) {
    ...
    b_i = (static_cast<int>(d_Y[1])) - 1;
    i1 = static_cast<int>(d_Y[2]);
  }
  if (d_X[1] > d_X[2]) {
    loop_ub_tmp = 0;
    i2 = 0;
                        ...
                        double boardSize[2])
{
  ...
  ::coder::array<float, 2U> I_45;
  ::coder::array<float, 2U> I_45_x;
  ::coder::array<float, 2U> Ig;
  ...
  if ((Ig.size(0) == 0) || (Ig.size(1) == 0)) {
    ...
    ::Codegen::coder::internal::conv2AXPYValidCMP(c_a, b);
  ...
  for (k = 0; k < b_i; k++) {
    h_N = Ix2.size(0);
    for (loop_ub = 0; loop_ub < h_N; loop_ub++) {
  ...
  if ((Ix2.size(0) != 0) && (Ix2.size(1) != 0)) {
    ...
    ::Codegen::coder::internal::c_conv2AXPYValidCMP(c_a, b);
  ...
  for (b_i = 0; b_i < loop_ub; b_i++) {
    s[b_i] = static_cast<double>(
        static_cast<int>((static_cast<int>(points0[b_i + points0.size(0)])) +
                         (h_N * ((static_cast<int>(points0[b_i])) - 1))));
  }
                                 ...
                                 double theta, b_Checkerboard *b_iobj_0)
{
  ...
  if (scores.size(0) == 0) {
    ...
    for (int c_i{0}; c_i < b_i; c_i++) {
      ...
      c_x[0] = ((-tb) * 0.707106769F) + (df * 0.707106769F);
      ...
      if ((static_cast<double>(std::abs(
              std::abs(std::abs(rt_atan2f_snf(adf, c_x[0])) - 3.14159274F) -
                       ...
                       const ::coder::array<float, 2U> &e_I)
{
  ...
  ::coder::array<float, 1U> g_I;
  ::coder::array<float, 1U> m_I;
  ::coder::array<int, 1U> r2;
  ::coder::array<int, 1U> r3;
  ::coder::array<signed char, 2U> b_ii;
  ::coder::array<bool, 3U> c_x;
  ...
  if (!std::isinf(b_x)) {
    ...
    if (b_y) {
      ...
      loop_ub = r3.size(0);
      for (b_i = 0; b_i < loop_ub; b_i++) {
        m_I[b_i] = e_I[r3[b_i] - 1];
      }
      if (!(b_mean(g_I) < b_mean(m_I))) {
        newBoardCoords2.set_size(b_board->BoardIdx.size(0),
                                 b_board->BoardIdx.size(1));
        loop_ub = b_board->BoardIdx.size(1) - 1;
        for (b_i = 0; b_i <= loop_ub; b_i++) {
      ...
      while ((!exitg1) && (b_loop_ub < 4)) {
        if (cornerIdx[b_loop_ub] != 0.0) {
          outsize_idx_1 = 1;
          b_ii[0] = static_cast<signed char>(b_loop_ub + 1);
          exitg1 = true;
        } else {
          b_loop_ub++;
        }
      ...
      numRot.set_size(1, b_ii.size(1));
      loop_ub = b_ii.size(1);
      for (b_i = 0; b_i < loop_ub; b_i++) {
        numRot[0] = (static_cast<double>(b_ii[0])) - 1.0;
      }
      if (b_ii.size(1) == 0) {
        numRot.set_size(1, 1);
        numRot[0] = 0.0;
    ...
    if ((b_loop_ub == 0) == (loop_ub == 0)) {
      ...
      if (b_y) {
        double g_x;
        g_x = b_board->BoardCoords[(b_board->BoardCoords.size(0) +
                                    (b_board->BoardCoords.size(0) *
