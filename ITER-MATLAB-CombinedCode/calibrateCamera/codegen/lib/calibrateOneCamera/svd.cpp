           ...
           ::coder::array<double, 1U> &s, double V[6][6])
{
  ...
  for (c_i = 0; c_i < 6; c_i++) {
    for (m = 0; m < b_i; m++) {
      e_A[m + (e_A.size(0) * c_i)] = A[m + (A.size(0) * c_i)];
    }
  }
  g_n = A.size(0);
  minnp = A.size(0);
  ...
  b_U.set_size(A.size(0), A.size(0));
  b_i = A.size(0);
  for (c_i = 0; c_i < b_i; c_i++) {
    qjj = A.size(0);
    for (m = 0; m < qjj; m++) {
      b_U[m + (b_U.size(0) * c_i)] = 0.0;
    }
  }
  (void)std::memset(&V[0][0], 0, 36U * (sizeof(double)));
  if (A.size(0) == 0) {
    for (int b_ii{0}; b_ii < 6; b_ii++) {
      V[b_ii][b_ii] = 1.0;
    }
  } else {
    double nrm;
    ...
    for (q = 0; q < c_i; q++) {
      ...
      if ((q + 1) <= nct) {
        for (b_ii = q + 1; b_ii <= g_n; b_ii++) {
          b_U[(b_ii + (b_U.size(0) * q)) - 1] =
              e_A[(b_ii + (e_A.size(0) * q)) - 1];
        }
      }
      if ((q + 1) <= nrt) {
    ...
    if (nct < 6) {
      b_s[nct] = e_A[nct + (e_A.size(0) * nct)];
    }
    if (A.size(0) < (m + 1)) {
      b_s[m] = 0.0;
    }
    ...
    if ((nct + 1) <= A.size(0)) {
      for (jj = nctp1; jj <= g_n; jj++) {
        for (b_ii = 0; b_ii < g_n; b_ii++) {
          b_U[b_ii + (b_U.size(0) * (jj - 1))] = 0.0;
        }
        b_U[(jj + (b_U.size(0) * (jj - 1))) - 1] = 1.0;
      }
    }
    for (q = nct; q >= 1; q--) {
      nmq = q + 1;
      ...
      if (b_s[q - 1] != 0.0) {
        for (jj = nmq; jj <= g_n; jj++) {
          ...
          if ((b_i + 1) >= 1) {
            for (k = 0; k <= b_i; k++) {
              nrm += b_U[qq + k] * b_U[(qjj + k) - 1];
            }
          }
          nrm = -(nrm / b_U[qq]);
          blas::xaxpy(b_i + 1, nrm, qq + 1, b_U, qjj);
        }
        for (b_ii = q; b_ii <= g_n; b_ii++) {
          b_U[(b_ii + (b_U.size(0) * (q - 1))) - 1] =
              -b_U[(b_ii + (b_U.size(0) * (q - 1))) - 1];
        }
        b_U[qq] = b_U[qq] + 1.0;
        for (b_ii = 0; b_ii <= (q - 2); b_ii++) {
          b_U[b_ii + (b_U.size(0) * (q - 1))] = 0.0;
        }
      } else {
        for (b_ii = 0; b_ii < g_n; b_ii++) {
          b_U[b_ii + (b_U.size(0) * (q - 1))] = 0.0;
    ...
    while (((m + 1) > 0) && (nctp1 < 75)) {
      ...
      while (!(exitg1 || (b_ii == 0))) {
        ...
        if (nrm <= (2.2204460492503131E-16 *
                    (std::abs(b_s[b_ii - 1]) + std::abs(b_s[b_ii])))) {
          e[b_ii - 1] = 0.0;
          exitg1 = true;
      ...
      case 1: {
        ...
        e[m - 1] = 0.0;
        for (k = m; k >= (b_ii + 1); k--) {
          blas::xrotg(&b_s[k - 1], &f, &sqds, &scale);
          if (k > (b_ii + 1)) {
            double b;
            b = e[k - 2];
            f = (-scale) * b;
        ...
        for (k = b_ii + 1; k <= (m + 1); k++) {
          ...
          if (g_n >= 1) {
            ...
            for (nmq = 0; nmq < g_n; nmq++) {
              qq = b_iy + nmq;
              b_i = qjj + nmq;
              nrm = (sqds * b_U[b_i]) + (scale * b_U[qq]);
              b_U[qq] = (sqds * b_U[qq]) - (scale * b_U[b_i]);
              b_U[b_i] = nrm;
            }
          }
        }
      ...
      case 3: {
        ...
        for (k = b_ii + 1; k <= m; k++) {
          ...
          b_s[k - 1] = (sqds * b) + (scale * nrm);
          blas::xrotg(&b_s[k - 1], &rt, &sqds, &scale);
          f = (sqds * e[k - 1]) + (scale * b_s[k]);
          b_s[k] = ((-scale) * e[k - 1]) + (sqds * b_s[k]);
          rt = scale * e[k];
          e[k] *= sqds;
          if ((k < g_n) && (g_n >= 1)) {
            qjj = g_n * (k - 1);
  ...
  for (q = 1; q >= 0; q--) {
    ...
    if (b_s[q] != 0.0) {
      for (kase = qp1; kase < 4; kase++) {
        ...
        blas::c_xaxpy(
            3 - q,
            -(blas::b_xdotc(3 - q, b_U, qq + 1, b_U, qjj) / (&b_U[0][0])[qq]),
            qq + 1, b_U, qjj);
      }
      for (k = q + 1; k < 4; k++) {
        b_U[q][k - 1] = -b_U[q][k - 1];
  ...
  while (((m + 2) > 0) && (qp1 < 75)) {
    ...
    case 2: {
      ...
      for (k = q + 1; k <= (m + 2); k++) {
        ...
        rt = (-sqds) * b;
        e[k - 1] = b * sm;
        blas::b_xrot(b_U, (3 * (k - 1)) + 1, (3 * (q - 1)) + 1, sm, sqds);
      }
    } break;
    case 3: {
      double b;
      ...
      for (k = q + 1; k <= kase; k++) {
        ...
        nrm = sqds * e[k];
        e[k] *= sm;
        blas::b_xrot(b_U, (3 * (k - 1)) + 1, (3 * k) + 1, sm, sqds);
      }
      e[m] = rt;
      qp1++;
    } break;
    default:
      ...
      while (((q + 1) < 3) && (b_s[q] < b_s[qp1])) {
        ...
        blas::b_xswap(b_U, (3 * q) + 1, (3 * (q + 1)) + 1);
        q = qp1;
        qp1++;
      }
      qp1 = 0;
