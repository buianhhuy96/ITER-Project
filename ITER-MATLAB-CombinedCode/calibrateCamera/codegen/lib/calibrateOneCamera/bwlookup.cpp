...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
    lookUpInd, b_i, b_rowInd)
            ...
            lookUpInd =
                (((((((((static_cast<int>(
                 ...
                 ((static_cast<int>(
                  ...
                  256)) +
                1;
            c_B[(b_rowInd + (c_B.size(0) * (b_colInd + 1))) + 1] =
                lut[lookUpInd - 1];
          }
        }
        ...
        for (colInd = 0; colInd <= (ub_loop - 3); colInd++) {
          ...
          c_B[((static_cast<int>(inDims[0])) + (c_B.size(0) * (colInd + 1))) -
              1] =
        ...
        for (rowInd = 0; rowInd <= (ub_loop - 3); rowInd++) {
          c_B[(rowInd + (c_B.size(0) * colInd)) + 1] = lut
              [(((((static_cast<int>(
    ...
    if (guard1) {
      ...
      if (bwin.size(0) > 1) {
        ...
        ub_loop = bwin.size(0);
        for (rowInd = 0; rowInd <= (ub_loop - 3); rowInd++) {
          c_B[rowInd + 1] = lut[(((static_cast<int>(bwin[rowInd])) * 8) +
                                 ((static_cast<int>(bwin[rowInd + 1])) * 16)) +
                                ((static_cast<int>(bwin[rowInd + 2])) * 32)];
        }
        c_B[bwin.size(0) - 1] =
