...
void bwmorphApplyOnce(::coder::array<bool, 2U> &bw)
{
  ...
  for (b_i = 0; b_i < loop_ub; b_i++) {
    ...
    for (i1 = 0; i1 < b_loop_ub; i1++) {
      m[i1 + (m.size(0) * b_i)] =
          ((bw[i1 + (bw.size(0) * b_i)]) && (!m[i1 + (m.size(0) * b_i)]));
    }
  }
  if (1 > m.size(0)) {
    b_i = 1;
  ...
  for (i3 = 0; i3 <= loop_ub; i3++) {
    b_loop_ub = div_s32_floor(i1 - 1, b_i);
    for (i6 = 0; i6 <= b_loop_ub; i6++) {
      bw[(i4 * i6) + (bw.size(0) * (i5 * i3))] =
          m[(b_i * i6) + (m.size(0) * (i2 * i3))];
    }
  }
  bwlookup(bw, m);
  m.set_size(bw.size(0), bw.size(1));
  ...
  for (i5 = 0; i5 <= loop_ub; i5++) {
    b_loop_ub = div_s32_floor((i2 - b_i) - 1, i1);
    for (int b_i10{0}; b_i10 <= b_loop_ub; b_i10++) {
      bw[(i6 + (i7 * b_i10)) + (bw.size(0) * (b_i8 + (i9 * i5)))] =
          m[(b_i + (i1 * b_i10)) + (m.size(0) * (i3 + (i4 * i5)))];
    }
  }
  bwlookup(bw, m);
  m.set_size(bw.size(0), bw.size(1));
  ...
  for (i4 = 0; i4 <= loop_ub; i4++) {
    b_loop_ub = div_s32_floor(i1 - 1, b_i);
    for (b_i8 = 0; b_i8 <= b_loop_ub; b_i8++) {
      bw[(i5 * b_i8) + (bw.size(0) * (i6 + (i7 * i4)))] =
          m[(b_i * b_i8) + (m.size(0) * (i2 + (i3 * i4)))];
    }
  }
  bwlookup(bw, m);
  m.set_size(bw.size(0), bw.size(1));
  ...
  for (i4 = 0; i4 <= loop_ub; i4++) {
    b_loop_ub = div_s32_floor((i2 - b_i) - 1, i1);
    for (b_i8 = 0; b_i8 <= b_loop_ub; b_i8++) {
      bw[(i5 + (i6 * b_i8)) + (bw.size(0) * (i7 * i4))] =
          m[(b_i + (i1 * b_i8)) + (m.size(0) * (i3 * i4))];
    }
  }
}
