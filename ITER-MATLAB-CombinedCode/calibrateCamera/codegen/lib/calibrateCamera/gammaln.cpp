...
void gammaln(real_T *b_x)
{
  ...
  if (!std::isnan(*b_x)) {
    if ((*b_x) <= 0.5) {
      ...
      *b_x = (0.5 * ((0.5 * (r / b_t)) + -0.57721566490153287)) +
             0.69314718055994529;
    } else if ((*b_x) <= 0.6796875) {
      real_T b_t;
      real_T r;
      b_t = 1.0;
      r = 0.0;
      for (int32_T b_i{0}; b_i < 8; b_i++) {
        r = (r * (((*b_x) - 0.5) - 0.5)) + p2[b_i];
        b_t = (b_t * (((*b_x) - 0.5) - 0.5)) + q2[b_i];
      }
      *b_x = (-std::log(*b_x)) +
             ((((*b_x) - 0.5) - 0.5) *
              (((((*b_x) - 0.5) - 0.5) * (r / b_t)) + 0.42278433509846713));
    } else {
      boolean_T guard1{false};
      guard1 = false;
      if ((*b_x) == std::floor(*b_x)) {
        if ((*b_x) <= 100.0) {
          *b_x = table100[(static_cast<int32_T>(*b_x)) - 1];
        } else {
          guard1 = true;
        }
      } else {
      ...
      if (guard1) {
        if ((*b_x) <= 1.5) {
          ...
          r = 0.0;
          for (int32_T b_i{0}; b_i < 8; b_i++) {
            r = (r * (((*b_x) - 0.5) - 0.5)) + p1[b_i];
            b_t = (b_t * (((*b_x) - 0.5) - 0.5)) + q1[b_i];
          }
          *b_x = (((*b_x) - 0.5) - 0.5) *
                 (((((*b_x) - 0.5) - 0.5) * (r / b_t)) + -0.57721566490153287);
        } else if ((*b_x) <= 4.0) {
          ...
          for (int32_T b_i{0}; b_i < 8; b_i++) {
            r = (r * ((*b_x) - 2.0)) + p2[b_i];
            b_t = (b_t * ((*b_x) - 2.0)) + q2[b_i];
          }
          *b_x = ((*b_x) - 2.0) *
                 ((((*b_x) - 2.0) * (r / b_t)) + 0.42278433509846713);
        } else if ((*b_x) <= 12.0) {
          real_T b_t;
          real_T r;
          b_t = -1.0;
          ...
          for (int32_T b_i{0}; b_i < 8; b_i++) {
            r = (r * ((*b_x) - 4.0)) + p4[b_i];
            b_t = (b_t * ((*b_x) - 4.0)) + q4[b_i];
          }
          *b_x = (((*b_x) - 4.0) * (r / b_t)) + 1.791759469228055;
        } else {
          real_T b_t;
          real_T r;
          r = 0.0057083835261;
          ...
          r /= *b_x;
          b_t = std::log(*b_x);
          *b_x = ((r + 0.91893853320467278) - (0.5 * b_t)) +
                 ((*b_x) * (b_t - 1.0));
        }
      }
    }
