          ...
          ::coder::array<int32_T, 1U> &idx)
{
  ...
  vlen = i - 1;
  vwork.set_size(&js_emlrtRTEI, sp, i);
  idx.set_size(&ks_emlrtRTEI, sp, x.size(0));
  st.site = &ff_emlrtRSI;
  vstride = 1;
  for (k = 0; k <= dim; k++) {
    vstride *= x.size(0);
          ...
          ::coder::array<int32_T, 1U> &idx)
{
  ...
  for (int32_T j{0}; j < vstride; j++) {
    ...
    dim = vwork.size(0);
    iidx.set_size(&ls_emlrtRTEI, &st, dim);
    for (i1 = 0; i1 < dim; i1++) {
      iidx[i1] = 0;
    }
    if (vwork.size(0) != 0) {
      ...
      x4[3] = 0;
      idx4[3] = 0;
      iwork.set_size(&ms_emlrtRTEI, &c_st, dim);
      for (i1 = 0; i1 < dim; i1++) {
        iwork[i1] = 0;
      }
      dim = vwork.size(0);
      xwork.set_size(&ns_emlrtRTEI, &c_st, dim);
      for (i1 = 0; i1 < dim; i1++) {
        xwork[i1] = 0;
      }
