              ...
              ::coder::array<real_T, 1U> &B)
{
  ...
  if ((A.size(0) == 0) || (B.size(0) == 0)) {
    ...
    f_st.site = &ju_emlrtRSI;
    g_st.site = &nu_emlrtRSI;
    for (i = 0; i < 7; i++) {
      ma = jpvt[i];
      if (ma != i + 1) {
        real_T tol;
    ...
    b_A.set_size(&xi_emlrtRTEI, &c_st, A.size(0), 8);
    maxmn = A.size(0) * 8;
    ...
    if ((rankA < ma) && (!emlrtSetWarningFlag(&c_st))) {
      ...
      e_emlrt_marshallIn(
          &e_st, b_sprintf(&e_st, y, b_emlrt_marshallOut(tol), &c_emlrtMCI),
    ...
    if (internal::lapack::infocheck(&f_st, (int32_T)info_t)) {
      maxmn = B.size(0);
      B.set_size(&bj_emlrtRTEI, &e_st, maxmn);
      for (ma = 0; ma < maxmn; ma++) {
        B[ma] = rtNaN;
      }
    }
