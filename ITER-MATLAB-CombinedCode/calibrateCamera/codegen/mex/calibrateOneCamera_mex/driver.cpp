...
#include "calibrateOneCamera_mex_internal_types1.h"
            ...
            ::coder::array<real_T, 2U> &jacobian)
{
  ...
  n = x0.size(0);
  gradf.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  dx.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  loop_ub = x0.size(0);
  for (i = 0; i < loop_ub; i++) {
    dx[i] = rtInf;
  }
  ...
  TypicalX.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  loop_ub = x0.size(0);
  for (i = 0; i < loop_ub; i++) {
    TypicalX[i] = 1.0;
  }
  hasLB.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  hasUB.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  st.site = &ny_emlrtRSI;
  for (idx = 1; idx <= x0.size(0); idx++) {
    if ((idx < 1) || (idx > hasLB.size(0))) {
      emlrtDynamicBoundsCheckR2012b(idx, 1, hasLB.size(0), &mt_emlrtBCI, &st);
    }
    hasLB[idx - 1] = false;
    if (idx > hasUB.size(0)) {
      emlrtDynamicBoundsCheckR2012b(idx, 1, hasUB.size(0), &mt_emlrtBCI, &st);
    }
    hasUB[idx - 1] = false;
  }
  x.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  loop_ub = x0.size(0);
  for (i = 0; i < loop_ub; i++) {
    x[i] = x0[i];
  }
  ...
  calibrateOneCamera_anonFcn1(
      &b_st, fun->workspace.calibration.CameraParameters.ImageSize,
      fun->workspace.calibration.PatternPositions, fun->workspace.genPoints,
      fun->workspace.detPoints, x0, f_temp);
  m_temp = f_temp.size(0);
  jacobian.set_size(&ec_emlrtRTEI, sp, f_temp.size(0), x0.size(0));
  m = f_temp.size(0);
  fCurrent.set_size(&ec_emlrtRTEI, sp, f_temp.size(0));
  fNew.set_size(&ec_emlrtRTEI, sp, f_temp.size(0));
  st.site = &ny_emlrtRSI;
  ...
  for (idx = 0; idx < m; idx++) {
    if (idx + 1 > f_temp.size(0)) {
      emlrtDynamicBoundsCheckR2012b(idx + 1, 1, f_temp.size(0), &lt_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    if (idx + 1 > fCurrent.size(0)) {
      emlrtDynamicBoundsCheckR2012b(idx + 1, 1, fCurrent.size(0), &lt_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    fCurrent[idx] = f_temp[idx];
  }
  augJacobian.set_size(&ec_emlrtRTEI, sp, f_temp.size(0) + x0.size(0),
                       x0.size(0));
  if (1 > f_temp.size(0)) {
    i = 0;
  } else {
  ...
  *resnorm = internal::blas::xdot(f_temp.size(0), fCurrent, fCurrent);
  ...
  utils::FiniteDifferences::factoryConstruct(
      ...
      &FiniteDifferences);
  c_finiteDifferenceRunTimeOption.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  loop_ub = x0.size(0);
  for (i = 0; i < loop_ub; i++) {
    c_finiteDifferenceRunTimeOption[i] = 1.0;
  }
  d_finiteDifferenceRunTimeOption.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  loop_ub = x0.size(0);
  for (i = 0; i < loop_ub; i++) {
  ...
  for (idx = 0; idx < n; idx++) {
    ...
    if ((i < 1) || (i > augJacobian.size(0))) {
      emlrtDynamicBoundsCheckR2012b(i, 1, augJacobian.size(0), &lt_emlrtBCI,
                                    (emlrtCTX)sp);
    }
  ...
  while ((!exitg1) && (idx == -5)) {
    ...
    f_temp.set_size(&ec_emlrtRTEI, sp, fCurrent.size(0));
    ...
    xp.set_size(&ec_emlrtRTEI, sp, x.size(0));
    loop_ub = x.size(0);
    for (i = 0; i < loop_ub; i++) {
      xp[i] = x[i] + dx[i];
    }
    ...
    calibrateOneCamera_anonFcn1(
        &b_st, fun->workspace.calibration.CameraParameters.ImageSize,
        fun->workspace.calibration.PatternPositions, fun->workspace.genPoints,
        fun->workspace.detPoints, xp, f_temp);
    st.site = &ny_emlrtRSI;
    ...
    for (idx = 0; idx < m; idx++) {
      ...
      if (idx + 1 > fNew.size(0)) {
        emlrtDynamicBoundsCheckR2012b(idx + 1, 1, fNew.size(0), &lt_emlrtBCI,
                                      (emlrtCTX)sp);
      }
      fNew[idx] = f_temp[idx];
    }
    ...
    if ((resnormNew < *resnorm) && evalOK) {
      ...
      *resnorm = resnormNew;
      fCurrent.set_size(&ec_emlrtRTEI, sp, fNew.size(0));
      loop_ub = fNew.size(0);
      for (i = 0; i < loop_ub; i++) {
        fCurrent[i] = fNew[i];
      }
      ...
      if (evalOK) {
        ...
        x.set_size(&ec_emlrtRTEI, sp, xp.size(0));
        loop_ub = xp.size(0);
        for (i = 0; i < loop_ub; i++) {
          x[i] = xp[i];
        }
  ...
  lambda_lower.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  loop_ub = x0.size(0);
  for (i = 0; i < loop_ub; i++) {
    lambda_lower[i] = 0.0;
  }
  lambda_upper.set_size(&ec_emlrtRTEI, sp, x0.size(0));
  loop_ub = x0.size(0);
  for (i = 0; i < loop_ub; i++) {
    lambda_upper[i] = 0.0;
  }
                     ...
                     ::coder::array<real_T, 1U> &varargout_2)
{
  ...
  CalculateCost(&c_st, x, fun->workspace.calibration.CameraParameters.ImageSize,
