               ...
               ::coder::array<real_T, 2U> &jacob)
{
  ...
  st.site = &gy_emlrtRSI;
  b.set_size(&us_emlrtRTEI, &st, xCurrent.size(0));
  loop_ub = xCurrent.size(0);
  for (i = 0; i < loop_ub; i++) {
    b[i] = muDoubleScalarIsInf(xCurrent[i]);
  }
  r.set_size(&ce_emlrtRTEI, &st, xCurrent.size(0));
  loop_ub = xCurrent.size(0);
  for (i = 0; i < loop_ub; i++) {
  ...
  f_st.site = &ly_emlrtRSI;
  g_st.site = &my_emlrtRSI;
  y = true;
  h_st.site = &rh_emlrtRSI;
  ...
  if (!y) {
    emlrtErrorWithMessageIdR2018a(
        &st, &kc_emlrtRTEI, "optim_codegen:common:InfNaNComplexDetected",
        "optim_codegen:common:InfNaNComplexDetected", 3, 4, 2, "x0");
  }
  b_xCurrent.set_size(&vs_emlrtRTEI, sp, xCurrent.size(0));
