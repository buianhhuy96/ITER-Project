                                    ...
                                    projective2d *tform)
{
  ...
  images::geotrans::internal::b_normalizeControlPoints(&st, xy, b_xy,
                                                       normMatrix2);
  if (1 > b_xy.size(1)) {
    emlrtDynamicBoundsCheckR2012b(1, 1, b_xy.size(1), &at_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  if (2 > b_xy.size(1)) {
    emlrtDynamicBoundsCheckR2012b(2, 1, b_xy.size(1), &ys_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  irank = b_uv.size(0);
  varargin_8.set_size(&pp_emlrtRTEI, sp, b_uv.size(0));
  for (i = 0; i < irank; i++) {
    varargin_8[i] = -b_uv[i];
  }
  ...
  irank = b_uv.size(0);
  b_varargin_8.set_size(&qp_emlrtRTEI, sp, b_uv.size(0));
  for (i = 0; i < irank; i++) {
  ...
  st.site = &dt_emlrtRSI;
  varargin_7.set_size(&pp_emlrtRTEI, &st, varargin_8.size(0));
  ...
  st.site = &et_emlrtRSI;
  b_varargin_7.set_size(&qp_emlrtRTEI, &st, b_varargin_8.size(0));
  ...
  b_X.set_size(&rp_emlrtRTEI, sp, b_xy.size(0) + b_xy.size(0), 8);
  for (i = 0; i < irank; i++) {
    b_X[i] = b_xy[i];
  }
  for (i = 0; i < loop_ub; i++) {
  ...
  irank = b_uv.size(0);
  loop_ub = b_uv.size(0);
  varargin_7.set_size(&sp_emlrtRTEI, sp, b_uv.size(0) + b_uv.size(0));
  for (i = 0; i < irank; i++) {
    varargin_7[i] = b_uv[i];
  }
  for (i = 0; i < loop_ub; i++) {
  ...
  if (8 != varargin_7.size(0)) {
    emlrtSubAssignSizeCheck1dR2017a(8, varargin_7.size(0), &xc_emlrtECI,
                                    (emlrtCTX)sp);
  }
  for (i = 0; i < 8; i++) {
    Tvec[i] = varargin_7[i];
                 ...
                 projective2d *tform)
{
  ...
  if (!p) {
    emlrtErrorWithMessageIdR2018a(
        &st, &tb_emlrtRTEI, "images:geotrans:differentNumbersOfControlPoints",
