                        ...
                        ::coder::array<real_T, 1U> &dx, int32_T m, int32_T n)
{
  ...
  if (dx.size(0) != n) {
    emlrtErrorWithMessageIdR2018a(
        &b_st, &mc_emlrtRTEI, "Coder:toolbox:ValidateattributesincorrectNumel",
        "MATLAB:incorrectNumel", 5, 4, 5, "input", 12, n);
  }
  jpvt.set_size(&xs_emlrtRTEI, sp, n);
  for (k = 0; k < n; k++) {
    jpvt[k] = 0;
  }
  st.site = &dbb_emlrtRSI;
  ...
  tau.set_size(&yi_emlrtRTEI, &b_st, minmana);
  jpvt_t.set_size(&aj_emlrtRTEI, &b_st, n);
  for (k = 0; k < n; k++) {
  ...
  k = rhs.size(0);
  if ((n < 1) || (n > k)) {
    emlrtDynamicBoundsCheckR2012b(n, 1, k, &wv_emlrtBCI, (emlrtCTX)sp);
  }
  t3_f2[0] = 1;
  t3_f2[1] = n;
  st.site = &dbb_emlrtRSI;
  ...
  st.site = &dbb_emlrtRSI;
  tau.set_size(&xs_emlrtRTEI, &st, n);
  for (k = 0; k < n; k++) {
    tau[k] = rhs[k];
  }
  ...
  k = dx.size(0);
  if (k != tau.size(0)) {
    emlrtSubAssignSizeCheck1dR2017a(k, tau.size(0), &de_emlrtECI, (emlrtCTX)sp);
  }
  dx.set_size(&xs_emlrtRTEI, sp, tau.size(0));
  na = tau.size(0);
  for (k = 0; k < na; k++) {
    dx[k] = tau[k];
  }
  ...
  for (k = 0; k < na; k++) {
    if ((jpvt[k] < 1) || (jpvt[k] > tau.size(0))) {
      emlrtDynamicBoundsCheckR2012b(jpvt[k], 1, tau.size(0), &wv_emlrtBCI,
                                    (emlrtCTX)sp);
    }
  }
  if (jpvt.size(0) != tau.size(0)) {
    emlrtSubAssignSizeCheck1dR2017a(jpvt.size(0), tau.size(0), &de_emlrtECI,
                                    (emlrtCTX)sp);
  }
  na = tau.size(0);
  for (k = 0; k < na; k++) {
