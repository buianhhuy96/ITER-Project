           ...
           real_T V[36])
{
  ...
  st.site = &vr_emlrtRSI;
  b_A.set_size(&bq_emlrtRTEI, &st, A.size(0), 6);
  info = A.size(0) * 6;
  for (i = 0; i < info; i++) {
    b_A[i] = A[i];
  }
  ...
  c_A.set_size(&cq_emlrtRTEI, &b_st, A.size(0), 6);
  info = A.size(0) * 6;
  for (i = 0; i < info; i++) {
    c_A[i] = A[i];
  }
  U.set_size(&dq_emlrtRTEI, &b_st, A.size(0), A.size(0));
  *s_size = muIntScalarMin_sint32(6, m);
  ...
  if (info > 0) {
    ...
    U.set_size(&eq_emlrtRTEI, &c_st, A.size(0), A.size(0));
    *s_size = info;
    if (A.size(0) != 0) {
