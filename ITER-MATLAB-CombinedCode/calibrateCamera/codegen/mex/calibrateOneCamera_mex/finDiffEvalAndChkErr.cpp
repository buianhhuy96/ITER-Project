                          ...
                          real_T *fplus)
{
  ...
  *evalOK = true;
  i = xk.size(0);
  if ((dim < 1) || (dim > i)) {
    emlrtDynamicBoundsCheckR2012b(dim, 1, i, &xv_emlrtBCI, (emlrtCTX)sp);
  }
  temp_tmp = xk[dim - 1];
  i = xk.size(0);
  if (dim > i) {
  ...
  if (i != varargout_2.size(0)) {
    emlrtSubAssignSizeCheck1dR2017a(i, varargout_2.size(0), &ee_emlrtECI,
                                    (emlrtCTX)sp);
  }
  cEqPlus.set_size(&ys_emlrtRTEI, sp, varargout_2.size(0));
