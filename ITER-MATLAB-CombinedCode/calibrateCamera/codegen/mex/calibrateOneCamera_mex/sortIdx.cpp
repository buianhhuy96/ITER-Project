             ...
             ::coder::array<int32_T, 1U> &idx)
{
  ...
  ib = x.size(0);
  idx.set_size(&ls_emlrtRTEI, sp, ib);
  ...
  if (x.size(0) != 0) {
    ...
    x4[3] = 0.0F;
    idx4[3] = 0;
    iwork.set_size(&ms_emlrtRTEI, &b_st, ib);
    for (quartetOffset = 0; quartetOffset < ib; quartetOffset++) {
      iwork[quartetOffset] = 0;
    }
    ib = x.size(0);
    xwork.set_size(&ns_emlrtRTEI, &b_st, ib);
    for (quartetOffset = 0; quartetOffset < ib; quartetOffset++) {
             ...
             ::coder::array<int32_T, 2U> &idx)
{
  ...
  emlrtHeapReferenceStackEnterFcnR2012b((emlrtCTX)sp);
  n = x.size(1) + 1;
  idx.set_size(&fk_emlrtRTEI, sp, 1, x.size(1));
  i = x.size(1);
  for (b_i = 0; b_i < i; b_i++) {
    idx[b_i] = 0;
  }
  if (x.size(1) != 0) {
    ...
    int32_T k;
    st.site = &pm_emlrtRSI;
    iwork.set_size(&ck_emlrtRTEI, &st, x.size(1));
    i = x.size(1) - 1;
