                  ...
                  const ::coder::array<boolean_T, 1U> &hasUB)
{
  ...
  for (int32_T i{0}; i < n; i++) {
    ...
    if ((i + 1 < 1) || (i + 1 > hasLB.size(0))) {
      emlrtDynamicBoundsCheckR2012b(i + 1, 1, hasLB.size(0), &tv_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    if (i + 1 > hasUB.size(0)) {
      emlrtDynamicBoundsCheckR2012b(i + 1, 1, hasUB.size(0), &tv_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    b_i = dx.size(0);
    if (i + 1 > b_i) {
      emlrtDynamicBoundsCheckR2012b(i + 1, 1, b_i, &tv_emlrtBCI, (emlrtCTX)sp);
    }
