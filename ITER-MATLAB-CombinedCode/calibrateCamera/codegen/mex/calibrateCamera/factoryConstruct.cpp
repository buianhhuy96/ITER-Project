                      ...
                      d_struct_T *obj)
{
  ...
  obj->nonlin = *nonlin;
  obj->f_1 = 0.0;
  obj->cEq_1.set_size(&tr_emlrtRTEI, sp, mCeq);
  obj->f_2 = 0.0;
  obj->cEq_2.set_size(&tr_emlrtRTEI, sp, mCeq);
  obj->nVar = nVar;
  obj->mIneq = 0;
  obj->mEq = mCeq;
  obj->numEvals = 0;
  ...
  obj->hasLB.set_size(&tr_emlrtRTEI, sp, nVar);
  obj->hasUB.set_size(&tr_emlrtRTEI, sp, nVar);
  b_bool = false;
  idx = 0;
  int32_T exitg1;
  do {
  ...
  for (idx = 1; idx <= nVar; idx++) {
    if ((idx < 1) || (idx > obj->hasLB.size(0))) {
      emlrtDynamicBoundsCheckR2012b(idx, 1, obj->hasLB.size(0), &wt_emlrtBCI,
                                    &st);
    }
    obj->hasLB[idx - 1] = false;
    if (idx > obj->hasUB.size(0)) {
      emlrtDynamicBoundsCheckR2012b(idx, 1, obj->hasUB.size(0), &wt_emlrtBCI,
                                    &st);
    }
