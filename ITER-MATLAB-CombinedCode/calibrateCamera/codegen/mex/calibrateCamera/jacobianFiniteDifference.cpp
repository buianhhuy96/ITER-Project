    ...
    const ::coder::array<real_T, 1U> &c_runTimeOptions_FiniteDifferen)
{
  ...
  JacCeqTrans.set_size(&ts_emlrtRTEI, sp, x.size(0), fCurrent.size(0));
  a__3.set_size(&ts_emlrtRTEI, sp, x.size(0));
  loop_ub = x.size(0);
  for (i = 0; i < loop_ub; i++) {
    a__3[i] = x[i];
  }
  ...
  if (1 > fCurrent.size(0)) {
    ...
    i1 = fCurrent.size(0);
    if ((i1 < 1) || (i1 > i)) {
      emlrtDynamicBoundsCheckR2012b(i1, 1, i, &nv_emlrtBCI, (emlrtCTX)sp);
    }
  }
  t1_f2[0] = i1;
