                      ...
                      ::coder::array<real_T, 2U> &loc)
{
  ...
  for (int32_T id{0}; id < i; id++) {
    ...
    i1 = loc.size(0);
    if ((id + 1 < 1) || (id + 1 > i1)) {
      emlrtDynamicBoundsCheckR2012b(id + 1, 1, i1, &mw_emlrtBCI, (emlrtCTX)sp);
    }
    loop_ub = loc.size(1);
    r.set_size(&vv_emlrtRTEI, sp, loop_ub);
    ...
    loop_ub = loc.size(1);
    b_loc.set_size(&wv_emlrtRTEI, &st, 1, loop_ub);
    for (i1 = 0; i1 < loop_ub; i1++) {
      b_loc[i1] = loc[id + loc.size(0) * i1];
    }
    loop_ub = loc.size(1);
    c_loc.set_size(&xv_emlrtRTEI, &st, loop_ub);
    for (i1 = 0; i1 < loop_ub; i1++) {
    ...
    if (b_any(&b_st, c_loc)) {
      ...
      i1 = loc.size(1);
      if (1 > i1) {
        emlrtDynamicBoundsCheckR2012b(1, 1, i1, &nw_emlrtBCI, &st);
      }
      if (loc[id] > (static_cast<real_T>(metric.size(1)) - 2.0) - 1.0) {
        ...
        i1 = loc.size(1);
        if (2 > i1) {
          emlrtDynamicBoundsCheckR2012b(2, 1, i1, &ow_emlrtBCI, &st);
        }
        if (loc[id + loc.size(0)] >
          ...
          i1 = loc.size(1);
          if (2 > i1) {
            emlrtDynamicBoundsCheckR2012b(2, 1, i1, &pw_emlrtBCI, &st);
          }
          i1 = loc.size(1);
          if (2 > i1) {
            emlrtDynamicBoundsCheckR2012b(2, 1, i1, &qw_emlrtBCI, &st);
          }
          if (loc[id + loc.size(0)] - 2.0 > loc[id + loc.size(0)] + 2.0) {
            ...
            if ((static_cast<int32_T>(d) < 1) ||
                (static_cast<int32_T>(d) > metric.size(0))) {
              emlrtDynamicBoundsCheckR2012b(static_cast<int32_T>(d), 1,
                                            metric.size(0), &sw_emlrtBCI, &st);
            }
            i2 = static_cast<int32_T>(d) - 1;
          }
          i3 = loc.size(1);
          if (1 > i3) {
            emlrtDynamicBoundsCheckR2012b(1, 1, i3, &tw_emlrtBCI, &st);
          }
          i3 = loc.size(1);
          if (1 > i3) {
            emlrtDynamicBoundsCheckR2012b(1, 1, i3, &uw_emlrtBCI, &st);
          }
          ...
          b_metric.set_size(&yv_emlrtRTEI, &b_st, loop_ub, b_loop_ub);
          ...
          loop_ub = loc.size(1);
          d_loc.set_size(&wv_emlrtRTEI, &st, 1, loop_ub);
          for (i1 = 0; i1 < loop_ub; i1++) {
          ...
          emlrtSizeEqCheckNDR2012b(&iv1[0], &iv[0], &de_emlrtECI, &st);
          subPixelLoc.set_size(&aw_emlrtRTEI, &st, 1, 2);
    ...
    if (guard1) {
      subPixelLoc.set_size(&bw_emlrtRTEI, &st, 1, b_loc.size(1));
      loop_ub = b_loc.size(1);
      for (i1 = 0; i1 < loop_ub; i1++) {
        subPixelLoc[i1] = static_cast<real32_T>(b_loc[i1]);
      }
    }
    b_loc.set_size(&cw_emlrtRTEI, sp, 1, subPixelLoc.size(1));
    ...
    iv[0] = 1;
    iv[1] = r.size(0);
    emlrtSubAssignSizeCheckR2012b(&iv[0], 2, b_loc.size(), 2, &ce_emlrtECI,
                                  (emlrtCTX)sp);
    loop_ub = b_loc.size(1);
    for (i1 = 0; i1 < loop_ub; i1++) {
