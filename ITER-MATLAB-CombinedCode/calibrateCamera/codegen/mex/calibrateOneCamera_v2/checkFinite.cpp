                      ...
                      int32_T m, int32_T n)
{
  ...
  if ((m == 1) || (n == 1)) {
    i = x.size(0) * x.size(1);
    if (i < 0) {
      emlrtDynamicBoundsCheckR2012b(i, 0, MAX_int32_T, &cx_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    if (i != m * n) {
      emlrtErrorWithMessageIdR2018a(
          sp, &vc_emlrtRTEI, "optim_codegen:common:IncorrectSizeObjective",
  ...
  for (int32_T b_i{0}; b_i < b; b_i++) {
    if (evalOK) {
      ...
      i = x.size(0) * x.size(1);
      if ((b_i + 1 < 1) || (b_i + 1 > i)) {
        emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, i, &dx_emlrtBCI,
                                      (emlrtCTX)sp);
      }
      b_x = x[b_i];
