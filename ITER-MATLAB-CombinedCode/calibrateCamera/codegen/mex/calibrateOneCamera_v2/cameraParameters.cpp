...
d_cameraParameters *d_cameraParameters::init(
    const emlrtStack *sp,
    const ::coder::array<real_T, 2U> &varargin_1_RadialDistortion,
    const real_T varargin_1_TangentialDistortion[2],
    const real_T varargin_1_ImageSize[2], const char_T varargin_1_WorldUnits[2],
    boolean_T varargin_1_EstimateSkew, real_T c_varargin_1_NumRadialDistortio,
...
{
  d_cameraParameters *this_;
  array<real_T, 2U> b_this;
  ...
  this_->RadialDistortion.set_size(&tt_emlrtRTEI, &st, 1,
                                   varargin_1_RadialDistortion.size(1));
  loop_ub = varargin_1_RadialDistortion.size(1);
  for (i = 0; i < loop_ub; i++) {
    this_->RadialDistortion[i] = varargin_1_RadialDistortion[i];
  }
  b_this.set_size(&ut_emlrtRTEI, &st, 1, this_->RadialDistortion.size(1));
  loop_ub =
      this_->RadialDistortion.size(0) * this_->RadialDistortion.size(1) - 1;
  for (i = 0; i <= loop_ub; i++) {
    b_this[i] = this_->RadialDistortion[i];
  }
  b_st.site = &n_emlrtRSI;
  cameraParameters::checkRadialDistortion(&b_st, b_this);
  ...
  c_this[0] = this_->ImageSize[0];
  c_this[1] = this_->ImageSize[1];
  b_st.site = &q_emlrtRSI;
  cameraParameters::checkImageSize(&b_st, c_this);
  this_->RotationVectors.set_size(&ss_emlrtRTEI, &st,
                                  varargin_1_RotationVectors.size(0),
                                  varargin_1_RotationVectors.size(1));
  loop_ub =
      varargin_1_RotationVectors.size(0) * varargin_1_RotationVectors.size(1);
  for (i = 0; i < loop_ub; i++) {
    this_->RotationVectors[i] = varargin_1_RotationVectors[i];
  }
  d_this.set_size(&ts_emlrtRTEI, &st, this_->RotationVectors.size(0),
                  this_->RotationVectors.size(1));
  loop_ub = this_->RotationVectors.size(0) * this_->RotationVectors.size(1) - 1;
  for (i = 0; i <= loop_ub; i++) {
    d_this[i] = this_->RotationVectors[i];
  }
  b_st.site = &pbb_emlrtRSI;
  cameraParameters::b_checkRotationVectors(&b_st, d_this);
  this_->TranslationVectors.set_size(&us_emlrtRTEI, &st,
                                     varargin_1_TranslationVectors.size(0),
                                     varargin_1_TranslationVectors.size(1));
  loop_ub = varargin_1_TranslationVectors.size(0) *
            varargin_1_TranslationVectors.size(1);
  for (i = 0; i < loop_ub; i++) {
    this_->TranslationVectors[i] = varargin_1_TranslationVectors[i];
  }
  d_this.set_size(&vs_emlrtRTEI, &st, this_->TranslationVectors.size(0),
                  this_->TranslationVectors.size(1));
  loop_ub =
      this_->TranslationVectors.size(0) * this_->TranslationVectors.size(1) - 1;
  for (i = 0; i <= loop_ub; i++) {
    d_this[i] = this_->TranslationVectors[i];
  }
  b_st.site = &qbb_emlrtRSI;
  cameraParameters::b_checkTranslationVectors(&b_st, d_this);
  this_->WorldUnits[0] = varargin_1_WorldUnits[0];
  this_->WorldUnits[1] = varargin_1_WorldUnits[1];
  this_->EstimateSkew = varargin_1_EstimateSkew;
  ...
  this_->EstimateTangentialDistortion = c_varargin_1_EstimateTangential;
  b = (this_->RotationVectors.size(0) == 0);
  b1 = (this_->RotationVectors.size(1) == 0);
  guard1 = false;
  guard2 = false;
  if (b || b1) {
    b = (this_->TranslationVectors.size(0) == 0);
    b1 = (this_->TranslationVectors.size(1) == 0);
    if ((!b) && (!b1)) {
      guard1 = true;
    } else {
      guard2 = true;
  ...
  e_this[1] =
      (this_->RotationVectors.size(1) != this_->TranslationVectors.size(1));
                         ...
                         const real_T varargin_20[2])
{
  ...
  this_->RotationVectors.set_size(&ss_emlrtRTEI, &st, varargin_4.size(0), 3);
  ...
  c_this.set_size(&ts_emlrtRTEI, &st, this_->RotationVectors.size(0), 3);
  loop_ub = this_->RotationVectors.size(0) * this_->RotationVectors.size(1) - 1;
  for (worldPoints_size_idx_1 = 0; worldPoints_size_idx_1 <= loop_ub;
       worldPoints_size_idx_1++) {
  ...
  this_->TranslationVectors.set_size(&us_emlrtRTEI, &st, varargin_6.size(0), 3);
  ...
  c_this.set_size(&vs_emlrtRTEI, &st, this_->TranslationVectors.size(0), 3);
  loop_ub =
      this_->TranslationVectors.size(0) * this_->TranslationVectors.size(1) - 1;
  for (worldPoints_size_idx_1 = 0; worldPoints_size_idx_1 <= loop_ub;
       worldPoints_size_idx_1++) {
  ...
  this_->WorldPoints.set_size(&ws_emlrtRTEI, &st, varargin_8.size(0),
                              varargin_8.size(1));
                                             ...
                                             const struct_T *varargin_1)
{
  ...
  b_st.site = &qbb_emlrtRSI;
  cameraParameters::checkTranslationVectors(&b_st, c_this);
  this_->WorldPoints.set_size(&ws_emlrtRTEI, &st,
  ...
  this_->WorldUnits.set_size(&ys_emlrtRTEI, &st, 1,
