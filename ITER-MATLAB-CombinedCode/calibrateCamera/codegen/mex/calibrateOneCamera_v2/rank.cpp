...
int32_T local_rank(const emlrtStack *sp, const ::coder::array<real_T, 2U> &A)
{
  ...
  if (A.size(0) != 0) {
    ...
    if (p) {
      ...
      b_A.set_size(&fs_emlrtRTEI, &d_st, A.size(0), 8);
      nx = A.size(0) * 8;
      for (k = 0; k < nx; k++) {
        b_A[k] = A[k];
      }
      ...
      s.set_size(&gs_emlrtRTEI, &e_st, nx);
      if (nx > 1) {
        superb.set_size(&is_emlrtRTEI, &e_st, nx - 1);
      } else {
        superb.set_size(&hs_emlrtRTEI, &e_st, 1);
      }
      ...
      if (nx < 0) {
        if (nx == -1010) {
          emlrtErrorWithMessageIdR2018a(&f_st, &ib_emlrtRTEI, "MATLAB:nomem",
                                        "MATLAB:nomem", 0);
        } else {
          emlrtErrorWithMessageIdR2018a(
              &f_st, &jb_emlrtRTEI, "Coder:toolbox:LAPACKCallErrorInfo",
      ...
      s.set_size(&es_emlrtRTEI, &b_st, nx);
      for (k = 0; k < nx; k++) {
        s[k] = rtNaN;
      }
    }
