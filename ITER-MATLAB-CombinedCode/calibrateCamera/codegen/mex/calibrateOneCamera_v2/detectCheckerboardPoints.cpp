                              ...
                              real_T boardSize[2])
{
  ...
  c_I.set_size(&qd_emlrtRTEI, &b_st, b_I.size(0), b_I.size(1));
  ...
  if ((imagePoints.size(0) == 0) || (imagePoints.size(1) == 0)) {
    ...
    Ig.set_size(&rd_emlrtRTEI, &d_st, c_I.size(0), c_I.size(1));
    ...
    d_imfilter(&e_st, Ig);
    Iy.set_size(&sd_emlrtRTEI, &d_st, Ig.size(0), Ig.size(1));
    ...
    b_imfilter(&e_st, Ig);
    Ixy.set_size(&td_emlrtRTEI, &d_st, Ig.size(0), Ig.size(1));
    loop_ub = Ig.size(0) * Ig.size(1);
    for (i = 0; i < loop_ub; i++) {
      Ixy[i] = Ig[i];
    }
    ...
    imfilter(&e_st, Ixy);
    r1.set_size(&ud_emlrtRTEI, &d_st, Ig.size(0), Ig.size(1));
    loop_ub = Ig.size(0) * Ig.size(1);
    for (i = 0; i < loop_ub; i++) {
      r1[i] = Ig[i] * 0.707106769F;
    }
    I_45.set_size(&vd_emlrtRTEI, &d_st, Iy.size(0), Iy.size(1));
    ...
    I_45.set_size(&wd_emlrtRTEI, &d_st, r1.size(0), r1.size(1));
    for (i = 0; i < loop_ub; i++) {
      I_45[i] = r1[i] + I_45[i];
    }
    r2.set_size(&xd_emlrtRTEI, &d_st, Iy.size(0), Iy.size(1));
    loop_ub = Iy.size(0) * Iy.size(1);
    for (i = 0; i < loop_ub; i++) {
    ...
    emlrtSizeEqCheckNDR2012b(&varargin_1[0], &varargin_2[0], &c_emlrtECI,
                             &d_st);
    I_45_x.set_size(&yd_emlrtRTEI, &d_st, I_45.size(0), I_45.size(1));
    ...
    I_45_y.set_size(&ae_emlrtRTEI, &d_st, I_45.size(0), I_45.size(1));
    loop_ub = I_45.size(0) * I_45.size(1);
    for (i = 0; i < loop_ub; i++) {
      I_45_y[i] = I_45[i];
    }
    ...
    I_45.set_size(&be_emlrtRTEI, &d_st, r1.size(0), r1.size(1));
    loop_ub = r1.size(0) * r1.size(1);
    for (i = 0; i < loop_ub; i++) {
      I_45[i] = r1[i] + r2[i];
    }
    ...
    emlrtSizeEqCheckNDR2012b(&varargin_1[0], &varargin_2[0], &f_emlrtECI,
                             &d_st);
    cxy.set_size(&ce_emlrtRTEI, &d_st, b.size(0), b.size(1));
    loop_ub = b.size(0) * b.size(1);
    for (i = 0; i < loop_ub; i++) {
      cxy[i] = b[i] - I_45_y[i];
    }
    ...
    r3.set_size(&de_emlrtRTEI, &d_st, trueCount);
    trueCount = 0;
    for (loop_ub = 0; loop_ub <= end; loop_ub++) {
      if (b[loop_ub] - I_45_y[loop_ub] < 0.0F) {
    ...
    r4.set_size(&de_emlrtRTEI, &d_st, trueCount);
    trueCount = 0;
    for (loop_ub = 0; loop_ub <= end; loop_ub++) {
      if (b[loop_ub] < 0.0F) {
    ...
    b_points0.set_size(&ee_emlrtRTEI, &d_st, points0.size(0));
    for (i = 0; i < loop_ub; i++) {
      b_points0[i] = points0[i + points0.size(0)];
    }
    loop_ub = points0.size(0);
    c_points0.set_size(&fe_emlrtRTEI, &d_st, points0.size(0));
    for (i = 0; i < loop_ub; i++) {
    ...
    s.set_size(&ge_emlrtRTEI, &d_st, r3.size(0));
    loop_ub = r3.size(0);
    for (i = 0; i < loop_ub; i++) {
      s[i] = r3[i];
    }
    ...
    b_points0.set_size(&he_emlrtRTEI, &c_st, s.size(0));
    loop_ub = s.size(0);
    for (i = 0; i < loop_ub; i++) {
    ...
    imagePoints.set_size(&ie_emlrtRTEI, &c_st, 0, 0);
    boardSize[0] = 0.0;
    boardSize[1] = 0.0;
    ...
    loop_ub = points0.size(0);
    b_points0.set_size(&je_emlrtRTEI, &d_st, points0.size(0));
    for (i = 0; i < loop_ub; i++) {
      b_points0[i] = points0[i + points0.size(0)];
    }
    loop_ub = points0.size(0);
    c_points0.set_size(&ke_emlrtRTEI, &d_st, points0.size(0));
    for (i = 0; i < loop_ub; i++) {
      c_points0[i] = points0[i];
    }
    varargin_1[0] = (*(int32_T(*)[2])b.size())[0];
    ...
    e_st.site = &rf_emlrtRSI;
    eml_sub2ind(&e_st, varargin_1, b_points0, c_points0, r3);
    s.set_size(&le_emlrtRTEI, &d_st, r3.size(0));
    loop_ub = r3.size(0);
    for (i = 0; i < loop_ub; i++) {
      s[i] = r3[i];
    }
    ...
    b_points0.set_size(&me_emlrtRTEI, &c_st, s.size(0));
    loop_ub = s.size(0);
    for (i = 0; i < loop_ub; i++) {
  ...
  if ((imagePoints.size(0) != 0) && (imagePoints.size(1) != 0)) {
    ...
    r.set_size(&pe_emlrtRTEI, &st, imagePoints.size(0));
    for (i = 0; i < loop_ub; i++) {
