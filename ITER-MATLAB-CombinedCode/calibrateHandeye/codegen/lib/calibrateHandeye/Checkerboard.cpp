                               ...
                               ::coder::array<double, 2U> &matchedIdx)
{
  ...
  c_ii = arr.size(1);
  for (idx = 0; idx <= (c_ii - 3); idx++) {
    bool y;
    d_ii.set_size(1, 3);
    for (nx = 0; nx < 3; nx++) {
      d_ii[nx] = static_cast<int>(arr[idx + nx]);
    }
    c_x[0] = (d_ii[0] == 1);
    c_x[1] = (d_ii[1] == 1);
    c_x[2] = (d_ii[2] == 1);
  ...
  for (c_ii = 0; c_ii < nx; c_ii++) {
    b_x[c_ii] = (matchArr[c_ii] == 1);
  }
  nx = b_x.size(1);
  idx = 0;
  d_ii.set_size(1, b_x.size(1));
  c_ii = 0;
  exitg1 = false;
  ...
  if (b_x.size(1) == 1) {
    ...
    d_ii.set_size(d_ii.size(0), idx);
  }
  matchedIdx.set_size(1, d_ii.size(1));
  nx = d_ii.size(1);
  for (c_ii = 0; c_ii < nx; c_ii++) {
    matchedIdx[c_ii] = static_cast<double>(d_ii[c_ii]);
  }
}
    ...
    if (c_ii.size(1) >= 2) {
      ...
      coordDist = (coordDist_tmp -
                   BoardCoords[(j + (BoardCoords.size(0) * (c_ii[1] - 1))) +
                               ((BoardCoords.size(0) * BoardCoords.size(1)) *
                                (i1 - 1))]) /
                  ((static_cast<double>(c_ii[1])) - (static_cast<double>(end)));
      loop_ub = 0;
      i2 = c_ii.size(1);
      b_index.set_size(1, c_ii.size(1));
      ...
      for (c_i = 0; c_i < i2; c_i++) {
        ...
        b_index[c_i] =
            BoardCoords[(j + (BoardCoords.size(0) * (c_ii[c_i] - 1))) +
                        ((BoardCoords.size(0) * BoardCoords.size(1)) *
                         (i1 - 1))];
        b_this[c_i] =
            BoardCoords[(j + (BoardCoords.size(0) * (c_ii[c_i] - 1))) +
    ...
    ::coder::array<double, 2U> &newIndices) const
{
  ...
  for (b_idx_tmp = 0; b_idx_tmp < b_i; b_idx_tmp++) {
    ...
    b_eml_find(b_index, c_ii);
    b_validIdx.set_size(1, c_ii.size(1));
    loop_ub = c_ii.size(1);
    for (i1 = 0; i1 < loop_ub; i1++) {
      b_validIdx[i1] = static_cast<double>(c_ii[i1]);
    }
    if (b_validIdx.size(1) >= 2) {
      ...
      bool exitg1;
      if (idx[0] == 1.0) {
        end = static_cast<int>(b_validIdx[0]);
        moveDistMultiplier = static_cast<double>(end);
        trueCount = static_cast<int>(b_validIdx[1]);
        i1 = static_cast<int>(coordsToUse[0]);
        coordDist =
        ...
        moveDistMultiplier = ((static_cast<double>(BoardCoords.size(1))) -
                             ...
                             1.0;
        end = static_cast<int>(b_validIdx[b_validIdx.size(1) - 1]);
        i1 = static_cast<int>(coordsToUse[0]);
        coordDist =
            (BoardCoords
                 [(b_idx_tmp +
                   (BoardCoords.size(0) *
                    ((static_cast<int>(b_validIdx[b_validIdx.size(1) - 1])) -
                     1))) +
                  ((BoardCoords.size(0) * BoardCoords.size(1)) * (i1 - 1))] -
             BoardCoords
                 [(b_idx_tmp +
                   (BoardCoords.size(0) *
                    ((static_cast<int>(b_validIdx[b_validIdx.size(1) - 2])) -
                     1))) +
                  ((BoardCoords.size(0) * BoardCoords.size(1)) * (i1 - 1))]) /
            ((static_cast<double>(
                 static_cast<int>(b_validIdx[b_validIdx.size(1) - 1]))) -
      ...
      for (idx_tmp = 0; idx_tmp < i1; idx_tmp++) {
        if ((static_cast<int>(b_validIdx[idx_tmp])) != 0) {
          trueCount++;
        }
      }
      b_index.set_size(1, b_validIdx.size(1));
      loop_ub = b_validIdx.size(1);
      for (i1 = 0; i1 < loop_ub; i1++) {
        b_index[i1] = BoardCoords[(b_idx_tmp +
                                   (BoardCoords.size(0) *
                                    ((static_cast<int>(b_validIdx[i1])) - 1))) +
                                  ((BoardCoords.size(0) * BoardCoords.size(1)) *
                                   ((static_cast<int>(coordsToUse[0])) - 1))];
      }
      b_this.set_size(1, b_validIdx.size(1));
      loop_ub = b_validIdx.size(1);
      for (i1 = 0; i1 < loop_ub; i1++) {
      ...
      b_polyfit(b_index, b_this, static_cast<double>(trueCount), b_validIdx);
      currRad = coordDist / 4.0;
      refCoordValue =
          BoardCoords[(b_idx_tmp + (BoardCoords.size(0) * (end - 1))) +
                      ((BoardCoords.size(0) * BoardCoords.size(1)) *
      ...
      while ((!exitg1) &&
        ...
        if (isequal(coordsToUse, currPt)) {
          if (b_validIdx.size(1) != 0) {
            ...
            for (idx_tmp = 0; idx_tmp <= (i1 - 2); idx_tmp++) {
              y = (currCoord * y) + b_validIdx[idx_tmp + 1];
            }
          }
          currPt[0] = currCoord;
          currPt[1] = y;
        ...
        findClosestOnCurve(currPt, std::abs(currRad), b_validIdx, coordsToUse,
                           removedIdx, b_index);
        if (b_index.size(1) != 0) {
...
float b_Checkerboard::computeNewEnergyHorizontal(float oldEnergy) const
{
  ...
  if (y) {
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      b_loop_ub = r2.size(0);
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        c_this[d_N + (c_this.size(0) * b_i)] =
            (BoardCoords[(r2[d_N] +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                         1] +
             BoardCoords[((r3[d_N] + (BoardCoords.size(0) * 2)) +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                         1]) -
            (2.0 *
             BoardCoords[((r4[d_N] + BoardCoords.size(0)) +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                         1]);
      }
    }
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        c_this[d_N + (c_this.size(0) * b_i)] =
            BoardCoords[(r5[d_N] +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1] -
            BoardCoords[((r6[d_N] + (BoardCoords.size(0) * 2)) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1];
      }
    }
  ...
  if (validNewColIdx.size(1) != 0) {
    ...
    for (c_i = 0; c_i < b_i; c_i++) {
      ...
      for (d_N = 0; d_N < loop_ub; d_N++) {
        num[d_N] =
            (BoardCoords[((static_cast<int>(d)) +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) * d_N)) -
                         1] +
    ...
    const ::coder::array<double, 2U> &idx, float oldEnergy) const
{
  ...
  if (y) {
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        c_this[d_N + (c_this.size(0) * b_i)] =
            (BoardCoords[((r2[d_N] + (BoardCoords.size(0) * (idx_tmp - 1))) +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                         1] +
             BoardCoords[((r3[d_N] + (BoardCoords.size(0) * (c_idx_tmp - 1))) +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                         1]) -
            (2.0 *
             BoardCoords[((r4[d_N] + (BoardCoords.size(0) * (b_idx_tmp - 1))) +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                         1]);
      }
    }
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        c_this[d_N + (c_this.size(0) * b_i)] =
            BoardCoords[((r5[d_N] + (BoardCoords.size(0) * (idx_tmp - 1))) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1] -
            BoardCoords[((r6[d_N] + (BoardCoords.size(0) * (c_idx_tmp - 1))) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1];
      }
    }
...
float b_Checkerboard::computeNewEnergyVertical(float oldEnergy) const
{
  ...
  if (y) {
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      b_loop_ub = r1.size(1);
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r2[d_N + (r2.size(1) * b_i)] =
            BoardCoords[(BoardCoords.size(0) * (r1[d_N] - 1)) +
                        ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)];
      }
    }
    k = b_this.size(1) - 1;
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      b_loop_ub = r3.size(1);
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r4[d_N + (r4.size(1) * b_i)] =
            BoardCoords[((BoardCoords.size(0) * (r3[d_N] - 1)) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) +
                        2];
      }
    }
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        b[d_N + (b.size(1) * b_i)] =
            BoardCoords[((BoardCoords.size(0) * (r5[d_N] - 1)) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) +
                        1];
      }
    }
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r2[d_N + (r2.size(1) * b_i)] =
            (r2[d_N + (r2.size(1) * b_i)] + r4[d_N + (r4.size(1) * b_i)]) -
            (2.0 * b[d_N + (b.size(1) * b_i)]);
      }
    }
    squeeze(r2, b_num);
    k = b_this.size(1) - 1;
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r2[d_N + (r2.size(1) * b_i)] =
            BoardCoords[(BoardCoords.size(0) * (r6[d_N] - 1)) +
                        ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)];
      }
    }
    k = b_this.size(1) - 1;
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r4[d_N + (r4.size(1) * b_i)] =
            BoardCoords[((BoardCoords.size(0) * (r7[d_N] - 1)) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) +
                        2];
      }
    }
    ...
    const ::coder::array<double, 2U> &idx, float oldEnergy) const
{
  ...
  if (y) {
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r2[d_N + (r2.size(1) * b_i)] =
            BoardCoords[((idx_tmp + (BoardCoords.size(0) * (r1[d_N] - 1))) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1];
      }
    }
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r4[d_N + (r4.size(1) * b_i)] =
            BoardCoords[((c_idx_tmp + (BoardCoords.size(0) * (r3[d_N] - 1))) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1];
      }
    }
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        b[d_N + (b.size(1) * b_i)] =
            BoardCoords[((b_idx_tmp + (BoardCoords.size(0) * (r5[d_N] - 1))) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1];
      }
    }
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r2[d_N + (r2.size(1) * b_i)] =
            BoardCoords[((idx_tmp + (BoardCoords.size(0) * (r6[d_N] - 1))) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1];
      }
    }
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (d_N = 0; d_N < b_loop_ub; d_N++) {
        r4[d_N + (r4.size(1) * b_i)] =
            BoardCoords[((c_idx_tmp + (BoardCoords.size(0) * (r7[d_N] - 1))) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) * b_i)) -
                        1];
      }
    }
    ...
    ::coder::array<double, 2U> &indices) const
{
  ...
  do_vectors(y, b_this, remIdx, c_ii, ib);
  if (remIdx.size(1) != 0) {
    int d_ii;
    int idx;
    bool exitg1;
    ...
    nx = c_ii.size(0);
    for (b_i = 0; b_i < nx; b_i++) {
      validPredictions[b_i] = static_cast<unsigned int>(c_ii[b_i]);
    }
    ...
    for (int c_i{0}; c_i < b_i; c_i++) {
      ...
      if (i2 != 0) {
        ...
        for (k = 0; k <= i2; k++) {
          ...
          for (int c_k{0}; c_k <= i3; c_k++) {
            diffs[c_k + (diffs.size(0) * k)] =
                b_a[(acoef * c_k) + (b_a.size(0) * d_ii)] -
                (static_cast<float>(
      ...
      ::coder::internal::minimum(dists, &b_ex, &nx);
    ...
    ::coder::array<double, 2U> &idx) const
{
  ...
  b_do_vectors(y, removedIdx, remIdx, c_ii, ib);
  ...
  if (queryPts.size(0) != 0) {
    ...
    for (k = 0; k < 2; k++) {
      ...
      for (c_k = 0; c_k <= b_i; c_k++) {
        diffs[c_k + (diffs.size(0) * k)] =
            queryPts[(b_acoef * c_k) + (queryPts.size(0) * b_idx)] -
  ...
  if (acoef > 1) {
    ...
    for (c_i = 0; c_i < b_i; c_i++) {
      ...
      for (k = 0; k < 2; k++) {
        acoef = z1.size(0);
        for (c_k = 0; c_k < acoef; c_k++) {
          z1[c_k + (z1.size(0) * k)] =
              rt_powf_snf(diffs[c_k + (diffs.size(0) * k)], 2.0F);
        }
      }
      if (z1.size(0) == 0) {
        b_y.set_size(0);
      ...
      b_ex = std::sqrt(b_ex);
      dist[c_i] = static_cast<double>(b_ex);
    }
    ::coder::internal::b_minimum(dist, &a_tmp, &acoef);
    s.set_size(dists.size(0));
    b_acoef = dists.size(0);
    for (b_i = 0; b_i < b_acoef; b_i++) {
      s[b_i] = ((static_cast<double>(dists[b_i])) < radius);
    ...
    b_idx = 0;
    c_ii.set_size(b_acoef);
    acoef = 0;
    exitg1 = false;
    while ((!exitg1) && (acoef <= (s.size(0) - 1))) {
      if (s[acoef]) {
        b_idx++;
        c_ii[b_idx - 1] = acoef + 1;
        if (b_idx >= b_acoef) {
          exitg1 = true;
        } else {
          acoef++;
    ...
    dist.set_size(c_ii.size(0));
    b_acoef = c_ii.size(0);
    for (b_i = 0; b_i < b_acoef; b_i++) {
      dist[b_i] = static_cast<double>(c_ii[b_i]);
    }
    idx.set_size(1, 1);
    ...
    ::coder::array<double, 2U> &newIndices) const
{
  ...
  for (c_i = 0; c_i < b_i; c_i++) {
    ...
    if (c_ii.size(0) >= 2) {
      ...
      i1 = static_cast<int>(coordsToUse[0]);
      coordDist_tmp =
          BoardCoords[((c_ii[0] + (BoardCoords.size(0) * c_i)) +
                       ((BoardCoords.size(0) * BoardCoords.size(1)) *
                        (i1 - 1))) -
                      1];
      coordDist =
          (coordDist_tmp -
           BoardCoords[((c_ii[1] + (BoardCoords.size(0) * c_i)) +
                        ((BoardCoords.size(0) * BoardCoords.size(1)) *
                         (i1 - 1))) -
                       1]) /
      ...
      for (loop_ub = 0; loop_ub < i2; loop_ub++) {
        ...
        b_this[loop_ub] =
            BoardCoords[((c_ii[loop_ub] + (BoardCoords.size(0) * c_i)) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) *
                          (i1 - 1))) -
                        1];
        c_this[loop_ub] =
  ...
  for (b_idx_tmp = 0; b_idx_tmp < b_i; b_idx_tmp++) {
    ...
    if (c_ii.size(0) >= 2) {
      ...
      if (idx[0] == 1.0) {
        ...
        coordDist =
            (BoardCoords[((c_ii[0] + (BoardCoords.size(0) * b_idx_tmp)) +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) *
                           (c_i - 1))) -
                         1] -
             BoardCoords[((c_ii[1] + (BoardCoords.size(0) * b_idx_tmp)) +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) *
                           (c_i - 1))) -
                         1]) /
            ((static_cast<double>(c_ii[1])) - (static_cast<double>(c_ii[0])));
        ...
        moveDistMultiplier = ((static_cast<double>(BoardCoords.size(0))) -
                             ...
                             1.0;
        end = c_ii[c_ii.size(0) - 1];
        c_i = static_cast<int>(coordsToUse[0]);
        coordDist = (BoardCoords[((c_ii[c_ii.size(0) - 1] +
                                   (BoardCoords.size(0) * b_idx_tmp)) +
                                  ((BoardCoords.size(0) * BoardCoords.size(1)) *
                                   (c_i - 1))) -
                                 1] -
                     BoardCoords[((c_ii[c_ii.size(0) - 2] +
                                   (BoardCoords.size(0) * b_idx_tmp)) +
                                  ((BoardCoords.size(0) * BoardCoords.size(1)) *
                                   (c_i - 1))) -
                                 1]) /
      ...
      for (idx_tmp = 0; idx_tmp < i1; idx_tmp++) {
        ...
        b_this[idx_tmp] =
            BoardCoords[((c_ii[idx_tmp] + (BoardCoords.size(0) * b_idx_tmp)) +
                         ((BoardCoords.size(0) * BoardCoords.size(1)) *
                          (c_i - 1))) -
                        1];
        c_this[idx_tmp] =
                                ...
                                const float v1[2], const float v2[2])
{
  ...
  if ((Points.size(0) != 0) && (csz_idx_1 != 0)) {
    ...
    for (k = 0; k <= b_i; k++) {
      ...
      for (int c_k{0}; c_k <= i1; c_k++) {
        pointVectors[c_k + (pointVectors.size(0) * k)] =
            Points[(csz_idx_1 * c_k) + (Points.size(0) * d_N)] -
            center[loop_ub];
      }
    }
  }
  euclideanDists.set_size(pointVectors.size(0));
  d_N = pointVectors.size(0);
  for (k = 0; k < d_N; k++) {
    euclideanDists[k] =
  ...
  d_N = BoardIdx.size(0) * BoardIdx.size(1);
  b_x.set_size(d_N);
  ...
  if (y) {
    ...
    d_N = static_cast<int>(BoardIdx[(BoardIdx.size(0) * 2) + 1]);
    loop_ub = Points.size(1);
    c_r.set_size(1, loop_ub);
    for (b_i = 0; b_i < loop_ub; b_i++) {
      c_r[b_i] = Points[(d_N + (Points.size(0) * b_i)) - 1];
    }
    loop_ub = BoardCoords.size(2);
    r.set_size(loop_ub);
    for (b_i = 0; b_i < loop_ub; b_i++) {
    ...
    for (b_i = 0; b_i < loop_ub; b_i++) {
      BoardCoords[((BoardCoords.size(0) * 2) +
                   ((BoardCoords.size(0) * BoardCoords.size(1)) * r[b_i])) +
                  1] = static_cast<double>(c_r[b_i]);
    }
    d_N = static_cast<int>(BoardIdx[1]);
    loop_ub = Points.size(1);
    k_l.set_size(1, loop_ub);
    for (b_i = 0; b_i < loop_ub; b_i++) {
      k_l[b_i] = Points[(d_N + (Points.size(0) * b_i)) - 1];
    }
    loop_ub = BoardCoords.size(2);
    r.set_size(loop_ub);
    for (b_i = 0; b_i < loop_ub; b_i++) {
    ...
    d_N = static_cast<int>(BoardIdx[BoardIdx.size(0) + 2]);
    loop_ub = Points.size(1);
    d.set_size(1, loop_ub);
    for (b_i = 0; b_i < loop_ub; b_i++) {
      d[b_i] = Points[(d_N + (Points.size(0) * b_i)) - 1];
    }
    loop_ub = BoardCoords.size(2);
    r.set_size(loop_ub);
    for (b_i = 0; b_i < loop_ub; b_i++) {
    ...
    d_N = static_cast<int>(BoardIdx[BoardIdx.size(0)]);
    loop_ub = Points.size(1);
    b_u.set_size(1, loop_ub);
    for (b_i = 0; b_i < loop_ub; b_i++) {
      b_u[b_i] = Points[(d_N + (Points.size(0) * b_i)) - 1];
    }
    loop_ub = BoardCoords.size(2);
    r.set_size(loop_ub);
    for (b_i = 0; b_i < loop_ub; b_i++) {
    ...
    if (isValid) {
      ...
      d_N = static_cast<int>(BoardIdx[0]);
      loop_ub = Points.size(1);
      r1.set_size(1, loop_ub);
      for (b_i = 0; b_i < loop_ub; b_i++) {
        r1[b_i] =
            static_cast<double>(Points[(d_N + (Points.size(0) * b_i)) - 1]);
      }
      loop_ub = r.size(0);
      for (b_i = 0; b_i < loop_ub; b_i++) {
      ...
      d_N = static_cast<int>(BoardIdx[2]);
      loop_ub = Points.size(1);
      r1.set_size(1, loop_ub);
      for (b_i = 0; b_i < loop_ub; b_i++) {
        r1[b_i] =
      ...
      d_N = static_cast<int>(BoardIdx[(BoardIdx.size(0) * 2) + 2]);
      loop_ub = Points.size(1);
      r1.set_size(1, loop_ub);
      for (b_i = 0; b_i < loop_ub; b_i++) {
        r1[b_i] =
