                ...
                ::coder::array<float, 2U> &loc)
{
  ...
  if ((static_cast<double>(maxMetric)) <= 4.94065645841247E-324) {
    ...
    b_idx.set_size(c_ii.size(0));
    idx = c_ii.size(0);
    for (b_i = 0; b_i < idx; b_i++) {
      b_idx[b_i] = static_cast<unsigned int>(c_ii[b_i]);
    }
    ...
    c_ii.set_size(b_idx.size(0));
    idx = b_idx.size(0);
    for (b_i = 0; b_i < idx; b_i++) {
      c_ii[b_i] = (static_cast<int>(b_idx[b_i])) - 1;
    }
    vk.set_size(c_ii.size(0));
    idx = c_ii.size(0);
    for (b_i = 0; b_i < idx; b_i++) {
      vk[b_i] = div_s32(c_ii[b_i], static_cast<int>(siz[0]));
    }
    idx = c_ii.size(0);
    for (b_i = 0; b_i < idx; b_i++) {
      c_ii[b_i] = c_ii[b_i] - (vk[b_i] * (static_cast<int>(siz[0])));
    }
    idx = c_ii.size(0);
    for (b_i = 0; b_i < idx; b_i++) {
      loc[b_i + loc.size(0)] =
          static_cast<float>(static_cast<int>(c_ii[b_i] + 1));
    }
    idx = vk.size(0);
    for (b_i = 0; b_i < idx; b_i++) {
