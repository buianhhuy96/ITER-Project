...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
    b_out_, imnh, fparamsAugmented, imnhInds, pind, c_i, i1, b_firstInd, m_i,  \
    exitg1)
  ...
  for (int secondInd = lb_loop; secondInd <= b_i; secondInd++) {
    ...
    for (b_firstInd = c_i; b_firstInd <= i1; b_firstInd++) {
      ...
      for (m_i = 0; m_i < 9; m_i++) {
        imnhInds[m_i] = b_imageNeighborLinearOffsets[m_i] + pind;
      }
      if (in.size(0) == 1) {
        imnh.set_size(1, 9);
        for (m_i = 0; m_i < 9; m_i++) {
          imnh[imnh.size(0) * m_i] = in[imnhInds[m_i] - 1];
        }
      } else {
        imnh.set_size(9, 1);
        for (m_i = 0; m_i < 9; m_i++) {
          imnh[m_i] = in[imnhInds[m_i] - 1];
        }
      }
