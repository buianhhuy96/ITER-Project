               ...
               ::coder::array<real_T, 1U> &Y)
{
  ...
  c_st.prev = &b_st;
  c_st.tls = b_st.tls;
  Y.set_size(&wj_emlrtRTEI, sp, A.size(1));
  loop_ub = A.size(1);
  for (i = 0; i < loop_ub; i++) {
    Y[i] = 0.0;
  }
  ...
  if ((A.size(0) != 0) && (A.size(1) != 0) && (B.size(0) != 0)) {
    ...
    if (lapack::infocheck(&c_st, (int32_T)nrc_t)) {
      loop_ub = B.size(0);
      B.set_size(&xj_emlrtRTEI, &b_st, loop_ub);
      for (i = 0; i < loop_ub; i++) {
        B[i] = rtNaN;
      }
    }
             ...
             const ::coder::array<real_T, 1U> &B, real_T Y[8])
{
  ...
  st.site = &gm_emlrtRSI;
  b_A.set_size(&tr_emlrtRTEI, &st, A.size(0), 8);
  maxmn = A.size(0) * 8;
  ...
  tau.set_size(&ur_emlrtRTEI, &b_st, minmana);
  if (b_A.size(0) == 0) {
    tau.set_size(&vr_emlrtRTEI, &b_st, minmana);
  ...
  st.site = &im_emlrtRSI;
  b_B.set_size(&vj_emlrtRTEI, &st, B.size(0));
  maxmn = B.size(0);
  for (minmn = 0; minmn < maxmn; minmn++) {
    b_B[minmn] = B[minmn];
  }
             ...
             int32_T *rankA)
{
  ...
  c_st.tls = b_st.tls;
  emlrtHeapReferenceStackEnterFcnR2012b((emlrtCTX)sp);
  b_A.set_size(&uj_emlrtRTEI, sp, A.size(0), A.size(1));
  ...
  *rankA = 0;
  ...
  b_B.set_size(&vj_emlrtRTEI, sp, B.size(0));
  minmn = B.size(0) - 1;
  for (k = 0; k <= minmn; k++) {
    b_B[k] = B[k];
  }
