...
int32_T local_rank(const emlrtStack *sp, const ::coder::array<real_T, 2U> &A)
{
  ...
  if (A.size(0) != 0) {
    ...
    if (p) {
      ...
      b_A.set_size(&pr_emlrtRTEI, &d_st, A.size(0), 8);
      nx = A.size(0) * 8;
      for (k = 0; k < nx; k++) {
        b_A[k] = A[k];
      }
      ...
      s.set_size(&qr_emlrtRTEI, &e_st, nx);
      if (nx > 1) {
        superb.set_size(&sr_emlrtRTEI, &e_st, nx - 1);
      } else {
        superb.set_size(&rr_emlrtRTEI, &e_st, 1);
      }
      ...
      s.set_size(&or_emlrtRTEI, &b_st, nx);
      for (k = 0; k < nx; k++) {
        s[k] = rtNaN;
      }
    }
