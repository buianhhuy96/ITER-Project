                ...
                ::coder::array<int32_T, 1U> &i)
{
  ...
  st.site = &mf_emlrtRSI;
  idx = 0;
  i.set_size(&if_emlrtRTEI, &st, x.size(0));
                ...
                ::coder::array<int32_T, 2U> &i)
{
  ...
  if (x.size(1) == 1) {
    if (idx == 0) {
      i.set_size(&jf_emlrtRTEI, &st, 1, 0);
    }
  } else {
    if (1 > idx) {
      nx = 0;
    ...
    b_i.set_size(&jf_emlrtRTEI, &st, 1, nx);
    for (idx = 0; idx < nx; idx++) {
      b_i[idx] = i[idx];
    }
    i.set_size(&jf_emlrtRTEI, &st, 1, b_i.size(1));
    nx = b_i.size(1);
    for (idx = 0; idx < nx; idx++) {
      i[idx] = b_i[idx];
    }
                ...
                real_T kin, ::coder::array<int32_T, 1U> &i)
{
  ...
  idx = 0;
  if (k < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(k), &h_emlrtDCI, &st);
  }
  i.set_size(&if_emlrtRTEI, &st, k);
  b_st.site = &of_emlrtRSI;
  if ((1 <= x.size(0)) && (x.size(0) > 2147483646)) {
    c_st.site = &cd_emlrtRSI;
    check_forloop_overflow_error(&c_st);
              ...
              ::coder::array<int32_T, 1U> &i)
{
  ...
  idx = 0;
  if (nx < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(nx), &h_emlrtDCI, &st);
  }
