                ...
                ::coder::array<real32_T, 2U> &loc)
{
  ...
  if (maxMetric <= 4.94065645841247E-324) {
    loc.set_size(&df_emlrtRTEI, &st, 0, 2);
  } else {
    boolean_T p;
    ...
    bw.set_size(&af_emlrtRTEI, &b_st, metric.size(0), metric.size(1));
    ...
    ii.set_size(&bf_emlrtRTEI, &st, last);
    last = 0;
    for (a = 0; a <= idx; a++) {
      if (metric[a] < maxMetric) {
        ii[last] = a + 1;
    ...
    for (a = 0; a <= last; a++) {
      if ((ii[a] < 1) || (ii[a] > idx)) {
        emlrtDynamicBoundsCheckR2012b(ii[a], 1, idx, &mb_emlrtBCI, &st);
      }
      bw[ii[a] - 1] = false;
    }
    ...
    bwmorph(&b_st, bw);
    if (1 > bw.size(0)) {
      emlrtDynamicBoundsCheckR2012b(1, 1, bw.size(0), &gb_emlrtBCI, &st);
    }
    last = bw.size(1);
    for (a = 0; a < last; a++) {
    ...
    idx = bw.size(0);
    if (bw.size(0) < 0) {
      emlrtDynamicBoundsCheckR2012b(bw.size(0), 0, MAX_int32_T, &hb_emlrtBCI,
                                    &st);
    }
    if (bw.size(0) < 1) {
      emlrtDynamicBoundsCheckR2012b(bw.size(0), 1, bw.size(0), &ib_emlrtBCI,
                                    &st);
    }
    last = bw.size(1);
    for (a = 0; a < last; a++) {
    ...
    if (1 > bw.size(1)) {
      emlrtDynamicBoundsCheckR2012b(1, 1, bw.size(1), &jb_emlrtBCI, &st);
    }
    last = bw.size(0);
    for (a = 0; a < last; a++) {
      bw[a] = false;
    ...
    idx = bw.size(1);
    if (bw.size(1) < 0) {
      emlrtDynamicBoundsCheckR2012b(bw.size(1), 0, MAX_int32_T, &kb_emlrtBCI,
                                    &st);
    }
    if (bw.size(1) < 1) {
      emlrtDynamicBoundsCheckR2012b(bw.size(1), 1, bw.size(1), &lb_emlrtBCI,
                                    &st);
    }
    last = bw.size(0);
    for (a = 0; a < last; a++) {
    ...
    eml_find(&c_st, bw, ii);
    b_idx.set_size(&cf_emlrtRTEI, &b_st, ii.size(0));
    last = ii.size(0);
    for (a = 0; a < last; a++) {
      b_idx[a] = ii[a];
    }
    imSizeT[0] = b_idx.size(0);
    loc.set_size(&df_emlrtRTEI, &st, b_idx.size(0), 2);
    last = b_idx.size(0) << 1;
    for (a = 0; a < last; a++) {
      loc[a] = 0.0F;
    }
    ...
    vk.set_size(&ef_emlrtRTEI, &c_st, ii.size(0));
    last = ii.size(0);
    for (a = 0; a < last; a++) {
    ...
    b_idx.set_size(&ff_emlrtRTEI, &b_st, ii.size(0));
    last = ii.size(0);
    for (a = 0; a < last; a++) {
      b_idx[a] = ii[a] + 1;
    }
    ...
    emlrtSubAssignSizeCheckR2012b(ii.size(), 1, b_idx.size(), 1, &l_emlrtECI,
                                  &st);
    last = b_idx.size(0);
    for (a = 0; a < last; a++) {
    ...
    last = loc.size(0);
    ii.set_size(&hf_emlrtRTEI, &st, loc.size(0));
    for (a = 0; a < last; a++) {
      ii[a] = a;
    }
    emlrtSubAssignSizeCheckR2012b(ii.size(), 1, vk.size(), 1, &m_emlrtECI, &st);
    last = vk.size(0);
    for (a = 0; a < last; a++) {
