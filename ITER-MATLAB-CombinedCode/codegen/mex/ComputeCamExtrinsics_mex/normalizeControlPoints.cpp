                            ...
                            real_T normMatrixInv[9])
{
  ...
  if ((pts.size(0) == 0) || (pts.size(1) == 0)) {
    cent.set_size(&fr_emlrtRTEI, &c_st, 1, pts.size(1));
    ncols = pts.size(1);
    ...
    cent.set_size(&er_emlrtRTEI, &e_st, 1, pts.size(1));
    ncols = pts.size(1) - 1;
    if (pts.size(0) < 4096) {
      f_st.site = &vx_emlrtRSI;
  ...
  cent.set_size(&gr_emlrtRTEI, &st, 1, cent.size(1));
  ncols = cent.size(1) - 1;
  for (nfb = 0; nfb <= ncols; nfb++) {
  ...
  ptsNorm.set_size(&xf_emlrtRTEI, &st, pts.size(0), csz_idx_1);
  ...
  st.site = &tx_emlrtRSI;
  if (1 > ptsNorm.size(1)) {
    emlrtDynamicBoundsCheckR2012b(1, 1, ptsNorm.size(1), &pt_emlrtBCI, &st);
  }
  if (2 > ptsNorm.size(1)) {
    emlrtDynamicBoundsCheckR2012b(2, 1, ptsNorm.size(1), &qt_emlrtBCI, &st);
  }
  ncols = ptsNorm.size(0);
  b_ptsNorm.set_size(&hr_emlrtRTEI, &st, ptsNorm.size(0));
  for (nfb = 0; nfb < ncols; nfb++) {
    b_ptsNorm[nfb] = ptsNorm[nfb];
  }
  ncols = ptsNorm.size(0);
  c_ptsNorm.set_size(&ir_emlrtRTEI, &st, ptsNorm.size(0));
  for (nfb = 0; nfb < ncols; nfb++) {
  ...
  if (1 > cent.size(1)) {
    emlrtDynamicBoundsCheckR2012b(1, 1, cent.size(1), &rt_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  if (2 > cent.size(1)) {
    emlrtDynamicBoundsCheckR2012b(2, 1, cent.size(1), &st_emlrtBCI,
                                  (emlrtCTX)sp);
  }
