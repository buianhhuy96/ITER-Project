            ...
            ::coder::array<real_T, 1U> &tau, ::coder::array<int32_T, 2U> &jpvt)
{
  ...
  m = A.size(0);
  na = A.size(1);
  jpvt.set_size(&qv_emlrtRTEI, sp, 1, jpvt.size(1));
  i = A.size(1);
  if (i < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(i), &tg_emlrtDCI,
                                (emlrtCTX)sp);
  }
  jpvt.set_size(&qv_emlrtRTEI, sp, jpvt.size(0), i);
  b_na = A.size(1);
  if (b_na < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(b_na), &ug_emlrtDCI,
                                (emlrtCTX)sp);
  }
  for (i = 0; i < b_na; i++) {
    jpvt[i] = 0;
  ...
  if (minmana < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(minmana), &vg_emlrtDCI,
                                &st);
  }
  tau.set_size(&pr_emlrtRTEI, &st, minmana);
  if ((A.size(0) == 0) || (A.size(1) == 0) || (A.size(0) < 1) ||
      (A.size(1) < 1)) {
    tau.set_size(&qr_emlrtRTEI, &st, minmana);
    for (i = 0; i < minmana; i++) {
      tau[i] = 0.0;
    }
    ...
    boolean_T p;
    jpvt_t.set_size(&rv_emlrtRTEI, &st, A.size(1));
    b_na = A.size(1);
    for (i = 0; i < b_na; i++) {
