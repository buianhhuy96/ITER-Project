...
void rotm2axang(const emlrtStack *sp, const real_T R[9], real_T axang[4])
{
  ...
  if (rEQ0 || b) {
    int32_T i;
    int32_T loop_ub;
    vspecial.set_size(&bs_emlrtRTEI, sp, 3, nz_tmp);
    loop_ub = 3 * nz_tmp;
    for (i = 0; i < loop_ub; i++) {
      vspecial[i] = 0.0;
    }
    if (rEQ0 || b) {
      ii.set_size(&il_emlrtRTEI, sp, 1, 1);
      ii[0] = 1;
    } else {
      ii.set_size(&il_emlrtRTEI, sp, 0, 0);
    }
    for (loop_ub = 0; loop_ub < nz_tmp; loop_ub++) {
      i = ii.size(0) * ii.size(1);
      if (1 > i) {
        emlrtDynamicBoundsCheckR2012b(1, 1, i, &gu_emlrtBCI, (emlrtCTX)sp);
      }
      for (i = 0; i < 9; i++) {
        b_I[i] = 0;
      }
      ...
      if (1 > vspecial.size(1)) {
        emlrtDynamicBoundsCheckR2012b(1, 1, vspecial.size(1), &hu_emlrtBCI,
                                      (emlrtCTX)sp);
      }
      vspecial[0] = V[6];
      vspecial[1] = V[7];
