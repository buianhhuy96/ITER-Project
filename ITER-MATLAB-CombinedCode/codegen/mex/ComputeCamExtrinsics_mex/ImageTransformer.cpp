    ...
    ::coder::array<uint8_T, 3U> &J, real_T newOrigin[2]) const
{
  ...
  if (fillValues.size(2) == 1) {
    ...
    fill.set_size(&yt_emlrtRTEI, &b_st, b_I.size(2), 1, 1);
    if (b_I.size(2) != 0) {
      k = b_I.size(2) - 1;
      for (t = 0; t <= k; t++) {
        fill[t] = fillValues[0];
    ...
    fill.set_size(&xt_emlrtRTEI, &st, 1, 1, fillValues.size(2));
    k = fillValues.size(2);
    for (t = 0; t < k; t++) {
      fill[t] = fillValues[t];
    }
  ...
  if (!(outputImageSize[0] >= 0.0)) {
    emlrtNonNegativeCheckR2012b(outputImageSize[0], &og_emlrtDCI, &b_st);
  }
  if (!(outputImageSize[1] >= 0.0)) {
    emlrtNonNegativeCheckR2012b(outputImageSize[1], &og_emlrtDCI, &b_st);
  }
  J.set_size(&au_emlrtRTEI, &b_st, static_cast<int32_T>(outputImageSize[0]),
  ...
  if (t < 0) {
    emlrtDynamicBoundsCheckR2012b(t, 0, MAX_int32_T, &tu_emlrtBCI, &c_st);
  }
                                ...
                                const real_T yBounds[2])
{
  ...
  d_st.tls = c_st.tls;
  emlrtHeapReferenceStackEnterFcnR2012b((emlrtCTX)sp);
  SizeOfImage.set_size(&us_emlrtRTEI, sp, 1, 3);
  SizeOfImage[0] = b_I.size(0);
  SizeOfImage[1] = b_I.size(1);
  SizeOfImage[2] = b_I.size(2);
  ClassOfImage.set_size(&vs_emlrtRTEI, sp, 1, 5);
  for (i = 0; i < 5; i++) {
    ClassOfImage[i] = b_cv[i];
  }
  OutputView.set_size(&ws_emlrtRTEI, sp, 1, 4);
  OutputView[0] = 's';
  OutputView[1] = 'a';
  OutputView[2] = 'm';
  OutputView[3] = 'e';
  ...
  if (muDoubleScalarIsNaN(m) || muDoubleScalarIsNaN(n)) {
    y.set_size(&tj_emlrtRTEI, &c_st, 1, 1);
    y[0] = rtNaN;
  } else if (n < m) {
    y.set_size(&tj_emlrtRTEI, &c_st, 1, 0);
    ...
    y.set_size(&tj_emlrtRTEI, &c_st, 1, loop_ub + 1);
    for (i = 0; i <= loop_ub; i++) {
  ...
  c_st.site = &fcb_emlrtRSI;
  if (muDoubleScalarIsNaN(m) || muDoubleScalarIsNaN(n)) {
    b_y.set_size(&tj_emlrtRTEI, &c_st, 1, 1);
    b_y[0] = rtNaN;
  } else if (n < m) {
    b_y.set_size(&tj_emlrtRTEI, &c_st, 1, 0);
  } else if ((muDoubleScalarIsInf(m) || muDoubleScalarIsInf(n)) && (m == n)) {
    b_y.set_size(&tj_emlrtRTEI, &c_st, 1, 1);
    b_y[0] = rtNaN;
  } else if (muDoubleScalarFloor(m) == m) {
    loop_ub = static_cast<int32_T>(muDoubleScalarFloor(n - m));
    b_y.set_size(&tj_emlrtRTEI, &c_st, 1, loop_ub + 1);
    for (i = 0; i <= loop_ub; i++) {
  ...
  c_X.set_size(&xs_emlrtRTEI, &st, maxdimlen, 2);
  ...
  if (b_bool) {
    ...
    if (static_cast<int32_T>(m) > maxdimlen) {
      emlrtErrorWithMessageIdR2018a(
          &b_st, &tb_emlrtRTEI, "Coder:toolbox:reshape_emptyReshapeLimit",
    ...
    if (i != ptsOut.size(0)) {
      emlrtErrorWithMessageIdR2018a(
          &b_st, &ub_emlrtRTEI, "Coder:MATLAB:getReshapeDims_notSameNumel",
    ...
    b_Y = ptsOut.size(0);
    b_ptsOut.set_size(&ys_emlrtRTEI, &st, ptsOut.size(0));
    for (i = 0; i < b_Y; i++) {
      b_ptsOut[i] = ptsOut[i];
    }
    XmapSingle.set_size(&at_emlrtRTEI, &st, static_cast<int32_T>(m),
    ...
    b_Y = ptsOut.size(0);
    b_ptsOut.set_size(&bt_emlrtRTEI, &st, ptsOut.size(0));
    for (i = 0; i < b_Y; i++) {
      b_ptsOut[i] = ptsOut[i + ptsOut.size(0)];
    }
    YmapSingle.set_size(&ct_emlrtRTEI, &st, static_cast<int32_T>(m),
                        static_cast<int32_T>(n));
    for (i = 0; i < loop_ub; i++) {
