                ...
                ::coder::array<real32_T, 2U> &loc)
{
  ...
  if (maxMetric <= 4.94065645841247E-324) {
    loc.set_size(&xe_emlrtRTEI, &st, 0, 2);
  } else {
    boolean_T p;
    ...
    bw.set_size(&ue_emlrtRTEI, &b_st, metric.size(0), metric.size(1));
    ...
    for (a = 0; a <= last; a++) {
      if ((ii[a] < 1) || (ii[a] > idx)) {
        emlrtDynamicBoundsCheckR2012b(ii[a], 1, idx, &jb_emlrtBCI, &st);
      }
      bw[ii[a] - 1] = false;
    }
    ...
    bwmorph(&b_st, bw);
    if (1 > bw.size(0)) {
      emlrtDynamicBoundsCheckR2012b(1, 1, bw.size(0), &db_emlrtBCI, &st);
    }
    last = bw.size(1);
    for (a = 0; a < last; a++) {
    ...
    idx = bw.size(0);
    if (bw.size(0) < 0) {
      emlrtDynamicBoundsCheckR2012b(bw.size(0), 0, MAX_int32_T, &eb_emlrtBCI,
                                    &st);
    }
    if (bw.size(0) < 1) {
      emlrtDynamicBoundsCheckR2012b(bw.size(0), 1, bw.size(0), &fb_emlrtBCI,
                                    &st);
    }
    last = bw.size(1);
    for (a = 0; a < last; a++) {
    ...
    if (1 > bw.size(1)) {
      emlrtDynamicBoundsCheckR2012b(1, 1, bw.size(1), &gb_emlrtBCI, &st);
    }
    last = bw.size(0);
    for (a = 0; a < last; a++) {
      bw[a] = false;
    ...
    idx = bw.size(1);
    if (bw.size(1) < 0) {
      emlrtDynamicBoundsCheckR2012b(bw.size(1), 0, MAX_int32_T, &hb_emlrtBCI,
                                    &st);
    }
    if (bw.size(1) < 1) {
      emlrtDynamicBoundsCheckR2012b(bw.size(1), 1, bw.size(1), &ib_emlrtBCI,
                                    &st);
    }
    last = bw.size(0);
    for (a = 0; a < last; a++) {
    ...
    eml_find(&c_st, bw, ii);
    b_idx.set_size(&we_emlrtRTEI, &b_st, ii.size(0));
    last = ii.size(0);
    for (a = 0; a < last; a++) {
      b_idx[a] = ii[a];
    }
    imSizeT[0] = b_idx.size(0);
    loc.set_size(&xe_emlrtRTEI, &st, b_idx.size(0), 2);
    last = b_idx.size(0) << 1;
    for (a = 0; a < last; a++) {
      loc[a] = 0.0F;
    }
    ...
    vk.set_size(&ye_emlrtRTEI, &c_st, ii.size(0));
    last = ii.size(0);
    for (a = 0; a < last; a++) {
    ...
    b_idx.set_size(&af_emlrtRTEI, &b_st, ii.size(0));
    last = ii.size(0);
    for (a = 0; a < last; a++) {
      b_idx[a] = ii[a] + 1;
    }
    ...
    last = loc.size(0);
    ii.set_size(&cf_emlrtRTEI, &st, loc.size(0));
    for (a = 0; a < last; a++) {
      ii[a] = a;
    }
