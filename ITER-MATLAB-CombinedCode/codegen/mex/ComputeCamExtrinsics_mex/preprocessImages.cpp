...
static emlrtRSInfo gcb_emlrtRSI{
    ...
    "CombinedCode\\calibrateHandeye\\preprocessImages.m" // pathName
};
...
static emlrtECInfo id_emlrtECI{
    ...
    "D:\\TUT_Research\\Gitlab\\f4e-grt\\ITER-MATLAB-"
    "CombinedCode\\calibrateHandeye\\preprocessImages.m" // pName
};
                      ...
                      coder::array<uint8_T, 4U> &gray_images)
{
  ...
  gray_images.set_size(&gs_emlrtRTEI, sp, images.size(0), images.size(1), 1,
                       images.size(3));
  ...
  for (int32_T b_i{0}; b_i < i; b_i++) {
    ...
    if (b_i + 1 > images.size(3)) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, images.size(3), &su_emlrtBCI,
                                    (emlrtCTX)sp);
    }
    loop_ub = images.size(0);
    b_loop_ub = images.size(1);
    c_loop_ub = images.size(2);
    current_images.set_size(&hs_emlrtRTEI, sp, images.size(0), images.size(1),
                            images.size(2));
    ...
    if (covrtLogIf(&emlrtCoverageInstance, 4, 0, 0, images.size(2) == 3)) {
      covrtLogBasicBlock(&emlrtCoverageInstance, 4, 2);
      st.site = &hcb_emlrtRSI;
      if (b_i + 1 > images.size(3)) {
        emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, images.size(3), &ru_emlrtBCI,
                                      &st);
      }
      loop_ub = images.size(0);
      b_loop_ub = images.size(1);
      current_images.set_size(&is_emlrtRTEI, &st, images.size(0),
                              images.size(1), 3);
      for (i1 = 0; i1 < 3; i1++) {
        for (k = 0; k < b_loop_ub; k++) {
          for (i2 = 0; i2 < loop_ub; i2++) {
      ...
      b_current_images.set_size(&js_emlrtRTEI, &st, images.size(0),
                                images.size(1));
      ...
      current_images.set_size(&ks_emlrtRTEI, sp, b_current_images.size(0),
    ...
    r.set_size(&ls_emlrtRTEI, sp, gray_images.size(0));
    for (i1 = 0; i1 < loop_ub; i1++) {
      r[i1] = i1;
    }
    loop_ub = gray_images.size(1);
    r1.set_size(&ms_emlrtRTEI, sp, gray_images.size(1));
    for (i1 = 0; i1 < loop_ub; i1++) {
      r1[i1] = i1;
    }
    ...
    if (current_images.size(2) > 1) {
      fillValues.set_size(&ns_emlrtRTEI, &b_st, 1, 1, 3);
      fillValues[0] = 0U;
      fillValues[1] = 0U;
      fillValues[2] = 0U;
    } else {
      fillValues.set_size(&ns_emlrtRTEI, &b_st, 1, 1, 1);
      fillValues[0] = 0U;
    }
    ...
    if (!p) {
      emlrtErrorWithMessageIdR2018a(
          &b_st, &lc_emlrtRTEI, "vision:calibrate:inconsistentImageSize",
    ...
    b.set_size(&os_emlrtRTEI, &c_st, 1,
    ...
    b.set_size(&ps_emlrtRTEI, &c_st, 1,
    ...
    b_J.set_size(&qs_emlrtRTEI, &b_st, current_images.size(0),
                 current_images.size(1), current_images.size(2));
    if (1 > J.size(0)) {
      emlrtDynamicBoundsCheckR2012b(1, 1, J.size(0), &pu_emlrtBCI, &b_st);
    }
    ...
    if (1 > J.size(1)) {
      emlrtDynamicBoundsCheckR2012b(1, 1, J.size(1), &nu_emlrtBCI, &b_st);
    }
    ...
    if (1 > J.size(2)) {
      emlrtDynamicBoundsCheckR2012b(1, 1, J.size(2), &lu_emlrtBCI, &b_st);
    }
    ...
    r2.set_size(&rs_emlrtRTEI, &b_st, current_images.size(0));
    for (i1 = 0; i1 < k; i1++) {
      r2[i1] = i1;
    }
    k = current_images.size(1);
    r3.set_size(&ss_emlrtRTEI, &b_st, current_images.size(1));
    for (i1 = 0; i1 < k; i1++) {
      r3[i1] = i1;
    }
    r4.set_size(&ts_emlrtRTEI, &b_st,
