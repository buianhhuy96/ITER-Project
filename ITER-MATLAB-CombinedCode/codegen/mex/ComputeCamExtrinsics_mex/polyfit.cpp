             ...
             ::coder::array<real_T, 2U> &p)
{
  ...
  st.site = &em_emlrtRSI;
  b_n = x.size(0) - 1;
  V.set_size(&nj_emlrtRTEI, &st, x.size(0), V.size(1));
  if (!(n + 1.0 >= 0.0)) {
    emlrtNonNegativeCheckR2012b(n + 1.0, &mb_emlrtDCI, &st);
  }
  V.set_size(&nj_emlrtRTEI, &st, V.size(0), static_cast<int32_T>(n + 1.0));
  ...
  p.set_size(&oj_emlrtRTEI, sp, 1, p1.size(0));
  b_n = p1.size(0);
  for (i = 0; i < b_n; i++) {
    p[i] = p1[i];
  }
             ...
             ::coder::array<real_T, 2U> &p)
{
  ...
  st.site = &em_emlrtRSI;
  b_n = x.size(1) - 1;
  V.set_size(&nj_emlrtRTEI, &st, x.size(1), V.size(1));
  if (!(n + 1.0 >= 0.0)) {
    emlrtNonNegativeCheckR2012b(n + 1.0, &mb_emlrtDCI, &st);
  }
  V.set_size(&nj_emlrtRTEI, &st, V.size(0), static_cast<int32_T>(n + 1.0));
