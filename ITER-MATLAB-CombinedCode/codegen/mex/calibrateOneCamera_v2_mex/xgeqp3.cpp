            ...
            ::coder::array<real_T, 1U> &tau, ::coder::array<int32_T, 2U> &jpvt)
{
  ...
  m = A.size(0);
  na = A.size(1);
  jpvt.set_size(&uy_emlrtRTEI, sp, 1, jpvt.size(1));
  i = A.size(1);
  if (i < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(i), &hh_emlrtDCI,
                                (emlrtCTX)sp);
  }
  jpvt.set_size(&uy_emlrtRTEI, sp, jpvt.size(0), i);
  b_na = A.size(1);
  if (b_na < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(b_na), &ih_emlrtDCI,
                                (emlrtCTX)sp);
  }
  for (i = 0; i < b_na; i++) {
    jpvt[i] = 0;
  ...
  tau.set_size(&os_emlrtRTEI, &st, minmana);
  if ((A.size(0) == 0) || (A.size(1) == 0) || (A.size(0) < 1) ||
      (A.size(1) < 1)) {
    tau.set_size(&ps_emlrtRTEI, &st, minmana);
    for (i = 0; i < minmana; i++) {
      tau[i] = 0.0;
    }
    ...
    boolean_T p;
    jpvt_t.set_size(&vy_emlrtRTEI, &st, A.size(1));
    b_na = A.size(1);
    for (i = 0; i < b_na; i++) {
    ...
    if (b_na != 0) {
      ...
      if (b_na != -4) {
        if (b_na == -1010) {
          emlrtErrorWithMessageIdR2018a(&b_st, &ib_emlrtRTEI, "MATLAB:nomem",
                                        "MATLAB:nomem", 0);
        } else {
          emlrtErrorWithMessageIdR2018a(
              &b_st, &jb_emlrtRTEI, "Coder:toolbox:LAPACKCallErrorInfo",
