                            ...
                            real_T normMatrixInv[9])
{
  ...
  if ((pts.size(0) == 0) || (pts.size(1) == 0)) {
    cent.set_size(&es_emlrtRTEI, &c_st, 1, pts.size(1));
    ncols = pts.size(1);
    ...
    cent.set_size(&ds_emlrtRTEI, &e_st, 1, pts.size(1));
    ncols = pts.size(1) - 1;
    if (pts.size(0) < 4096) {
  ...
  cent.set_size(&fs_emlrtRTEI, &st, 1, cent.size(1));
  ncols = cent.size(1) - 1;
  for (nfb = 0; nfb <= ncols; nfb++) {
  ...
  ptsNorm.set_size(&ig_emlrtRTEI, &st, pts.size(0), csz_idx_1);
  ...
  st.site = &qx_emlrtRSI;
  if (1 > ptsNorm.size(1)) {
    emlrtDynamicBoundsCheckR2012b(1, 1, ptsNorm.size(1), &bu_emlrtBCI, &st);
  }
  if (2 > ptsNorm.size(1)) {
    emlrtDynamicBoundsCheckR2012b(2, 1, ptsNorm.size(1), &cu_emlrtBCI, &st);
  }
  ncols = ptsNorm.size(0);
  b_ptsNorm.set_size(&gs_emlrtRTEI, &st, ptsNorm.size(0));
  for (nfb = 0; nfb < ncols; nfb++) {
    b_ptsNorm[nfb] = ptsNorm[nfb];
  }
  ncols = ptsNorm.size(0);
  c_ptsNorm.set_size(&hs_emlrtRTEI, &st, ptsNorm.size(0));
  for (nfb = 0; nfb < ncols; nfb++) {
  ...
  if (1 > cent.size(1)) {
    emlrtDynamicBoundsCheckR2012b(1, 1, cent.size(1), &du_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  if (2 > cent.size(1)) {
    emlrtDynamicBoundsCheckR2012b(2, 1, cent.size(1), &eu_emlrtBCI,
                                  (emlrtCTX)sp);
  }
