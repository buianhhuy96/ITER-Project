                               ...
                               real_T delta, ::coder::array<real_T, 1U> &xk)
{
  ...
  i = xk.size(0);
  if ((dim < 1) || (dim > i)) {
    emlrtDynamicBoundsCheckR2012b(dim, 1, i, &fab_emlrtBCI, (emlrtCTX)sp);
  }
  temp_tmp = xk[dim - 1];
  i = xk.size(0);
  if (dim > i) {
  ...
  if (!obj_SpecifyConstraintGradient) {
    ...
    if (i != varargout_2.size(0)) {
      emlrtSubAssignSizeCheck1dR2017a(i, varargout_2.size(0), &pe_emlrtECI,
                                      (emlrtCTX)sp);
    }
    cEqPlus.set_size(&qab_emlrtRTEI, sp, varargout_2.size(0));
    ...
    loop_ub = 1;
    while (1 <= obj_mIneq) {
      emlrtDynamicBoundsCheckR2012b(1, 1, 0, &gab_emlrtBCI, (emlrtCTX)sp);
    }
    while (evalOK && (loop_ub <= obj_mEq)) {
      real_T evalOK_tmp;
      i = cEqPlus.size(0);
      if ((loop_ub < 1) || (loop_ub > i)) {
        emlrtDynamicBoundsCheckR2012b(loop_ub, 1, i, &fab_emlrtBCI,
                                      (emlrtCTX)sp);
      }
