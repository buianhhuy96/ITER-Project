            ...
            const ::coder::array<real_T, 1U> &b, ::coder::array<real_T, 2U> &c)
{
  ...
  c.set_size(&ig_emlrtRTEI, sp, csz_idx_0, 2);
  if (csz_idx_0 != 0) {
    int32_T bcoef;
            ...
            const real_T b[2], ::coder::array<real32_T, 2U> &c)
{
  ...
  c.set_size(&ig_emlrtRTEI, sp, a.size(0), 2);
  if (a.size(0) != 0) {
    int32_T acoef;
    int32_T b_acoef;
    acoef = (a.size(1) != 1);
            ...
            const ::coder::array<real_T, 2U> &b, ::coder::array<real_T, 2U> &c)
{
  ...
  c.set_size(&ig_emlrtRTEI, sp, 3, csz_idx_1);
  if (csz_idx_1 != 0) {
    int32_T acoef;
    int32_T bcoef;
    acoef = (a.size(1) != 1);
...
void bsxfun(const emlrtStack *sp, const ::coder::array<real_T, 2U> &a,
            const real_T b[2], ::coder::array<real_T, 2U> &c)
{
  c.set_size(&ig_emlrtRTEI, sp, a.size(0), 2);
  if (a.size(0) != 0) {
    int32_T acoef;
    ...
    for (int32_T k{0}; k < 2; k++) {
      ...
      b_b = c.size(0) - 1;
      for (int32_T b_k{0}; b_k <= b_b; b_k++) {
        c[b_k + c.size(0) * k] = a[acoef * b_k + a.size(0) * k] - b[k];
      }
    }
  }
}
