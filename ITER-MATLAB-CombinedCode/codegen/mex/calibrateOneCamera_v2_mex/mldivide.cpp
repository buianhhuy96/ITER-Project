...
static const char_T cv4[19]{'L', 'A', 'P', 'A', 'C', 'K', 'E', '_', 'd', 'g',
                            'e', 't', 'r', 'f', '_', 'w', 'o', 'r', 'k'};
              ...
              ::coder::array<real_T, 1U> &B)
{
  ...
  if (B.size(0) != A.size(0)) {
    emlrtErrorWithMessageIdR2018a(sp, &xc_emlrtRTEI, "MATLAB:dimagree",
                                  "MATLAB:dimagree", 0);
  }
  st.site = &fy_emlrtRSI;
  if ((A.size(0) == 0) || (B.size(0) == 0)) {
    B.set_size(&hw_emlrtRTEI, &st, 8);
    ...
    f_st.site = &my_emlrtRSI;
    b_A.set_size(&lab_emlrtRTEI, &f_st, 8, 8);
    for (kAcol = 0; kAcol < 64; kAcol++) {
      b_A[kAcol] = A[kAcol];
    }
    ptrdiff_t info_t;
    g_st.site = &oy_emlrtRSI;
    info_t = LAPACKE_dgetrf_work(102, (ptrdiff_t)8, (ptrdiff_t)8,
                                 &(b_A.data())[0], (ptrdiff_t)8, &ipiv_t[0]);
    info = (int32_T)info_t;
    ipiv.set_size(&mab_emlrtRTEI, &g_st, 1, 8);
    h_st.site = &qy_emlrtRSI;
    if (info < 0) {
      if (info == -1010) {
        emlrtErrorWithMessageIdR2018a(&h_st, &ib_emlrtRTEI, "MATLAB:nomem",
                                      "MATLAB:nomem", 0);
      } else {
        emlrtErrorWithMessageIdR2018a(
            &h_st, &jb_emlrtRTEI, "Coder:toolbox:LAPACKCallErrorInfo",
            "Coder:toolbox:LAPACKCallErrorInfo", 5, 4, 19, &cv4[0], 12, info);
      }
    }
    for (k = 0; k < 8; k++) {
              ...
              ::coder::array<real_T, 1U> &Y)
{
  ...
  if ((A.size(0) == 0) || (A.size(1) == 0) || (B.size(0) == 0)) {
    int32_T minmn;
    Y.set_size(&hw_emlrtRTEI, &st, A.size(1));
    minmn = A.size(1);
    ...
    d_st.site = &ky_emlrtRSI;
    e_st.site = &ly_emlrtRSI;
    b_A.set_size(&ew_emlrtRTEI, &e_st, A.size(0), A.size(1));
    minmn = A.size(0) * A.size(1);
    for (maxmn = 0; maxmn < minmn; maxmn++) {
      b_A[maxmn] = A[maxmn];
    }
    Y.set_size(&fw_emlrtRTEI, &e_st, B.size(0));
    minmn = B.size(0);
    for (maxmn = 0; maxmn < minmn; maxmn++) {
      Y[maxmn] = B[maxmn];
    }
    ...
    f_st.site = &phb_emlrtRSI;
    IPIV.set_size(&gw_emlrtRTEI, &f_st, minmn);
    N = (ptrdiff_t)minmn;
    LDA = (ptrdiff_t)b_A.size(0);
    INFO = LAPACKE_dgetrf_work(102, N, N, &(b_A.data())[0], LDA,
                               &(IPIV.data())[0]);
    minmn = (int32_T)INFO;
    g_st.site = &rhb_emlrtRSI;
    if (minmn < 0) {
      if (minmn == -1010) {
        emlrtErrorWithMessageIdR2018a(&g_st, &ib_emlrtRTEI, "MATLAB:nomem",
                                      "MATLAB:nomem", 0);
      } else {
        emlrtErrorWithMessageIdR2018a(
            &g_st, &jb_emlrtRTEI, "Coder:toolbox:LAPACKCallErrorInfo",
            "Coder:toolbox:LAPACKCallErrorInfo", 5, 4, 19, &cv4[0], 12, minmn);
      }
    }
    ...
    int32_T rankA;
    b_st.site = &hy_emlrtRSI;
    b_A.set_size(&ak_emlrtRTEI, &b_st, A.size(0), A.size(1));
    minmn = A.size(0) * A.size(1);
    for (maxmn = 0; maxmn < minmn; maxmn++) {
      b_A[maxmn] = A[maxmn];
    }
    ...
    b_B.set_size(&dw_emlrtRTEI, &b_st, B.size(0));
    minmn = B.size(0) - 1;
    for (maxmn = 0; maxmn <= minmn; maxmn++) {
      b_B[maxmn] = B[maxmn];
    }
