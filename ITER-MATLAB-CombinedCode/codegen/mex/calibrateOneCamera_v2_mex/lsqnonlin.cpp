               ...
               ::coder::array<real_T, 2U> &jacob)
{
  ...
  b.set_size(&nab_emlrtRTEI, &st, xCurrent.size(0));
  loop_ub = xCurrent.size(0);
  for (i = 0; i < loop_ub; i++) {
    b[i] = muDoubleScalarIsInf(xCurrent[i]);
  }
  r.set_size(&ye_emlrtRTEI, &st, xCurrent.size(0));
  loop_ub = xCurrent.size(0);
  for (i = 0; i < loop_ub; i++) {
  ...
  if (!y) {
    emlrtErrorWithMessageIdR2018a(
        &st, &dd_emlrtRTEI, "optim_codegen:common:InfNaNComplexDetected",
        "optim_codegen:common:InfNaNComplexDetected", 3, 4, 2, "x0");
  }
  i = xCurrent.size(0);
  if (i < 0) {
    emlrtDynamicBoundsCheckR2012b(i, 0, MAX_int32_T, &sy_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  b_xCurrent.set_size(&oab_emlrtRTEI, sp, xCurrent.size(0));
