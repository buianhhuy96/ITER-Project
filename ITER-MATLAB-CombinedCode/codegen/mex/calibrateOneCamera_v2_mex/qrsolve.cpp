               ...
               ::coder::array<real_T, 1U> &Y)
{
  ...
  b_st.prev = &st;
  b_st.tls = st.tls;
  Y.set_size(&ck_emlrtRTEI, sp, A.size(1));
  loop_ub = A.size(1);
  for (i = 0; i < loop_ub; i++) {
    Y[i] = 0.0;
  }
  st.site = &dn_emlrtRSI;
  lapack::xunormqr(&st, A, B, tau);
  st.site = &en_emlrtRSI;
             ...
             const ::coder::array<real_T, 1U> &B, real_T Y[8])
{
  ...
  st.site = &dm_emlrtRSI;
  b_A.set_size(&ns_emlrtRTEI, &st, A.size(0), 8);
  maxmn = A.size(0) * 8;
  ...
  st.site = &fm_emlrtRSI;
  b_B.set_size(&bk_emlrtRTEI, &st, B.size(0));
  maxmn = B.size(0);
  for (minmn = 0; minmn < maxmn; minmn++) {
    b_B[minmn] = B[minmn];
  }
  ...
  if ((b_A.size(0) != 0) && (b_B.size(0) != 0)) {
    ...
    if (lapack::infocheck(&d_st, (int32_T)info_t)) {
      maxmn = b_B.size(0);
      b_B.set_size(&qs_emlrtRTEI, &c_st, maxmn);
      for (minmn = 0; minmn < maxmn; minmn++) {
        b_B[minmn] = rtNaN;
      }
    }
             ...
             int32_T *rankA)
{
  ...
  c_st.tls = b_st.tls;
  emlrtHeapReferenceStackEnterFcnR2012b((emlrtCTX)sp);
  b_A.set_size(&ak_emlrtRTEI, sp, A.size(0), A.size(1));
  ...
  *rankA = 0;
  ...
  b_B.set_size(&bk_emlrtRTEI, sp, B.size(0));
  minmn = B.size(0) - 1;
  for (k = 0; k <= minmn; k++) {
    b_B[k] = B[k];
  }
