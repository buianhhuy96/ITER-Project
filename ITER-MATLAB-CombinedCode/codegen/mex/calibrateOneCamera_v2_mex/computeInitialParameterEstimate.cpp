    ...
    int32_T *err)
{
  ...
  *err = 0;
  ...
  validIdx.set_size(&fr_emlrtRTEI, &st, imagePoints.size(2));
  loop_ub = imagePoints.size(2);
  for (i = 0; i < loop_ub; i++) {
    validIdx[i] = true;
  }
  H.set_size(&gr_emlrtRTEI, &st, 3, 3, imagePoints.size(2));
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    b_imagePoints.set_size(&hr_emlrtRTEI, &b_st, imagePoints.size(0),
                           imagePoints.size(1));
    ...
    if (1 > imagePoints.size(1)) {
      emlrtDynamicBoundsCheckR2012b(1, 1, imagePoints.size(1), &kt_emlrtBCI,
                                    &b_st);
    }
    loop_ub = imagePoints.size(0) - 1;
    validPointsIdx.set_size(&ye_emlrtRTEI, &b_st, imagePoints.size(0));
    ...
    r1.set_size(&ir_emlrtRTEI, &b_st, loop_ub);
    loop_ub = 0;
    for (c_i = 0; c_i <= b_loop_ub; c_i++) {
      if (validPointsIdx[c_i]) {
        r1[loop_ub] = c_i + 1;
    ...
    b_worldPoints.set_size(&jr_emlrtRTEI, &b_st, r1.size(0),
                           worldPoints.size(1));
    for (i1 = 0; i1 < b_loop_ub; i1++) {
      c_i = r1.size(0);
      for (i2 = 0; i2 < c_i; i2++) {
    ...
    c_imagePoints.set_size(&kr_emlrtRTEI, &b_st, r1.size(0),
                           imagePoints.size(1));
    for (i1 = 0; i1 <= loop_ub; i1++) {
      b_loop_ub = r1.size(0);
    ...
    if (b_i + 1 > H.size(2)) {
      emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, H.size(2), &lt_emlrtBCI, &b_st);
    }
    for (i1 = 0; i1 < 3; i1++) {
  ...
  r.set_size(&lr_emlrtRTEI, &st, loop_ub);
  for (b_i = 0; b_i <= b_loop_ub; b_i++) {
    r[b_i] = b_i + 1;
  }
  b_H.set_size(&mr_emlrtRTEI, &st, 3, 3, r.size(0));
  loop_ub = r.size(0);
  for (i = 0; i < loop_ub; i++) {
    for (i1 = 0; i1 < 3; i1++) {
      if (r[i] > H.size(2)) {
        emlrtDynamicBoundsCheckR2012b(r[i], 1, H.size(2), &vt_emlrtBCI, &st);
      }
  ...
  H.set_size(&nr_emlrtRTEI, &st, 3, 3, b_H.size(2));
  ...
  if (covrtLogIf(&emlrtCoverageInstance, 2, 0, 0, *err == 0)) {
    ...
    V.set_size(&pr_emlrtRTEI, &st, static_cast<int32_T>(fy), 6);
    fy = 2.0 * static_cast<real_T>(H.size(2));
    if (fy != static_cast<int32_T>(fy)) {
      emlrtIntegerCheckR2012b(fy, &pg_emlrtDCI, &st);
    }
    ...
    for (b_i = 0; b_i < i; b_i++) {
      ...
      if (b_i + 1 > H.size(2)) {
        emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, H.size(2), &ot_emlrtBCI, &st);
      }
      for (i1 = 0; i1 < 3; i1++) {
        d_H[3 * i1] = H[i1 + 9 * b_i];
    ...
    rotationVectors.set_size(&qr_emlrtRTEI, &st, 3, H.size(2));
    translationVectors.set_size(&qr_emlrtRTEI, &st, 3, H.size(2));
    ...
    // 3));
    b_rotationVectors.set_size(&rr_emlrtRTEI, sp, rotationVectors.size(1), 3);
    b_translationVectors.set_size(&sr_emlrtRTEI, sp, translationVectors.size(1),
                                  3);
    ...
    initialParams->WorldUnits.set_size(&tr_emlrtRTEI, sp, 1, 1);
    initialParams->WorldUnits[0] = t10_WorldUnits;
    ...
    initialParams->WorldPoints.set_size(&or_emlrtRTEI, sp, 0, 2);
    initialParams->WorldUnits.set_size(&or_emlrtRTEI, sp, 1, 2);
    ...
    initialParams->EstimateTangentialDistortion =
        t9_EstimateTangentialDistortion;
    initialParams->RotationVectors.set_size(&or_emlrtRTEI, sp, 0, 3);
    initialParams->TranslationVectors.set_size(&or_emlrtRTEI, sp, 0, 3);
  }
