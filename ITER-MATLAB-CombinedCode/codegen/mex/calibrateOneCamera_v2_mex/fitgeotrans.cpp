                                    ...
                                    projective2d *tform)
{
  ...
  images::geotrans::internal::normalizeControlPoints(&st, xy, b_xy,
                                                     normMatrix2);
  if (1 > b_xy.size(1)) {
    emlrtDynamicBoundsCheckR2012b(1, 1, b_xy.size(1), &wt_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  irank = b_xy.size(0);
  x.set_size(&ur_emlrtRTEI, sp, b_xy.size(0));
  for (i = 0; i < irank; i++) {
    x[i] = b_xy[i];
  }
  if (2 > b_xy.size(1)) {
    emlrtDynamicBoundsCheckR2012b(2, 1, b_xy.size(1), &xt_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  irank = b_xy.size(0);
  y.set_size(&vr_emlrtRTEI, sp, b_xy.size(0));
  for (i = 0; i < irank; i++) {
    y[i] = b_xy[i + b_xy.size(0)];
  }
  if (1 > b_uv.size(1)) {
    emlrtDynamicBoundsCheckR2012b(1, 1, b_uv.size(1), &yt_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  irank = b_uv.size(0);
  u.set_size(&wr_emlrtRTEI, sp, b_uv.size(0));
  for (i = 0; i < irank; i++) {
    u[i] = b_uv[i];
  }
  if (2 > b_uv.size(1)) {
    emlrtDynamicBoundsCheckR2012b(2, 1, b_uv.size(1), &au_emlrtBCI,
                                  (emlrtCTX)sp);
  }
  irank = b_uv.size(0);
  v.set_size(&xr_emlrtRTEI, sp, b_uv.size(0));
  for (i = 0; i < irank; i++) {
    v[i] = b_uv[i + b_uv.size(0)];
  }
  varargin_8.set_size(&yr_emlrtRTEI, sp, u.size(0));
  irank = u.size(0);
  for (i = 0; i < irank; i++) {
    varargin_8[i] = -u[i];
  }
  ...
  b_varargin_8.set_size(&as_emlrtRTEI, sp, v.size(0));
  irank = v.size(0);
  for (i = 0; i < irank; i++) {
    b_varargin_8[i] = -v[i];
  }
  ...
  st.site = &hx_emlrtRSI;
  varargin_7.set_size(&yr_emlrtRTEI, &st, varargin_8.size(0));
  ...
  st.site = &ix_emlrtRSI;
  b_varargin_7.set_size(&as_emlrtRTEI, &st, b_varargin_8.size(0));
  ...
  b_X.set_size(&bs_emlrtRTEI, sp, x.size(0) + b_xy.size(0), 8);
  irank = x.size(0);
  for (i = 0; i < irank; i++) {
    b_X[i] = x[i];
  }
  ...
  x.set_size(&cs_emlrtRTEI, sp, u.size(0) + v.size(0));
  irank = u.size(0);
  for (i = 0; i < irank; i++) {
    x[i] = u[i];
  }
