                ...
                ::coder::array<int32_T, 1U> &i)
{
  ...
  st.site = &kf_emlrtRSI;
  idx = 0;
  i.set_size(&nf_emlrtRTEI, &st, x.size(0));
                ...
                ::coder::array<int32_T, 2U> &i)
{
  ...
  st.site = &kf_emlrtRSI;
  idx = 0;
  i.set_size(&do_emlrtRTEI, &st, 1, x.size(1));
  b_st.site = &mf_emlrtRSI;
  ...
  if (x.size(1) == 1) {
    if (idx == 0) {
      i.set_size(&of_emlrtRTEI, &st, 1, 0);
    }
  } else {
    if (1 > idx) {
      nx = 0;
    ...
    b_i.set_size(&of_emlrtRTEI, &st, 1, nx);
    for (idx = 0; idx < nx; idx++) {
      b_i[idx] = i[idx];
    }
    i.set_size(&of_emlrtRTEI, &st, 1, b_i.size(1));
    nx = b_i.size(1);
    for (idx = 0; idx < nx; idx++) {
      i[idx] = b_i[idx];
    }
