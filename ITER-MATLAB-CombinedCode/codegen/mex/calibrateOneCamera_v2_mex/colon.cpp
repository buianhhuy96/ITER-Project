                     ...
                     ::coder::array<real_T, 2U> &y)
{
  ...
  if (ndbl > 2.147483647E+9) {
    emlrtErrorWithMessageIdR2018a(&st, &mb_emlrtRTEI, "Coder:MATLAB:pmaxsize",
                                  "Coder:MATLAB:pmaxsize", 0);
  }
  y.set_size(&il_emlrtRTEI, sp, 1, y.size(1));
  if (n < 0) {
    emlrtNonNegativeCheckR2012b(static_cast<real_T>(n), &rb_emlrtDCI,
                                (emlrtCTX)sp);
  }
  y.set_size(&il_emlrtRTEI, sp, y.size(0), n);
  if (n > 0) {
    y[0] = a;
    if (n > 1) {
      int32_T nm1d2;
