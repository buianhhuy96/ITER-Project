                                ...
                                const double b_yBounds[2])
{
  ...
  if (guard1) {
    if (std::floor(c_a) == c_a) {
      int b_loop_ub;
      b_loop_ub = static_cast<int>(std::floor(b_b - c_a));
    ...
    for (int k{0}; k < 2; k++) {
      ...
      for (int b_k{0}; b_k <= i7; b_k++) {
        centeredPoints[b_k + (centeredPoints.size(0) * k)] =
            b_points[(acoef * b_k) + (b_points.size(0) * k)] - f_b[k];
      }
    }
  }
  ...
  for (int e_k{0}; e_k < p_N; e_k++) {
    z1[e_k] = rt_powd_snf(xNorm[e_k], 2.0);
  }
