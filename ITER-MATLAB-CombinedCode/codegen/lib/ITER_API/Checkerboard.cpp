...
static float rt_hypotf_snf(float u0, float b_u1);
    ...
    ::coder::array<double, 2U> &newIndices) const
{
  ...
  if (std::abs(combineVectorElements(b_x) /
      ...
      std::abs(combineVectorElements(d_x) /
               (static_cast<double>(d_x.size(0))))) {
    coordsToUse[0] = 1.0;
    coordsToUse[1] = 2.0;
  } else {
    coordsToUse[0] = 2.0;
    ...
    if (c_ii.size(1) >= 2) {
      ...
      for (int k{0}; k < i8; k++) {
        ...
        b_this[k] = BoardCoords[(j + (BoardCoords.size(0) * (c_ii[k] - 1))) +
                                ((BoardCoords.size(0) * BoardCoords.size(1)) *
                                 (i7 - 1))];
        d_this[k] = BoardCoords[(j + (BoardCoords.size(0) * (c_ii[k] - 1))) +
                                ((BoardCoords.size(0) * BoardCoords.size(1)) *
    ...
    ::coder::array<double, 2U> &newIndices) const
{
  ...
  d_loop_ub = r4.size(0);
  for (int i3{0}; i3 < d_loop_ub; i3++) {
    d_x[i3] = BoardCoords[((r4[i3] + (BoardCoords.size(0) * (b_idx_tmp - 1))) +
                           (BoardCoords.size(0) * BoardCoords.size(1))) -
                          1] -
...
float b_Checkerboard::computeNewEnergyHorizontal(float oldEnergy) const
{
  ...
  if (y) {
    ...
    d_this.set_size(r2.size(0), 1, g_loop_ub);
    for (int i7{0}; i7 < g_loop_ub; i7++) {
      int h_loop_ub;
      h_loop_ub = r2.size(0);
      for (int i8{0}; i8 < h_loop_ub; i8++) {
        d_this[i8 + (d_this.size(0) * i7)] =
            (BoardCoords[(r2[i8] +
    ...
    d_this.set_size(r5.size(0), 1, i_loop_ub);
    for (int i9{0}; i9 < i_loop_ub; i9++) {
      int j_loop_ub;
      j_loop_ub = r5.size(0);
      for (int i10{0}; i10 < j_loop_ub; i10++) {
        d_this[i10 + (d_this.size(0) * i9)] =
            BoardCoords[(r5[i10] +
    ...
    if (f_num.size(1) > 1) {
      ...
      d_x.set_size(csz_idx_0);
      for (int b_k{0}; b_k < csz_idx_0; b_k++) {
        d_x[b_k] = rt_hypotd_snf(g_num[b_k], h_num[b_k]);
      }
      ...
      q_loop_ub = d_x.size(0);
      for (int i15{0}; i15 < q_loop_ub; i15++) {
        d_x[i15] = d_x[i15] / b_y[i15];
      }
      newEnergy = std::fmax(
          oldEnergy, static_cast<float>(::coder::internal::b_maximum(d_x)));
    } else {
      newEnergy =
...
float b_Checkerboard::computeNewEnergyVertical(float oldEnergy) const
{
  ...
  if (y) {
    ...
    if (f_num.size(1) > 1) {
      ...
      bb_loop_ub = d_x.size(0);
      for (int i25{0}; i25 < bb_loop_ub; i25++) {
        d_x[i25] = d_x[i25] / b_y[i25];
      }
      newEnergy = std::fmax(
          oldEnergy, static_cast<float>(::coder::internal::b_maximum(d_x)));
    } else {
    ...
    ::coder::array<double, 3U> &newBoardCoords) const
{
  ...
  unnamed_idx_0 = r3.size(1);
  d_this = BoardCoords.size(1);
  h_loop_ub = r.size(0);
  for (int i12{0}; i12 < h_loop_ub; i12++) {
    for (int i13{0}; i13 < unnamed_idx_0; i13++) {
      newBoardCoords[((r3[i13] + (newBoardCoords.size(0) * d_this)) +
                      ((newBoardCoords.size(0) * newBoardCoords.size(1)) *
                       r[i12])) -
    ...
    ::coder::array<double, 2U> &indices) const
{
  ...
  do_vectors(y, d_this, remIdx, ia, b_ib);
  if (remIdx.size(1) != 0) {
    int b_nx;
    int c_loop_ub;
    int d_loop_ub;
  ...
  if (h_n > 1) {
    ...
    if (guard1) {
      if (std::floor(c_a) == c_a) {
        int i_loop_ub;
        i_loop_ub = static_cast<int>(std::floor(b_b - c_a));
    ...
    for (int h_i{0}; h_i < i24; h_i++) {
      ...
      d_x = std::sqrt(d_x);
      dist[h_i] = static_cast<double>(d_x);
    }
    ::coder::internal::b_minimum(dist, &ex, &iindx);
                             ...
                             const float e_v[2]) const
{
  ...
  for (int k{0}; k < inner; k++) {
    for (int c_i{0}; c_i <= mc; c_i++) {
      y[c_i] =
          y[c_i] + (pointVectors[c_i + (pointVectors.size(0) * k)] * e_v[k]);
    }
  }
      ...
      for (int k{0}; k < i12; k++) {
        ...
        b_this[k] = BoardCoords[((c_ii[k] + (BoardCoords.size(0) * j)) +
                                 ((BoardCoords.size(0) * BoardCoords.size(1)) *
                                  (i11 - 1))) -
                                1];
        d_this[k] = BoardCoords[((c_ii[k] + (BoardCoords.size(0) * j)) +
                                 ((BoardCoords.size(0) * BoardCoords.size(1)) *
                                  ((static_cast<int>(coordsToUse[1])) - 1))) -
                                1];
      }
...
void b_Checkerboard::expandBoardDirectionally(double direction)
{
    ...
    if (IsDistortionHigh) {
      ...
      if (!y) {
        ...
        for (int i48{0}; i48 < ac_loop_ub; i48++) {
          ...
          for (int i58{0}; i58 < oc_loop_ub; i58++) {
            ...
            d4 = (d4 + d4) - b_p1[i58 + (b_p1.size(0) * i48)];
      ...
      for (int i20{0}; i20 < w_loop_ub; i20++) {
        ...
        for (int i28{0}; i28 < pb_loop_ub; i28++) {
          ...
          d = (d + d) - b_p1[i28 + (b_p1.size(0) * i20)];
  ...
  case 2: {
    ...
    if (IsDistortionHigh) {
      ...
      if (!c_y) {
        ...
        for (int i66{0}; i66 < uc_loop_ub; i66++) {
          ...
          for (int i70{0}; i70 < xc_loop_ub; i70++) {
            ...
            d6 = (d6 + d6) - b_p1[i70 + (b_p1.size(0) * i66)];
      ...
      for (int i44{0}; i44 < vb_loop_ub; i44++) {
        ...
        for (int i53{0}; i53 < jc_loop_ub; i53++) {
          ...
          d2 = (d2 + d2) - b_p1[i53 + (b_p1.size(0) * i44)];
    ...
    partialTrueCount = 0;
    for (int k_i{0}; k_i <= e_end; k_i++) {
      if (newIndices[k_i] > 0.0) {
        r3[partialTrueCount] = k_i + 1;
        partialTrueCount++;
      }
    }
    ...
    b_trueCount = 0;
    for (int m_i{0}; m_i <= f_end; m_i++) {
      if (newIndices[m_i] > 0.0) {
        b_trueCount++;
      }
    }
  ...
  case 3: {
    ...
    if (IsDistortionHigh) {
      ...
      if (!b_y) {
        ...
        d_this.set_size(r_loop_ub, 1, t_loop_ub);
        for (int i12{0}; i12 < t_loop_ub; i12++) {
          for (int i19{0}; i19 < r_loop_ub; i19++) {
            d_this[i19 + (d_this.size(0) * i12)] =
        ...
        d_this.set_size(gb_loop_ub, 1, ob_loop_ub);
        for (int i34{0}; i34 < ob_loop_ub; i34++) {
          for (int i42{0}; i42 < gb_loop_ub; i42++) {
            d_this[i42 + (d_this.size(0) * i34)] =
        ...
        for (int i50{0}; i50 < dc_loop_ub; i50++) {
          ...
          for (int i60{0}; i60 < qc_loop_ub; i60++) {
            ...
            d5 = (d5 + d5) - b_p1[i60 + (b_p1.size(0) * i50)];
      ...
      e_loop_ub = BoardCoords.size(2);
      d_this.set_size(b_loop_ub, 1, e_loop_ub);
      for (int i2{0}; i2 < e_loop_ub; i2++) {
        for (int i5{0}; i5 < b_loop_ub; i5++) {
          d_this[i5 + (d_this.size(0) * i2)] =
      ...
      k_loop_ub = BoardCoords.size(0);
      q_loop_ub = BoardCoords.size(2);
      d_this.set_size(k_loop_ub, 1, q_loop_ub);
      for (int i9{0}; i9 < q_loop_ub; i9++) {
        for (int i15{0}; i15 < k_loop_ub; i15++) {
          d_this[i15 + (d_this.size(0) * i9)] =
              BoardCoords[i15 +
                          ((BoardCoords.size(0) * BoardCoords.size(1)) * i9)];
        }
      }
      ...
      for (int i21{0}; i21 < ab_loop_ub; i21++) {
        ...
        for (int i32{0}; i32 < rb_loop_ub; i32++) {
          ...
          b_d1 = (b_d1 + b_d1) - b_p1[i32 + (b_p1.size(0) * i21)];
  ...
  case 4: {
    ...
    if (IsDistortionHigh) {
      ...
      if (!d_y) {
        ...
        d_this.set_size(cb_loop_ub, 1, ib_loop_ub);
        for (int i27{0}; i27 < ib_loop_ub; i27++) {
          for (int i39{0}; i39 < cb_loop_ub; i39++) {
            d_this[i39 + (d_this.size(0) * i27)] =
        ...
        d_this.set_size(fc_loop_ub, 1, hc_loop_ub);
        for (int i57{0}; i57 < hc_loop_ub; i57++) {
          for (int i64{0}; i64 < fc_loop_ub; i64++) {
            d_this[i64 + (d_this.size(0) * i57)] =
        ...
        for (int i68{0}; i68 < wc_loop_ub; i68++) {
          ...
          for (int i71{0}; i71 < yc_loop_ub; i71++) {
            ...
            d7 = (d7 + d7) - b_p1[i71 + (b_p1.size(0) * i68)];
      ...
      d_this.set_size(h_loop_ub, 1, m_loop_ub);
      for (int i7{0}; i7 < m_loop_ub; i7++) {
        for (int i13{0}; i13 < h_loop_ub; i13++) {
          d_this[i13 + (d_this.size(0) * i7)] =
      ...
      d_this.set_size(bb_loop_ub, 1, hb_loop_ub);
      for (int i26{0}; i26 < hb_loop_ub; i26++) {
        for (int i38{0}; i38 < bb_loop_ub; i38++) {
          d_this[i38 + (d_this.size(0) * i26)] =
      ...
      for (int i46{0}; i46 < xb_loop_ub; i46++) {
        ...
        for (int i56{0}; i56 < mc_loop_ub; i56++) {
          ...
          d3 = (d3 + d3) - b_p1[i56 + (b_p1.size(0) * i46)];
                                ...
                                const float b_v1[2], const float v2[2])
{
  ...
  if (c_y) {
    ...
    c_r.set_size(1, g_loop_ub);
    for (int i14{0}; i14 < g_loop_ub; i14++) {
      c_r[i14] = Points[(d_this + (Points.size(0) * i14)) - 1];
    }
    h_loop_ub = BoardCoords.size(2);
    ...
    for (int i16{0}; i16 < i_loop_ub; i16++) {
      BoardCoords[((BoardCoords.size(0) * 2) +
                  ...
                  1] = static_cast<double>(c_r[i16]);
    }
    e_this = static_cast<int>(BoardIdx[1]);
    j_loop_ub = Points.size(1);
    w_l.set_size(1, j_loop_ub);
    for (int i17{0}; i17 < j_loop_ub; i17++) {
      w_l[i17] = Points[(e_this + (Points.size(0) * i17)) - 1];
    }
    k_loop_ub = BoardCoords.size(2);
    ...
    for (int i25{0}; i25 < t_loop_ub; i25++) {
      BoardCoords[BoardCoords.size(0) +
          ...
          static_cast<double>(c_u[i25]);
    }
    up.set_size(1, c_u.size(1));
    u_loop_ub = c_u.size(1);
    ...
    w_loop_ub = c_r.size(1);
    for (int i28{0}; i28 < w_loop_ub; i28++) {
      right[i28] = c_r[i28] - center[i28];
    }
    left.set_size(1, w_l.size(1));
    x_loop_ub = w_l.size(1);
    for (int i29{0}; i29 < x_loop_ub; i29++) {
      left[i29] = w_l[i29] - center[i29];
    }
    ...
    if (isValid) {
      ...
      for (int i36{0}; i36 < eb_loop_ub; i36++) {
        r1[i36] =
            static_cast<double>(Points[(h_this + (Points.size(0) * i36)) - 1]);
      }
      fb_loop_ub = r.size(0);
      ...
      for (int i39{0}; i39 < hb_loop_ub; i39++) {
        r1[i39] =
            static_cast<double>(Points[(i_this + (Points.size(0) * i39)) - 1]);
      }
      ib_loop_ub = r.size(0);
      ...
      for (int i42{0}; i42 < kb_loop_ub; i42++) {
        r1[i42] =
            static_cast<double>(Points[(j_this + (Points.size(0) * i42)) - 1]);
      }
      mb_loop_ub = r.size(0);
      ...
      for (int i45{0}; i45 < pb_loop_ub; i45++) {
        r1[i45] =
            static_cast<double>(Points[(k_this + (Points.size(0) * i45)) - 1]);
      }
      qb_loop_ub = r.size(0);
      ...
      isValid = ((static_cast<double>(Energy)) < (static_cast<double>(m_this)));
    }
  }
}
