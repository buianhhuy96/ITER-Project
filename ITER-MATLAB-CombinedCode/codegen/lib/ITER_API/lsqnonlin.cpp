  ...
  while ((!exitg1) && (b_exitflag == -5)) {
    ...
    if ((resnormNew < b_resnorm) && evalOK) {
      ...
      for (int h_i{0}; h_i < h_n; h_i++) {
        int b_ix0;
        int d_iy0;
        b_ix0 = (m_temp + h_n) * h_i;
        d_iy0 = m_temp * h_i;
        for (int i_k{0}; i_k <= m; i_k++) {
          jacob[d_iy0 + i_k] = augJacobian[b_ix0 + i_k];
        }
      }
      if (b_evalOK) {
        int h_loop_ub;
    ...
    if (guard1) {
      ...
      for (int k_i{0}; k_i < h_n; k_i++) {
        int c_iy0;
        c_iy0 = ((m_temp + h_n) * (k_i + 1)) - h_n;
        for (int g_k{0}; g_k < h_n; g_k++) {
          augJacobian[c_iy0 + g_k] = 0.0;
        }
        augJacobian[(m_temp + k_i) + (augJacobian.size(0) * k_i)] = sqrtGamma;
      }
