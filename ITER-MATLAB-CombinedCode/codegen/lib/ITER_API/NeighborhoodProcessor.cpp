  ...
  if (ProcessBorder) {
    ...
    for (int b_secondInd{0}; b_secondInd < secondDimExtents_idx_1;
      ...
      for (int firstInd{0}; firstInd < firstDimExtents_idx_1_tmp; firstInd++) {
        ...
        if (Padding == 2.0) {
          ...
          for (int cb_i{0}; cb_i < 9; cb_i++) {
            if (!isInside[cb_i]) {
              e_r[b_partialTrueCount] = static_cast<signed char>(cb_i + 1);
              b_partialTrueCount++;
            }
          }
    ...
    for (int c_secondInd{secondDimExtents_idx_0};
      ...
      for (int c_firstInd{0}; c_firstInd < firstDimExtents_idx_1_tmp;
        ...
        if (ImageSize[0] == 0) {
          ...
          c_r = c_pind - (ImageSize[0] * div_s32(c_pind, ImageSize[0]));
        }
        g_x = c_pind - c_r;
        if (ImageSize[0] == 0) {
          if (g_x == 0) {
          ...
          b_z = div_s32(h_x, b_y);
          h_x -= b_z * b_y;
          if ((h_x > 0) && (h_x >= (asr_s32(b_y, 1U) + (b_y & 1)))) {
            b_z++;
          }
          if ((g_x < 0) != (ImageSize[0] < 0)) {
            b_z = -b_z;
          }
        }
        pixelSub[1] = b_z + 1;
        ...
        case 1: {
          ...
          for (int db_i{0}; db_i < 9; db_i++) {
            if (isInside[db_i]) {
              d_imnhInds[c_partialTrueCount] = b_imnhInds_[db_i];
              c_partialTrueCount++;
            }
          }
        } break;
        ...
        if (Padding == 2.0) {
          ...
          for (int hb_i{0}; hb_i < 9; hb_i++) {
            if (!isInside[hb_i]) {
              r1[d_partialTrueCount] = static_cast<signed char>(hb_i + 1);
              d_partialTrueCount++;
            }
          }
          i_loop_ub = r1.size(0);
    ...
    for (int d_secondInd{1}; d_secondInd <= i5; d_secondInd++) {
      for (int d_firstInd{0}; d_firstInd < firstDimExtents_idx_1;
        ...
        if (ImageSize[0] == 0) {
          ...
          c_z = div_s32(m_x, c_y);
          m_x -= c_z * c_y;
          if ((m_x > 0) && (m_x >= (asr_s32(c_y, 1U) + (c_y & 1)))) {
            c_z++;
          }
        ...
        case 1: {
          ...
          for (int jb_i{0}; jb_i < 9; jb_i++) {
            if (isInside[jb_i]) {
              e_imnhInds[e_partialTrueCount] = b_imnhInds_[jb_i];
              e_partialTrueCount++;
            }
          }
        } break;
        ...
        if (Padding == 2.0) {
          ...
          for (int ob_i{0}; ob_i < 9; ob_i++) {
            if (!isInside[ob_i]) {
              r2[f_partialTrueCount] = static_cast<signed char>(ob_i + 1);
              f_partialTrueCount++;
            }
          }
          m_loop_ub = r2.size(0);
    ...
    for (int e_secondInd{0}; e_secondInd < i5; e_secondInd++) {
      for (int e_firstInd{firstDimExtents_idx_0};
        ...
        for (int ab_i{0}; ab_i < 9; ab_i++) {
          b_imnhInds_[ab_i] = (ImageNeighborLinearOffsets[ab_i] + e_pind) + 1;
        }
        if (ImageSize[0] == 0) {
          f_r = 0;
        } else {
        ...
        if (ImageSize[0] == 0) {
          ...
          e_z = div_s32(p_x, d_y);
          p_x -= e_z * d_y;
          if ((p_x > 0) && (p_x >= (asr_s32(d_y, 1U) + (d_y & 1)))) {
            e_z++;
          }
        ...
        for (int g_k{0}; g_k < 2; g_k++) {
          for (int i_k{0}; i_k < 9; i_k++) {
            imnhSubs[g_k][i_k] =
                NeighborSubscriptOffsets[g_k][i_k] + pixelSub[g_k];
          }
        }
        ...
        case 1: {
          ...
          for (int pb_i{0}; pb_i < 9; pb_i++) {
            if (isInside[pb_i]) {
              f_imnhInds[g_partialTrueCount] = b_imnhInds_[pb_i];
              g_partialTrueCount++;
            }
          }
        } break;
        ...
        if (Padding == 2.0) {
          ...
          for (int rb_i{0}; rb_i < 9; rb_i++) {
            if (!isInside[rb_i]) {
              r3[h_partialTrueCount] = static_cast<signed char>(rb_i + 1);
              h_partialTrueCount++;
            }
          }
          o_loop_ub = r3.size(0);
