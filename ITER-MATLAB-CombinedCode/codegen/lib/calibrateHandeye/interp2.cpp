...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
    ix, b_iy, zx1y1, zx2y1, zx1y2, zx2y2, f, qx1, qx2, f1, ry, rx)

  for (int k = 0; k <= ub_loop; k++) {
    if (Xq[k] >= e_X[0]) {
      if (Xq[k] <= e_X[e_X.size(1) - 1]) {
        if (Yq[k] >= h_Y[0]) {
          if (Yq[k] <= h_Y[h_Y.size(1) - 1]) {
            ix = ::coder::internal::b_bsearch(e_X, Xq[k]);
            b_iy = ::coder::internal::b_bsearch(h_Y, Yq[k]);
            zx1y1 = V[(b_iy + (V.size(0) * (ix - 1))) - 1];
            zx2y1 = V[(b_iy + (V.size(0) * ix)) - 1];
            zx1y2 = V[b_iy + (V.size(0) * (ix - 1))];
            zx2y2 = V[b_iy + (V.size(0) * ix)];
            f = e_X[ix - 1];
            if (Xq[k] == f) {
              qx1 = zx1y1;
              qx2 = zx1y2;
            ...
            if (Yq[k] == f1) {
              ...
              Vq[k] = qx1;
            } else if (Yq[k] == h_Y[b_iy]) {
              Vq[k] = qx2;
            } else {
              ry = (Yq[k] - f1) / (h_Y[b_iy] - f1);
