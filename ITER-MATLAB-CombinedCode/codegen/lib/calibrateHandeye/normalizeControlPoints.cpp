                            ...
                            double normMatrixInv[3][3])
{
  ...
  if ((pts.size(0) != 0) && (csz_idx_1 != 0)) {
    ...
    for (int d_k{0}; d_k <= i4; d_k++) {
      ...
      i7 = ptsNorm.size(0) - 1;
      for (int f_k{0}; f_k <= i7; f_k++) {
        ptsNorm[f_k + (ptsNorm.size(0) * d_k)] =
            pts[(b_acoef * f_k) + (pts.size(0) * varargin_2)] -
            cent[varargin_3];
      }
    }
  }
  ...
  c_u1 = c_ptsNorm.size(0);
  if (c_u0 <= c_u1) {
    csz_idx_0 = c_u0;
  } else {
    csz_idx_0 = c_u1;
  }
  b_a.set_size(csz_idx_0);
  for (int g_k{0}; g_k < csz_idx_0; g_k++) {
    b_a[g_k] = rt_hypotd_snf(b_ptsNorm[g_k], c_ptsNorm[g_k]);
  }
  b_x.set_size(b_a.size(0));
  f_N = b_a.size(0);
  for (int h_k{0}; h_k < f_N; h_k++) {
    b_x[h_k] = rt_powd_snf(b_a[h_k], 2.0);
  }
