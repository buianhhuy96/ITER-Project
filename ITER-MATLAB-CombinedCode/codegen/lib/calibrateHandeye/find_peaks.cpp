                ...
                ::coder::array<float, 2U> &loc)
{
  ...
  if ((static_cast<double>(maxMetric)) <= 4.94065645841247E-324) {
    ...
    v1.set_size(c_idx.size(0));
    p_loop_ub = c_idx.size(0);
    for (int i19{0}; i19 < p_loop_ub; i19++) {
      v1[i19] = (static_cast<int>(c_idx[i19])) - 1;
    }
    vk.set_size(v1.size(0));
    q_loop_ub = v1.size(0);
    for (int i20{0}; i20 < q_loop_ub; i20++) {
      vk[i20] = div_s32(v1[i20], static_cast<int>(siz[0]));
    }
    r_loop_ub = v1.size(0);
    for (int i21{0}; i21 < r_loop_ub; i21++) {
      v1[i21] = v1[i21] - (vk[i21] * (static_cast<int>(siz[0])));
    }
    s_loop_ub = v1.size(0);
    for (int i22{0}; i22 < s_loop_ub; i22++) {
      loc[i22 + loc.size(0)] =
          static_cast<float>(static_cast<int>(v1[i22] + 1));
    }
    t_loop_ub = vk.size(0);
    for (int i23{0}; i23 < t_loop_ub; i23++) {
      loc[i23] = static_cast<float>(static_cast<int>(vk[i23] + 1));
    }
  }
}
