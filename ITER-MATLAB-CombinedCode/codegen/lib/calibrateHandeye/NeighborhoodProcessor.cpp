...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
  ...
  if (ProcessBorder) {
    ...
    if (u0 <= u1) {
      ...
      firstDimExtents_idx_1_tmp = u1;
    }
    b_u1 = ImageSize[1];
    if (secondDimExtents_idx_1 > b_u1) {
      secondDimExtents_idx_1 = b_u1;
    }
      ...
      for (int firstInd{0}; firstInd < firstDimExtents_idx_1_tmp; firstInd++) {
        ...
        case 1: {
          ...
          for (int o_i{0}; o_i < 9; o_i++) {
            if (isInside[o_i]) {
              c_imnhInds[partialTrueCount] = b_imnhInds_[o_i];
              partialTrueCount++;
            }
          }
        } break;
    ...
    for (int c_secondInd{secondDimExtents_idx_0};
      ...
      for (int c_firstInd{0}; c_firstInd < firstDimExtents_idx_1_tmp;
        ...
        if (ImageSize[0] == 0) {
          ...
          b_z = div_s32(g_x, b_y);
          g_x -= b_z * b_y;
          if ((g_x > 0) && (g_x >= (asr_s32(b_y, 1U) + (b_y & 1)))) {
            b_z++;
          }
    ...
    d_u1 = ImageSize[0];
    if (firstDimExtents_idx_1 > d_u1) {
      firstDimExtents_idx_1 = d_u1;
    }
    b_u0 = ImageSize[1];
    e_u1 = ImageSize[1];
    if (b_u0 <= e_u1) {
    ...
    for (int d_secondInd{1}; d_secondInd <= i5; d_secondInd++) {
      for (int d_firstInd{0}; d_firstInd < firstDimExtents_idx_1;
        ...
        if (ImageSize[0] == 0) {
          ...
          c_z = div_s32(k_x, c_y);
          k_x -= c_z * c_y;
          if ((k_x > 0) && (k_x >= (asr_s32(c_y, 1U) + (c_y & 1)))) {
            c_z++;
          }
        ...
        for (int f_k{0}; f_k < 2; f_k++) {
          for (int g_k{0}; g_k < 9; g_k++) {
            imnhSubs[f_k][g_k] =
                NeighborSubscriptOffsets[f_k][g_k] + pixelSub[f_k];
          }
        }
    ...
    for (int e_secondInd{0}; e_secondInd < i5; e_secondInd++) {
      for (int e_firstInd{firstDimExtents_idx_0};
        ...
        if (ImageSize[0] == 0) {
          ...
          e_z = div_s32(o_x, d_y);
          o_x -= e_z * d_y;
          if ((o_x > 0) && (o_x >= (asr_s32(d_y, 1U) + (d_y & 1)))) {
            e_z++;
          }
          if ((m_x < 0) != (ImageSize[0] < 0)) {
            e_z = -e_z;
          }
        }
        pixelSub[1] = e_z + 1;
