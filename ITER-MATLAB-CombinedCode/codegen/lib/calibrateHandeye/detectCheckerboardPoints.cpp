...
//
// Arguments    : const ::coder::array<unsigned char, 2U> &k_I
//                ::coder::array<double, 2U> &c_imagePoints
//                double boardSize[2]
// Return Type  : void
//
namespace coder {
void detectCheckerboardPoints(const ::coder::array<unsigned char, 2U> &k_I,
                              ::coder::array<double, 2U> &c_imagePoints,
                              double boardSize[2])
{
  ...
  bool guard1{false};
  bool guard2{false};
  m_I.set_size(k_I.size(0), k_I.size(1));
  loop_ub = k_I.size(1);
  for (int b_i{0}; b_i < loop_ub; b_i++) {
    int b_loop_ub;
    b_loop_ub = k_I.size(0);
    for (int i1{0}; i1 < b_loop_ub; i1++) {
      m_I[i1 + (m_I.size(0) * b_i)] =
          (static_cast<float>(k_I[i1 + (k_I.size(0) * b_i)])) / 255.0F;
    }
  }
  finalSize[0] = static_cast<double>(m_I.size(0));
  pad[0] = 7.0;
  finalSize[1] = static_cast<double>(m_I.size(1));
  pad[1] = 7.0;
  if ((m_I.size(0) == 0) || (m_I.size(1) == 0)) {
    int c_loop_ub;
    Ig.set_size(m_I.size(0), m_I.size(1));
    c_loop_ub = m_I.size(1);
    for (int i2{0}; i2 < c_loop_ub; i2++) {
      int e_loop_ub;
      e_loop_ub = m_I.size(0);
      for (int i4{0}; i4 < e_loop_ub; i4++) {
        Ig[i4 + (Ig.size(0) * i2)] = m_I[i4 + (m_I.size(0) * i2)];
      }
    }
  } else {
    int d_loop_ub;
    int g_loop_ub;
    padImage_outSize(m_I, pad, b_a);
    c_a.set_size(b_a.size(0), b_a.size(1));
    d_loop_ub = b_a.size(1);
    for (int i3{0}; i3 < d_loop_ub; i3++) {
      int f_loop_ub;
      f_loop_ub = b_a.size(0);
      for (int i5{0}; i5 < f_loop_ub; i5++) {
        c_a[i5 + (c_a.size(0) * i3)] =
            static_cast<double>(b_a[i5 + (b_a.size(0) * i3)]);
      }
    }
  ...
  if ((Ig.size(0) == 0) || (Ig.size(1) == 0)) {
    ...
    for (int i7{0}; i7 < h_loop_ub; i7++) {
      ...
      for (int i10{0}; i10 < k_loop_ub; i10++) {
        Iy[i10 + (Iy.size(0) * i7)] = Ig[i10 + (Ig.size(0) * i7)];
      }
    }
  } else {
    int j_loop_ub;
    ...
    for (int i9{0}; i9 < j_loop_ub; i9++) {
      int m_loop_ub;
      m_loop_ub = b_a.size(0);
      for (int i11{0}; i11 < m_loop_ub; i11++) {
        c_a[i11 + (c_a.size(0) * i9)] =
            static_cast<double>(b_a[i11 + (b_a.size(0) * i9)]);
      }
    }
  ...
  if ((Ig.size(0) == 0) || (Ig.size(1) == 0)) {
    ...
    padImage_outSize(Ig, pad, b_a);
    c_a.set_size(b_a.size(0), b_a.size(1));
    r_loop_ub = b_a.size(1);
    for (int i15{0}; i15 < r_loop_ub; i15++) {
      int t_loop_ub;
      t_loop_ub = b_a.size(0);
      for (int i17{0}; i17 < t_loop_ub; i17++) {
        c_a[i17 + (c_a.size(0) * i15)] =
            static_cast<double>(b_a[i17 + (b_a.size(0) * i15)]);
      }
    }
    internal::conv2AXPYValidCMP(c_a, b);
  ...
  for (int i22{0}; i22 < y_loop_ub; i22++) {
    ...
    for (int i23{0}; i23 < ab_loop_ub; i23++) {
      b_I_45[i23 + (b_I_45.size(0) * i22)] =
          r[i23 + (r.size(0) * i22)] +
          (Iy[i23 + (Iy.size(0) * i22)] * 0.707106769F);
    }
  }
  ...
  if ((b_I_45.size(0) != 0) && (b_I_45.size(1) != 0)) {
    ...
    for (int i34{0}; i34 < mb_loop_ub; i34++) {
      ...
      for (int i37{0}; i37 < pb_loop_ub; i37++) {
        c_b[i37 + (c_b.size(0) * i34)] = r1[i37 + (r1.size(0) * i34)];
      }
    }
    i36 = b_I_45.size(1);
    for (int k{0}; k < i36; k++) {
    ...
    rb_loop_ub = c_b.size(1);
    for (int i40{0}; i40 < rb_loop_ub; i40++) {
      int sb_loop_ub;
      sb_loop_ub = c_b.size(0);
      for (int i43{0}; i43 < sb_loop_ub; i43++) {
        r1[i43 + (r1.size(0) * i40)] = c_b[i43 + (c_b.size(0) * i40)];
      }
    }
  }
  ...
  for (int i35{0}; i35 < ob_loop_ub; i35++) {
    ...
    for (int i38{0}; i38 < qb_loop_ub; i38++) {
      b_x[i38 + (b_x.size(0) * i35)] =
          r[i38 + (r.size(0) * i35)] +
          (Iy[i38 + (Iy.size(0) * i35)] * -0.707106769F);
    }
  }
  ...
  ub_loop_ub = c_b.size(1);
  for (int i45{0}; i45 < ub_loop_ub; i45++) {
    int xb_loop_ub;
    xb_loop_ub = c_b.size(0);
    for (int i50{0}; i50 < xb_loop_ub; i50++) {
      r2[i50 + (r2.size(0) * i45)] = 4.0F * c_b[i50 + (c_b.size(0) * i45)];
    }
  }
  ...
  if ((Ig.size(0) != 0) && (Ig.size(1) != 0)) {
    ...
    for (int i56{0}; i56 < b_loop_ub_tmp; i56++) {
      ...
      for (int i57{0}; i57 < ec_loop_ub; i57++) {
        c_b[i57 + (c_b.size(0) * i56)] = r[i57 + (r.size(0) * i56)];
      }
    }
    ...
    for (int i61{0}; i61 < fc_loop_ub; i61++) {
      ...
      for (int i66{0}; i66 < ic_loop_ub; i66++) {
        r[i66 + (r.size(0) * i61)] = c_b[i66 + (c_b.size(0) * i61)];
      }
    }
  }
  ...
  if ((Iy.size(0) != 0) && (Iy.size(1) != 0)) {
    ...
    for (int i58{0}; i58 < c_loop_ub_tmp; i58++) {
      ...
      for (int i62{0}; i62 < gc_loop_ub; i62++) {
        c_b[i62 + (c_b.size(0) * i58)] = r1[i62 + (r1.size(0) * i58)];
      }
    }
    ...
    r1.set_size(c_b.size(0), c_b.size(1));
    jc_loop_ub = c_b.size(1);
    for (int i67{0}; i67 < jc_loop_ub; i67++) {
      int mc_loop_ub;
      mc_loop_ub = c_b.size(0);
      for (int i69{0}; i69 < mc_loop_ub; i69++) {
        r1[i69 + (r1.size(0) * i67)] = c_b[i69 + (c_b.size(0) * i67)];
      }
    }
  }
  ...
  for (int i71{0}; i71 < pc_loop_ub; i71++) {
    ...
    for (int i72{0}; i72 < qc_loop_ub; i72++) {
      c_b[i72 + (c_b.size(0) * i71)] = Ix2[i72 + (Ix2.size(0) * i71)];
    }
  }
  f_N = Ig.size(1);
  ...
  rc_loop_ub = c_b.size(1);
  for (int i73{0}; i73 < rc_loop_ub; i73++) {
    int sc_loop_ub;
    sc_loop_ub = c_b.size(0);
    for (int i74{0}; i74 < sc_loop_ub; i74++) {
      Ix2[i74 + (Ix2.size(0) * i73)] = c_b[i74 + (c_b.size(0) * i73)];
    }
  }
  ...
  if ((Iy2.size(0) != 0) && (Iy2.size(1) != 0)) {
    ...
    c_a.set_size(b_a.size(0), b_a.size(1));
    wc_loop_ub = b_a.size(1);
    for (int i78{0}; i78 < wc_loop_ub; i78++) {
      int ad_loop_ub;
      ad_loop_ub = b_a.size(0);
      for (int i81{0}; i81 < ad_loop_ub; i81++) {
        c_a[i81 + (c_a.size(0) * i78)] =
            static_cast<double>(b_a[i81 + (b_a.size(0) * i78)]);
      }
    }
    internal::c_conv2AXPYValidCMP(c_a, b);
  ...
  for (int i77{0}; i77 < vc_loop_ub; i77++) {
    ...
    for (int i80{0}; i80 < yc_loop_ub; i80++) {
      IxIy[i80 + (IxIy.size(0) * i77)] =
          Ig[i80 + (Ig.size(0) * i77)] * Iy[i80 + (Iy.size(0) * i77)];
    }
  }
  pad[0] = 3.0;
  pad[1] = 3.0;
  if ((IxIy.size(0) != 0) && (IxIy.size(1) != 0)) {
    ...
    c_a.set_size(b_a.size(0), b_a.size(1));
    dd_loop_ub = b_a.size(1);
    for (int i84{0}; i84 < dd_loop_ub; i84++) {
      int gd_loop_ub;
      gd_loop_ub = b_a.size(0);
      for (int i87{0}; i87 < gd_loop_ub; i87++) {
        c_a[i87 + (c_a.size(0) * i84)] =
            static_cast<double>(b_a[i87 + (b_a.size(0) * i84)]);
      }
    }
    internal::c_conv2AXPYValidCMP(c_a, b);
  ...
  if (guard2) {
    board0 = vision::internal::calibration::checkerboard::orient(board0, m_I);
