...
static void poly2RectMask(double e_X[4], double h_Y[4], double height,
                          ...
                          double width, ::coder::array<bool, 2U> &mask)
{
  ...
  int loop_ub_tmp;
  int unnamed_idx_0;
  ::coder::internal::d_sort(e_X);
  ::coder::internal::d_sort(h_Y);
  ...
  if (h_Y[1] > h_Y[2]) {
    ...
    i2 = (static_cast<int>(h_Y[1])) - 1;
    i3 = static_cast<int>(h_Y[2]);
  }
  if (e_X[1] > e_X[2]) {
    i4 = 0;
    i5 = 0;
  } else {
    i4 = (static_cast<int>(e_X[1])) - 1;
    i5 = static_cast<int>(e_X[2]);
  }
  unnamed_idx_0 = i3 - i2;
  loop_ub = i5 - i4;
...
//
// Arguments    : const ::coder::array<float, 2U> &k_I
...
namespace calibration {
namespace checkerboard {
void detectCheckerboard(const ::coder::array<float, 2U> &k_I,
                        ::coder::array<double, 2U> &b_points,
                        double boardSize[2])
{
  ...
  bool guard1{false};
  bool guard2{false};
  finalSize[0] = static_cast<double>(k_I.size(0));
  pad[0] = 14.0;
  finalSize[1] = static_cast<double>(k_I.size(1));
  pad[1] = 14.0;
  if ((k_I.size(0) == 0) || (k_I.size(1) == 0)) {
    int loop_ub;
    Ig.set_size(k_I.size(0), k_I.size(1));
    loop_ub = k_I.size(1);
    for (int b_i{0}; b_i < loop_ub; b_i++) {
      int c_loop_ub;
      c_loop_ub = k_I.size(0);
      for (int i2{0}; i2 < c_loop_ub; i2++) {
        Ig[i2 + (Ig.size(0) * b_i)] = k_I[i2 + (k_I.size(0) * b_i)];
      }
    }
  } else {
    int b_loop_ub;
    int e_loop_ub;
    padImage_outSize(k_I, pad, b_a);
  ...
  if ((b_I_45.size(0) != 0) && (b_I_45.size(1) != 0)) {
    ...
    int jb_loop_ub;
    int pb_loop_ub;
    c_b.set_size(b_I_45.size(0), b_I_45.size(1));
    ...
    for (int i32{0}; i32 < jb_loop_ub; i32++) {
      ...
      for (int i35{0}; i35 < mb_loop_ub; i35++) {
        c_b[i35 + (c_b.size(0) * i32)] = r1[i35 + (r1.size(0) * i32)];
      }
    }
    i34 = b_I_45.size(1);
    for (int k{0}; k < i34; k++) {
      ...
      for (int b_k{0}; b_k < i37; b_k++) {
        c_b[b_k + (c_b.size(0) * k)] =
            std::abs(b_I_45[b_k + (b_I_45.size(0) * k)]);
      }
    }
    r1.set_size(c_b.size(0), c_b.size(1));
    pb_loop_ub = c_b.size(1);
    for (int i38{0}; i38 < pb_loop_ub; i38++) {
      int qb_loop_ub;
      qb_loop_ub = c_b.size(0);
      for (int i41{0}; i41 < qb_loop_ub; i41++) {
        r1[i41 + (r1.size(0) * i38)] = c_b[i41 + (c_b.size(0) * i38)];
      }
    }
  }
  ...
  if ((b_x.size(0) != 0) && (b_x.size(1) != 0)) {
    int loop_ub_tmp;
    int tb_loop_ub;
    c_b.set_size(b_x.size(0), b_x.size(1));
    ...
    for (int i39{0}; i39 < loop_ub_tmp; i39++) {
      ...
      for (int i42{0}; i42 < rb_loop_ub; i42++) {
        c_b[i42 + (c_b.size(0) * i39)] = r[i42 + (r.size(0) * i39)];
      }
    }
    for (int d_k{0}; d_k < loop_ub_tmp; d_k++) {
      ...
      for (int g_k{0}; g_k < i45; g_k++) {
        c_b[g_k + (c_b.size(0) * d_k)] =
            std::abs(b_x[g_k + (b_x.size(0) * d_k)]);
      }
    }
    r.set_size(c_b.size(0), c_b.size(1));
    tb_loop_ub = c_b.size(1);
    for (int i46{0}; i46 < tb_loop_ub; i46++) {
      int wb_loop_ub;
      wb_loop_ub = c_b.size(0);
      for (int i49{0}; i49 < wb_loop_ub; i49++) {
        r[i49 + (r.size(0) * i46)] = c_b[i49 + (c_b.size(0) * i46)];
      }
    }
  }
  c_b.set_size(Ixy.size(0), Ixy.size(1));
  if ((Ixy.size(0) != 0) && (Ixy.size(1) != 0)) {
    ...
    for (int c_k{0}; c_k < i40; c_k++) {
      int i44;
      i44 = c_b.size(0);
      for (int f_k{0}; f_k < i44; f_k++) {
        c_b[f_k + (c_b.size(0) * c_k)] =
            std::abs(Ixy[f_k + (Ixy.size(0) * c_k)]);
      }
    }
  }
  r2.set_size(c_b.size(0), c_b.size(1));
  sb_loop_ub = c_b.size(1);
  for (int i43{0}; i43 < sb_loop_ub; i43++) {
    int vb_loop_ub;
    vb_loop_ub = c_b.size(0);
    for (int i48{0}; i48 < vb_loop_ub; i48++) {
      r2[i48 + (r2.size(0) * i43)] = 16.0F * c_b[i48 + (c_b.size(0) * i43)];
    }
  }
  ...
  if ((Ig.size(0) != 0) && (Ig.size(1) != 0)) {
    int b_loop_ub_tmp;
    int dc_loop_ub;
    c_b.set_size(Ig.size(0), Ig.size(1));
    ...
    for (int i54{0}; i54 < b_loop_ub_tmp; i54++) {
      ...
      for (int i55{0}; i55 < cc_loop_ub; i55++) {
        c_b[i55 + (c_b.size(0) * i54)] = r[i55 + (r.size(0) * i54)];
      }
    }
    for (int h_k{0}; h_k < b_loop_ub_tmp; h_k++) {
      int i57;
      i57 = c_b.size(0);
      for (int i_k{0}; i_k < i57; i_k++) {
        c_b[i_k + (c_b.size(0) * h_k)] = std::abs(Ig[i_k + (Ig.size(0) * h_k)]);
      }
    }
    r.set_size(c_b.size(0), c_b.size(1));
    dc_loop_ub = c_b.size(1);
    for (int i59{0}; i59 < dc_loop_ub; i59++) {
      int gc_loop_ub;
      gc_loop_ub = c_b.size(0);
      for (int i64{0}; i64 < gc_loop_ub; i64++) {
        r[i64 + (r.size(0) * i59)] = c_b[i64 + (c_b.size(0) * i59)];
      }
    }
  }
  ...
  if ((Iy.size(0) != 0) && (Iy.size(1) != 0)) {
    int c_loop_ub_tmp;
    int hc_loop_ub;
    c_b.set_size(Iy.size(0), Iy.size(1));
    ...
    for (int i56{0}; i56 < c_loop_ub_tmp; i56++) {
      ...
      for (int i60{0}; i60 < ec_loop_ub; i60++) {
        c_b[i60 + (c_b.size(0) * i56)] = r1[i60 + (r1.size(0) * i56)];
      }
    }
    for (int k_k{0}; k_k < c_loop_ub_tmp; k_k++) {
      ...
      for (int o_k{0}; o_k < i63; o_k++) {
        c_b[o_k + (c_b.size(0) * k_k)] = std::abs(Iy[o_k + (Iy.size(0) * k_k)]);
      }
    }
    r1.set_size(c_b.size(0), c_b.size(1));
    hc_loop_ub = c_b.size(1);
    for (int i65{0}; i65 < hc_loop_ub; i65++) {
      int jc_loop_ub;
      jc_loop_ub = c_b.size(0);
      for (int i67{0}; i67 < jc_loop_ub; i67++) {
        r1[i67 + (r1.size(0) * i65)] = c_b[i67 + (c_b.size(0) * i65)];
      }
    }
  }
  c_b.set_size(I_45_45.size(0), I_45_45.size(1));
  if ((I_45_45.size(0) != 0) && (I_45_45.size(1) != 0)) {
    ...
    for (int j_k{0}; j_k < i58; j_k++) {
      ...
      for (int m_k{0}; m_k < i62; m_k++) {
        c_b[m_k + (c_b.size(0) * j_k)] =
            std::abs(I_45_45[m_k + (I_45_45.size(0) * j_k)]);
      }
    }
  }
  c45.set_size(c_b.size(0), c_b.size(1));
  fc_loop_ub = c_b.size(1);
  for (int i61{0}; i61 < fc_loop_ub; i61++) {
    int ic_loop_ub;
    ic_loop_ub = c_b.size(0);
    for (int i66{0}; i66 < ic_loop_ub; i66++) {
      c45[i66 + (c45.size(0) * i61)] =
          (16.0F * c_b[i66 + (c_b.size(0) * i61)]) -
  ...
  for (int i69{0}; i69 < mc_loop_ub; i69++) {
    ...
    for (int i70{0}; i70 < oc_loop_ub; i70++) {
      c_b[i70 + (c_b.size(0) * i69)] = Ix2[i70 + (Ix2.size(0) * i69)];
    }
  }
  f_N = Ig.size(1);
  for (int p_k{0}; p_k < f_N; p_k++) {
    int g_N;
    g_N = c_b.size(0);
    for (int q_k{0}; q_k < g_N; q_k++) {
      c_b[q_k + (c_b.size(0) * p_k)] =
          rt_powf_snf(Ig[q_k + (Ig.size(0) * p_k)], 2.0F);
    }
  }
  Ix2.set_size(c_b.size(0), c_b.size(1));
  pc_loop_ub = c_b.size(1);
  for (int i71{0}; i71 < pc_loop_ub; i71++) {
    int qc_loop_ub;
    qc_loop_ub = c_b.size(0);
    for (int i72{0}; i72 < qc_loop_ub; i72++) {
      Ix2[i72 + (Ix2.size(0) * i71)] = c_b[i72 + (c_b.size(0) * i71)];
    }
  }
  ...
  pad[0] = 3.0;
  pad[1] = 3.0;
  if ((c_b.size(0) != 0) && (c_b.size(1) != 0)) {
    int rc_loop_ub;
    int vc_loop_ub;
    padImage_outSize(c_b, pad, b_a);
    c_a.set_size(b_a.size(0), b_a.size(1));
  ...
  if (scores.size(0) == 0) {
    ...
    for (int c_i{0}; c_i < i16; c_i++) {
      ...
      if ((static_cast<double>(std::abs(
              std::abs(std::abs(rt_atan2f_snf(v1[1], v1[0])) - 3.14159274F) -
      ...
      if (guard1) {
        currentBoard->initialize(
            static_cast<double>(static_cast<unsigned int>(d)), b_points, v1,
            v2);
...
//                const ::coder::array<float, 2U> &k_I
// Return Type  : b_Checkerboard *
//
                       ...
                       const ::coder::array<float, 2U> &k_I)
{
  ...
  if (!std::isinf(b_x)) {
    ...
    for (int k{0}; k < b_hi; k++) {
      ...
      b_k = 0;
      exitg1 = false;
      while ((!exitg1) && (b_k <= (c_x.size(0) - 1))) {
        if (c_x[b_k + (c_x.size(0) * k)] == 0.0) {
          y[k] = false;
          exitg1 = true;
        } else {
          b_k++;
    ...
    if (b_y) {
      ...
      poly2RectMask(upperLeftPolyX, upperLeftPolyY,
                    static_cast<double>(k_I.size(0)),
                    static_cast<double>(k_I.size(1)), upperLeftMask);
      ...
      poly2RectMask(nextSquarePolyX, nextSquarePolyY,
                    static_cast<double>(k_I.size(0)),
                    static_cast<double>(k_I.size(1)), nextSquareMask);
    ...
    g_x = b_board->BoardCoords.size(0);
    if (g_x == 0) {
      b_r = 0;
    } else {
      b_r = static_cast<int>(std::fmod(static_cast<double>(g_x), 2.0));
      if (b_r == 0) {
        b_r = 0;
      } else if (g_x < 0) {
        b_r += 2;
    ...
    j_x = b_board->BoardCoords.size(1);
    if (j_x == 0) {
      c_r = 0;
    } else {
      c_r = static_cast<int>(std::fmod(static_cast<double>(j_x), 2.0));
      if (c_r == 0) {
        c_r = 0;
      } else if (j_x < 0) {
        c_r += 2;
    ...
    if ((b_r == 0) == (c_r == 0)) {
      ...
      k_x.set_size(1, 1, r6.size(2));
      kb_loop_ub = r6.size(2);
      for (int i36{0}; i36 < kb_loop_ub; i36++) {
        k_x[i36] = (r6[i36] > r7[i36]);
      }
      c_y = false;
      d_k = 0;
      exitg1 = false;
      while ((!exitg1) && (d_k <= (k_x.size(2) - 1))) {
        if (k_x[d_k]) {
          c_y = true;
          exitg1 = true;
        } else {
      ...
      if (c_y) {
        double m_x;
        m_x = b_board->BoardCoords[(b_board->BoardCoords.size(0) +
                                    (b_board->BoardCoords.size(0) *
              ...
              ::coder::array<double, 2U> &b_points, double boardSize[2])
{
  ...
  if (y) {
    ...
    c_x.set_size(c_loop_ub, b_loop_ub);
    for (int i3{0}; i3 < b_loop_ub; i3++) {
      for (int i4{0}; i4 < c_loop_ub; i4++) {
        c_x[i4 + (c_x.size(0) * i3)] =
            b_this->BoardCoords[i3 + (b_this->BoardCoords.size(0) * i4)];
      }
    }
    d_x = c_x.size(0) * c_x.size(1);
    for (int i5{0}; i5 < d_x; i5++) {
      b_points[i5] = c_x[i5];
    }
