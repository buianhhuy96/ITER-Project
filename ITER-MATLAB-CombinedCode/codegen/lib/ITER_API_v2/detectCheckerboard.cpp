...
static void poly2RectMask(double g_X[4], double h_Y[4], double height,
...
static float rt_atan2f_snf(float u0, float c_u1);

// Function Definitions
//
// Arguments    : double g_X[4]
//                double h_Y[4]
//                double height
//                double width
                          ...
                          double width, ::coder::array<bool, 2U> &mask)
{
  ...
  if (g_X[1] > g_X[2]) {
    ...
    i4 = (static_cast<int>(g_X[1])) - 1;
    i5 = static_cast<int>(g_X[2]);
  }
  unnamed_idx_0 = i3 - i2;
  loop_ub = i5 - i4;
...
static float rt_atan2f_snf(float u0, float c_u1)
{
  ...
  if ((std::isnan(u0)) || (std::isnan(c_u1))) {
    ...
    y = std::atan2(static_cast<float>(b_u0), static_cast<float>(d_u1));
  } else if (c_u1 == 0.0F) {
    if (u0 > 0.0F) {
      y = RT_PIF / 2.0F;
    } else if (u0 < 0.0F) {
...
//
// Arguments    : const ::coder::array<float, 2U> &w_I
...
namespace calibration {
namespace checkerboard {
void detectCheckerboard(const ::coder::array<float, 2U> &w_I,
                        ::coder::array<double, 2U> &b_points,
                        double boardSize[2])
{
  ...
  bool guard1{false};
  bool guard2{false};
  finalSize[0] = static_cast<double>(w_I.size(0));
  pad[0] = 14.0;
  finalSize[1] = static_cast<double>(w_I.size(1));
  pad[1] = 14.0;
  if ((w_I.size(0) == 0) || (w_I.size(1) == 0)) {
    int loop_ub;
    Ig.set_size(w_I.size(0), w_I.size(1));
    loop_ub = w_I.size(1);
    for (int b_i{0}; b_i < loop_ub; b_i++) {
      int c_loop_ub;
      c_loop_ub = w_I.size(0);
      for (int i2{0}; i2 < c_loop_ub; i2++) {
        Ig[i2 + (Ig.size(0) * b_i)] = w_I[i2 + (w_I.size(0) * b_i)];
      }
    }
  } else {
    int b_loop_ub;
    int e_loop_ub;
    padImage_outSize(w_I, pad, a);
    ...
    for (int i1{0}; i1 < b_loop_ub; i1++) {
      ...
      for (int i3{0}; i3 < d_loop_ub; i3++) {
        b_a[i3 + (b_a.size(0) * i1)] =
            static_cast<double>(a[i3 + (a.size(0) * i1)]);
      }
    }
  ...
  if ((Ig.size(0) == 0) || (Ig.size(1) == 0)) {
    ...
    for (int i5{0}; i5 < f_loop_ub; i5++) {
      ...
      for (int i8{0}; i8 < i_loop_ub; i8++) {
        b_Iy[i8 + (b_Iy.size(0) * i5)] = Ig[i8 + (Ig.size(0) * i5)];
      }
    }
  } else {
    int h_loop_ub;
    ...
    for (int i7{0}; i7 < h_loop_ub; i7++) {
      ...
      for (int i9{0}; i9 < j_loop_ub; i9++) {
        b_a[i9 + (b_a.size(0) * i7)] =
            static_cast<double>(a[i9 + (a.size(0) * i7)]);
      }
    }
    ::coder::internal::conv2AXPYValidCMP(b_a, b);
  ...
  if ((Ig.size(0) == 0) || (Ig.size(1) == 0)) {
    ...
    for (int i13{0}; i13 < p_loop_ub; i13++) {
      ...
      for (int i15{0}; i15 < r_loop_ub; i15++) {
        b_a[i15 + (b_a.size(0) * i13)] =
            static_cast<double>(a[i15 + (a.size(0) * i13)]);
      }
    }
    ::coder::internal::conv2AXPYValidCMP(b_a, b);
  ...
  for (int i20{0}; i20 < w_loop_ub; i20++) {
    ...
    for (int i21{0}; i21 < x_loop_ub; i21++) {
      b_I_45[i21 + (b_I_45.size(0) * i20)] =
          r[i21 + (r.size(0) * i20)] +
          (b_Iy[i21 + (b_Iy.size(0) * i20)] * 0.707106769F);
    }
  }
  ...
  if ((b_I_45.size(0) == 0) || (b_I_45.size(1) == 0)) {
    ...
    for (int i25{0}; i25 < cb_loop_ub; i25++) {
      ...
      for (int i27{0}; i27 < eb_loop_ub; i27++) {
        b_a[i27 + (b_a.size(0) * i25)] =
            static_cast<double>(a[i27 + (a.size(0) * i25)]);
      }
    }
    ::coder::internal::conv2AXPYValidCMP(b_a, b);
  ...
  for (int i33{0}; i33 < kb_loop_ub; i33++) {
    ...
    for (int i36{0}; i36 < ob_loop_ub; i36++) {
      b_x[i36 + (b_x.size(0) * i33)] =
          r[i36 + (r.size(0) * i33)] +
          (b_Iy[i36 + (b_Iy.size(0) * i33)] * -0.707106769F);
    }
  }
  ...
  if ((b_b.size(0) != 0) && (b_b.size(1) != 0)) {
    ...
    for (int i73{0}; i73 < rc_loop_ub; i73++) {
      ...
      for (int i74{0}; i74 < sc_loop_ub; i74++) {
        b_a[i74 + (b_a.size(0) * i73)] =
            static_cast<double>(a[i74 + (a.size(0) * i73)]);
      }
    }
    ::coder::internal::c_conv2AXPYValidCMP(b_a, b);
  ...
  if ((Iy2.size(0) != 0) && (Iy2.size(1) != 0)) {
    ...
    for (int i76{0}; i76 < uc_loop_ub; i76++) {
      ...
      for (int i79{0}; i79 < xc_loop_ub; i79++) {
        b_a[i79 + (b_a.size(0) * i76)] =
            static_cast<double>(a[i79 + (a.size(0) * i76)]);
      }
    }
    ::coder::internal::c_conv2AXPYValidCMP(b_a, b);
  ...
  for (int i75{0}; i75 < tc_loop_ub; i75++) {
    ...
    for (int i78{0}; i78 < wc_loop_ub; i78++) {
      IxIy[i78 + (IxIy.size(0) * i75)] =
          Ig[i78 + (Ig.size(0) * i75)] * b_Iy[i78 + (b_Iy.size(0) * i75)];
    }
  }
  pad[0] = 3.0;
  pad[1] = 3.0;
  if ((IxIy.size(0) != 0) && (IxIy.size(1) != 0)) {
    ...
    for (int i82{0}; i82 < bd_loop_ub; i82++) {
      ...
      for (int i85{0}; i85 < ed_loop_ub; i85++) {
        b_a[i85 + (b_a.size(0) * i82)] =
            static_cast<double>(a[i85 + (a.size(0) * i82)]);
      }
    }
    ::coder::internal::c_conv2AXPYValidCMP(b_a, b);
...
//                const ::coder::array<float, 2U> &w_I
// Return Type  : b_Checkerboard *
//
  ...
  if (!std::isinf(b_x)) {
    ...
    if (b_y) {
      ...
      poly2RectMask(upperLeftPolyX, upperLeftPolyY,
                    static_cast<double>(w_I.size(0)),
                    static_cast<double>(w_I.size(1)), upperLeftMask);
      ...
      poly2RectMask(nextSquarePolyX, nextSquarePolyY,
                    static_cast<double>(w_I.size(0)),
                    static_cast<double>(w_I.size(1)), nextSquareMask);
      ...
      b_partialTrueCount = 0;
      for (int h_i{0}; h_i <= b_end; h_i++) {
        if (nextSquareMask[h_i]) {
          r4[b_partialTrueCount] = h_i + 1;
          b_partialTrueCount++;
        }
      }
      ab_I.set_size(r2.size(0));
      ab_loop_ub = r2.size(0);
      for (int i22{0}; i22 < ab_loop_ub; i22++) {
        ab_I[i22] = w_I[r2[i22] - 1];
      }
      db_I.set_size(r4.size(0));
      cb_loop_ub = r4.size(0);
      for (int i27{0}; i27 < cb_loop_ub; i27++) {
        db_I[i27] = w_I[r4[i27] - 1];
      }
      if (!(b_mean(ab_I) < b_mean(db_I))) {
        int hb_loop_ub;
        int ob_loop_ub;
        int pb_loop_ub;
        int rb_loop_ub;
    ...
    e_x = b_board->BoardCoords.size(0);
    if (e_x == 0) {
      b_r = 0;
    } else {
      b_r = static_cast<int>(std::fmod(static_cast<double>(e_x), 2.0));
      if (b_r == 0) {
        b_r = 0;
      } else if (e_x < 0) {
        b_r += 2;
    ...
    if (k_x == 0) {
      ...
      c_r = static_cast<int>(std::fmod(static_cast<double>(k_x), 2.0));
      if (c_r == 0) {
        c_r = 0;
      } else if (k_x < 0) {
        c_r += 2;
              ...
              ::coder::array<double, 2U> &b_points, double boardSize[2])
{
  ...
  if (y) {
    ...
    d_y = b_y.size(0) * b_y.size(1);
    for (int i8{0}; i8 < d_y; i8++) {
