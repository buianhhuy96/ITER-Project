                 ...
                 projective2d *tform)
{
  ...
  c_loop_ub = uv.size(0);
  u.set_size(uv.size(0));
  for (int i2{0}; i2 < c_loop_ub; i2++) {
    u[i2] = uv[i2];
  }
  d_loop_ub = uv.size(0);
  v.set_size(uv.size(0));
  for (int i3{0}; i3 < d_loop_ub; i3++) {
    v[i3] = uv[i3 + uv.size(0)];
  }
  c_B.set_size(u.size(0));
  e_loop_ub = u.size(0);
  ...
  g_X.set_size(b_x.size(0) + xy.size(0), 8);
  ...
  if (g_X.size(0) != 0) {
    ...
    h_n = static_cast<int>(std::fmin(static_cast<double>(g_X.size(0)), 8.0));
    if (p) {
      internal::b_svd(g_X, s);
    } else {
      s.set_size(h_n);
  ...
  c_B.set_size(u.size(0) + v.size(0));
  y_loop_ub = u.size(0);
  ...
  if (g_X.size(0) == 0) {
    ...
    for (int j{0}; j < 7; j++) {
      ...
      i_n = 8 - j;
      a = 0;
      smax = std::abs(g_X[b_tmp]);
      for (int e_k{2}; e_k <= i_n; e_k++) {
        double b_s;
        b_s = std::abs(g_X[(b_tmp + e_k) - 1]);
        if (b_s > smax) {
          a = e_k - 1;
          smax = b_s;
        }
      ...
      if (g_X[b_tmp + a] != 0.0) {
        ...
        if (a != 0) {
          ...
          for (int f_k{0}; f_k < 8; f_k++) {
            ...
            b_temp = g_X[b_temp_tmp];
            i38 = ipiv_tmp + temp_tmp;
            g_X[b_temp_tmp] = g_X[i38];
            g_X[i38] = b_temp;
          }
        }
        i32 = (b_tmp - j) + 8;
        for (int h_i{jp1j}; h_i <= i32; h_i++) {
          g_X[h_i - 1] = g_X[h_i - 1] / g_X[b_tmp];
        }
      }
      jA = b_tmp;
      for (int c_j{0}; c_j <= mmj_tmp; c_j++) {
        ...
        if (g_X[yjy_tmp] != 0.0) {
          ...
          for (int ijA{i35}; ijA <= i37; ijA++) {
            g_X[ijA - 1] =
                g_X[ijA - 1] + (g_X[((b_tmp + ijA) - jA) - 9] * (-yjy));
          }
        }
        jA += 8;
      }
    ...
    for (int c_k{0}; c_k < 8; c_k++) {
      ...
      if (c_B[c_k] != 0.0) {
        ...
        for (int c_i{i30}; c_i < 9; c_i++) {
          c_B[c_i - 1] = c_B[c_i - 1] - (c_B[c_k] * g_X[(c_i + kAcol) - 1]);
        }
      }
    }
    for (int d_k{7}; d_k >= 0; d_k--) {
      ...
      b_kAcol = d_k * 8;
      if (c_B[d_k] != 0.0) {
        c_B[d_k] = c_B[d_k] / g_X[d_k + b_kAcol];
        for (int e_i{0}; e_i < d_k; e_i++) {
          c_B[e_i] = c_B[e_i] - (c_B[d_k] * g_X[e_i + b_kAcol]);
        }
      }
    }
  } else {
    ...
    int rankA;
    int u0;
    internal::lapack::b_xgeqp3(g_X, tau, jpvt);
    rankA = 0;
    if (g_X.size(0) < 8) {
      minmn = g_X.size(0);
      maxmn = 8;
    ...
    if (minmn > 0) {
      ...
      while ((!exitg1) && (rankA < minmn)) {
        if (!(std::abs(g_X[rankA + (g_X.size(0) * rankA)]) <= tol)) {
          rankA++;
        } else {
          exitg1 = true;
        }
    ...
    for (int b_j{0}; b_j < b_mn; b_j++) {
      if (tau[b_j] != 0.0) {
        ...
        wj = c_B[b_j];
        i34 = b_j + 2;
        for (int k_i{i34}; k_i <= m; k_i++) {
          wj += g_X[(k_i + (g_X.size(0) * b_j)) - 1] * c_B[k_i - 1];
        }
        wj *= tau[b_j];
        if (wj != 0.0) {
          c_B[b_j] = c_B[b_j] - wj;
          for (int o_i{i34}; o_i <= m; o_i++) {
            c_B[o_i - 1] =
                c_B[o_i - 1] - (g_X[(o_i + (g_X.size(0) * b_j)) - 1] * wj);
          }
        }
      }
    }
    for (int g_i{0}; g_i < rankA; g_i++) {
      h_Y[jpvt[g_i] - 1] = c_B[g_i];
    }
    for (int d_j{rankA}; d_j >= 1; d_j--) {
      int Y_tmp;
      Y_tmp = jpvt[d_j - 1] - 1;
      h_Y[Y_tmp] /= g_X[(d_j + (g_X.size(0) * (d_j - 1))) - 1];
      for (int m_i{0}; m_i <= (d_j - 2); m_i++) {
        int i39;
        i39 = jpvt[m_i];
        h_Y[i39 - 1] -= h_Y[Y_tmp] * g_X[m_i + (g_X.size(0) * (d_j - 1))];
      }
    }
    c_B.set_size(8);
