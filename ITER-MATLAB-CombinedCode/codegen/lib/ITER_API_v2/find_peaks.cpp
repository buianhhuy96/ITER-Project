                ...
                ::coder::array<float, 2U> &loc)
{
  ...
  if ((static_cast<double>(maxMetric)) <= 4.94065645841247E-324) {
    ...
    for (int g_i{0}; g_i <= end; g_i++) {
      if (metric[g_i] < threshold) {
        r[partialTrueCount] = g_i + 1;
        partialTrueCount++;
      }
    }
    e_loop_ub = r.size(0);
    for (int i8{0}; i8 < e_loop_ub; i8++) {
      expl_temp_bw[r[i8] - 1] = false;
    }
    ...
    for (int i19{0}; i19 < p_loop_ub; i19++) {
      e_v1[i19] = (static_cast<int>(c_idx[i19])) - 1;
    }
    b_vk.set_size(e_v1.size(0));
    q_loop_ub = e_v1.size(0);
    for (int i20{0}; i20 < q_loop_ub; i20++) {
      b_vk[i20] = div_s32(e_v1[i20], static_cast<int>(siz[0]));
    }
    r_loop_ub = e_v1.size(0);
    for (int i21{0}; i21 < r_loop_ub; i21++) {
      e_v1[i21] = e_v1[i21] - (b_vk[i21] * (static_cast<int>(siz[0])));
    }
    s_loop_ub = e_v1.size(0);
    for (int i22{0}; i22 < s_loop_ub; i22++) {
      loc[i22 + loc.size(0)] =
          static_cast<float>(static_cast<int>(e_v1[i22] + 1));
    }
