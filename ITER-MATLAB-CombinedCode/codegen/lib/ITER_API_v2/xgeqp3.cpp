...
//
// Arguments    : ::coder::array<double, 2U> &d_A
...
namespace internal {
namespace lapack {
void b_xgeqp3(::coder::array<double, 2U> &d_A, ::coder::array<double, 1U> &tau,
              int jpvt[8])
{
  double vn1[8];
  double vn2[8];
    ...
    if (y < 1) {
      ...
      for (int c_i{0}; c_i < minmn; c_i++) {
        ...
        if ((c_i + 1) < m) {
          ...
          if (b_mmi > 0) {
            double xnorm_tmp;
            xnorm_tmp = blas::xnrm2(b_mmi - 1, d_A, c_ii + 2);
            if (xnorm_tmp != 0.0) {
              double beta1;
              beta1 = rt_hypotd_snf(d_A[c_ii], xnorm_tmp);
              if (d_A[c_ii] >= 0.0) {
                beta1 = -beta1;
              }
              if (std::abs(beta1) < 1.0020841800044864E-292) {
                ...
                do {
                  ...
                  for (int e_k{ix0}; e_k <= i3; e_k++) {
                    d_A[e_k - 1] = 9.9792015476736E+291 * d_A[e_k - 1];
                  }
                  beta1 *= 9.9792015476736E+291;
                  b_atmp *= 9.9792015476736E+291;
                } while (!(std::abs(beta1) >= 1.0020841800044864E-292));
                beta1 = rt_hypotd_snf(b_atmp,
                                      blas::xnrm2(b_mmi - 1, d_A, c_ii + 2));
                if (b_atmp >= 0.0) {
                  beta1 = -beta1;
                }
                tau[c_i] = (beta1 - b_atmp) / beta1;
                b_a = 1.0 / (b_atmp - beta1);
                for (int f_k{ix0}; f_k <= i3; f_k++) {
                  d_A[f_k - 1] = b_a * d_A[f_k - 1];
                }
                ...
                double a;
                int i4;
                tau[c_i] = (beta1 - d_A[c_ii]) / beta1;
                a = 1.0 / (d_A[c_ii] - beta1);
                i4 = c_ii + b_mmi;
                for (int d_k{ix0}; d_k <= i4; d_k++) {
                  d_A[d_k - 1] = a * d_A[d_k - 1];
                }
                b_atmp = beta1;
              }
            }
            ...
            if ((lastc + 1) != 0) {
              ...
              while (((ma > 0) && (iac <= i5)) || ((ma < 0) && (iac >= i5))) {
                ...
                for (int b_ia{iac}; b_ia <= i7; b_ia++) {
                  c += d_A[b_ia - 1] * d_A[(c_ii + b_ia) - iac];
                }
                work[c_iy] += c;
                c_iy++;
                iac += ma;
              ...
              for (int d_j{0}; d_j <= lastc; d_j++) {
                ...
                if (d3 != 0.0) {
                  ...
                  for (int ijA{jA}; ijA <= i6; ijA++) {
                    d_A[ijA - 1] =
                        d_A[ijA - 1] + (d_A[(c_ii + ijA) - jA] * b_temp);
                  }
                }
                jA += ma;
              }
        ...
        for (int c_j{ip1}; c_j < 9; c_j++) {
          ...
          if (b_d1 != 0.0) {
            ...
            if (temp2 <= 1.4901161193847656E-8) {
              if ((c_i + 1) < m) {
                double d2;
                d2 = blas::xnrm2(b_mmi - 1, d_A, c_ij + 2);
                vn1[c_j - 1] = d2;
                vn2[c_j - 1] = d2;
              } else {
                vn1[c_j - 1] = 0.0;
...
// Return Type  : void
//
void xgeqp3(::coder::array<double, 2U> &d_A, ::coder::array<double, 1U> &tau,
            ::coder::array<int, 2U> &jpvt)
{
  array<double, 1U> vn1;
  array<double, 1U> vn2;
  ...
  if ((d_A.size(0) == 0) || (d_A.size(1) == 0)) {
    ...
    if (y < 1) {
      ...
      for (int c_i{0}; c_i < minmn; c_i++) {
        ...
        if ((c_i + 1) < m) {
          b_atmp = d_A[c_ii];
          b_d1 = reflapack::xzlarfg(b_mmi, &b_atmp, d_A, c_ii + 2);
          tau[c_i] = b_d1;
          d_A[c_ii] = b_atmp;
        } else {
          b_d1 = 0.0;
        ...
        if ((c_i + 1) < h_n) {
          ...
          c_atmp = d_A[c_ii];
          d_A[c_ii] = 1.0;
          reflapack::xzlarf(b_mmi, nmi - 1, c_ii + 1, b_d1, d_A,
                            (c_ii + ma) + 1, ma, work);
          d_A[c_ii] = c_atmp;
        }
        for (int c_j{ip1}; c_j <= h_n; c_j++) {
          ...
          if (d2 != 0.0) {
            ...
            if (temp2 <= 1.4901161193847656E-8) {
              if ((c_i + 1) < m) {
                double d3;
                d3 = blas::xnrm2(b_mmi - 1, d_A, c_ij + 2);
                vn1[c_j - 1] = d3;
                vn2[c_j - 1] = d3;
              } else {
                vn1[c_j - 1] = 0.0;
