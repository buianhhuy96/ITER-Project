...
static float rt_hypotf_snf(float u0, float c_u1);
    ...
    ::coder::array<double, 2U> &newIndices) const
{
  ...
  for (int p_i{0}; p_i <= d_end; p_i++) {
    if ((r[p_i]) && (r1[p_i])) {
      r5[d_partialTrueCount] = p_i + 1;
      d_partialTrueCount++;
    }
  }
  ...
  for (int j{0}; j < i5; j++) {
    ...
    if (c_ii.size(1) >= 2) {
      ...
      b_polyfit(b_this, c_this, static_cast<double>(i_n), currCurve);
...
float b_Checkerboard::computeNewEnergyHorizontal(float oldEnergy) const
{
  ...
  if (y) {
    ...
    for (int t_i{0}; t_i <= e_end; t_i++) {
      if ((r[t_i]) && (r1[t_i])) {
        r6[e_partialTrueCount] = t_i + 1;
        e_partialTrueCount++;
      }
    }
    ...
    ::coder::array<double, 2U> &indices) const
{
  ...
  if (remIdx.size(1) != 0) {
    ...
    for (int c_i{0}; c_i < i6; c_i++) {
      ...
      int u0;
      f_loop_ub = Points.size(1);
      a.set_size(remIdx.size(1), f_loop_ub);
      for (int i7{0}; i7 < f_loop_ub; i7++) {
        int g_loop_ub;
      ...
      diffs.set_size(a.size(0), i8);
      if (i8 != 0) {
        int acoef;
        int bcoef;
        int i10;
        acoef = static_cast<int>(a.size(1) != 1);
        ...
        for (int k{0}; k <= i10; k++) {
          ...
          varargin_2 = acoef * k;
          varargin_3 = bcoef * k;
          b_acoef = static_cast<int>(a.size(0) != 1);
          i13 = diffs.size(0) - 1;
          for (int b_k{0}; b_k <= i13; b_k++) {
            diffs[b_k + (diffs.size(0) * k)] =
                a[(b_acoef * b_k) + (a.size(0) * varargin_2)] -
                (static_cast<float>(
    ...
    ::coder::array<double, 2U> &idx) const
{
  ...
  a.set_size(remIdx.size(1), c_loop_ub);
  for (int i2{0}; i2 < c_loop_ub; i2++) {
    int d_loop_ub;
  ...
  if (h_n > 1) {
    ...
    if (guard1) {
      if (std::floor(b_a) == b_a) {
        int i_loop_ub;
        i_loop_ub = static_cast<int>(std::floor(b_b - b_a));
        firstCoord.set_size(1, i_loop_ub + 1);
        for (int i11{0}; i11 <= i_loop_ub; i11++) {
          firstCoord[i11] = b_a + (static_cast<double>(i11));
        }
      } else {
        eml_float_colon(b_a, b_b, firstCoord);
      }
    }
    dv[0] = 1.0;
    dv[1] = 2.0;
    ...
    b_partialTrueCount = 0;
    for (int h_i{0}; h_i <= b_end; h_i++) {
      if ((static_cast<double>(dists[h_i])) < radius) {
        r[b_partialTrueCount] = h_i + 1;
        b_partialTrueCount++;
      }
    }
    ...
    for (int k_i{0}; k_i < i24; k_i++) {
      ...
      if ((outsize_idx_0 != 0) && (outsize_idx_1 != 0)) {
        ...
        for (int h_k{0}; h_k <= na; h_k++) {
          int i28;
          i28 = dataPts.size(0) - 1;
          for (int b_t{0}; b_t <= i28; b_t++) {
            currPt[b_t + (currPt.size(0) * h_k)] =
                queryPts[k_i + (queryPts.size(0) * h_k)];
          }
        }
      }
      c_a.set_size(dataPts.size(0), 2);
      ...
      z1.set_size(c_a.size(0), 2);
      for (int j_k{0}; j_k < 2; j_k++) {
        int m_N;
        m_N = z1.size(0);
        for (int m_k{0}; m_k < m_N; m_k++) {
          z1[m_k + (z1.size(0) * j_k)] =
              rt_powf_snf(c_a[m_k + (c_a.size(0) * j_k)], 2.0F);
        }
      }
      if (z1.size(0) == 0) {
        b_y.set_size(0);
      ...
      c_x = std::sqrt(c_x);
      dist[k_i] = static_cast<double>(c_x);
    }
                             ...
                             const float v[2]) const
{
  ...
  for (int k{0}; k < inner; k++) {
    for (int c_i{0}; c_i <= mc; c_i++) {
      y[c_i] = y[c_i] + (pointVectors[c_i + (pointVectors.size(0) * k)] * v[k]);
    }
  }
  b = rt_hypotf_snf(v[0], v[1]);
  angleCosines.set_size(y.size(0));
  loop_ub = y.size(0);
  for (int e_i{0}; e_i < loop_ub; e_i++) {
    angleCosines[e_i] = y[e_i] / (euclideanDists[e_i] * b);
  }
    ...
    ::coder::array<double, 2U> &newIndices) const
{
  ...
  for (int j{0}; j < i9; j++) {
    ...
    if (c_ii.size(0) >= 2) {
      ...
      polyfit(b_this, c_this, static_cast<double>(i_n), currCurve);
      currRad = coordDist / 4.0;
      currCoord = currRad + coordDist_tmp;
      exitg1 = false;
      while ((!exitg1) &&
...
void b_Checkerboard::expandBoardDirectionally(double direction)
{
  ...
  case 2: {
    ...
    partialTrueCount = 0;
    for (int m_i{0}; m_i <= e_end; m_i++) {
      if (newIndices[m_i] > 0.0) {
        r3[partialTrueCount] = m_i + 1;
        partialTrueCount++;
      }
    }
    ...
    b_partialTrueCount = 0;
    for (int p_i{0}; p_i <= f_end; p_i++) {
      if (newIndices[p_i] > 0.0) {
        r5[b_partialTrueCount] = p_i + 1;
        b_partialTrueCount++;
      }
    }
                                ...
                                const float e_v1[2], const float v2[2])
{
  ...
  BoardIdx[(BoardIdx.size(0) * 2) + 1] =
      findNeighbor(pointVectors, euclideanDists, e_v1);
  f_v1[0] = -e_v1[0];
  f_v1[1] = -e_v1[1];
  ...
  if (d_y) {
    ...
    cb_l.set_size(1, j_loop_ub);
    for (int i17{0}; i17 < j_loop_ub; i17++) {
      cb_l[i17] = Points[(d_this + (Points.size(0) * i17)) - 1];
    }
    k_loop_ub = BoardCoords.size(2);
    ...
    u.set_size(1, r_loop_ub);
    for (int i23{0}; i23 < r_loop_ub; i23++) {
      u[i23] = Points[(f_this + (Points.size(0) * i23)) - 1];
    }
    s_loop_ub = BoardCoords.size(2);
    ...
    for (int i25{0}; i25 < t_loop_ub; i25++) {
      BoardCoords[BoardCoords.size(0) +
          ...
          static_cast<double>(u[i25]);
    }
    up.set_size(1, u.size(1));
    u_loop_ub = u.size(1);
    for (int i26{0}; i26 < u_loop_ub; i26++) {
      up[i26] = u[i26] - center[i26];
    }
    ...
    left.set_size(1, cb_l.size(1));
    x_loop_ub = cb_l.size(1);
    for (int i29{0}; i29 < x_loop_ub; i29++) {
      left[i29] = cb_l[i29] - center[i29];
    }
