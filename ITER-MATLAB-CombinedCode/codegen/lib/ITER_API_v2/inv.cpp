...
void inv(const double b_x[3][3], double y[3][3])
{
  ...
  t3 = ((c_x[0][1] * c_x[1][2]) - c_x[0][2]) / c_x[2][2];
  t2 = (-(c_x[0][1] + (c_x[2][1] * t3))) / c_x[1][1];
  (&y[0][0])[b_p1] = ((1.0 - (c_x[1][0] * t2)) - (c_x[2][0] * t3)) / c_x[0][0];
  (&y[0][0])[b_p1 + 1] = t2;
  (&y[0][0])[b_p1 + 2] = t3;
  t3 = (-c_x[1][2]) / c_x[2][2];
  t2 = (1.0 - (c_x[2][1] * t3)) / c_x[1][1];
  (&y[0][0])[p2] = (-((c_x[1][0] * t2) + (c_x[2][0] * t3))) / c_x[0][0];
  (&y[0][0])[p2 + 1] = t2;
  (&y[0][0])[p2 + 2] = t3;
  t3 = 1.0 / c_x[2][2];
  t2 = ((-c_x[2][1]) * t3) / c_x[1][1];
  (&y[0][0])[p3] = (-((c_x[1][0] * t2) + (c_x[2][0] * t3))) / c_x[0][0];
  (&y[0][0])[p3 + 1] = t2;
  (&y[0][0])[p3 + 2] = t3;
}
