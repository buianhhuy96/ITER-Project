  ...
  for (int j{0}; j < vstride; j++) {
    ...
    if (vwork.size(0) != 0) {
      ...
      for (int b_j{0}; b_j < nQuartets; b_j++) {
        ...
        b_idx[e_i] = idx4[perm[0] - 1];
        b_idx[e_i + 1] = idx4[perm[1] - 1];
        b_idx[e_i + 2] = idx4[perm[2] - 1];
        b_idx[e_i + 3] = idx4[perm[3] - 1];
        c_x[e_i] = x4[perm[0] - 1];
        c_x[e_i + 1] = x4[perm[1] - 1];
        c_x[e_i + 2] = x4[perm[2] - 1];
        c_x[e_i + 3] = x4[perm[3] - 1];
      }
      nDone = nQuartets * 4;
    ...
    if (vwork.size(0) != 0) {
          ...
          if (b_nBlocks > 0) {
            ...
            for (int b{0}; b < b_nBlocks; b++) {
              ...
              for (int b_b{0}; b_b < 6; b_b++) {
                ...
                nPairs = asr_s32(
                    256, static_cast<unsigned int>(static_cast<int>(b_b + 3)));
                for (int j_k{0}; j_k < nPairs; j_k++) {
                  int blockOffset;
                  int iout;
                  int p;
                  int q;
                  blockOffset = (offset + (j_k * bLen2)) + 1;
                  for (int b_j{0}; b_j < bLen2; b_j++) {
                    int iwork_tmp;
                    iwork_tmp = blockOffset + b_j;
                    b_iwork[b_j] = b_idx[iwork_tmp];
      ...
      if ((nNaNs > 0) && (nNonNaN > 0)) {
        for (int g_k{0}; g_k < nNaNs; g_k++) {
          ...
          xwork[g_k] = c_x[xwork_tmp];
          iwork[g_k] = b_idx[xwork_tmp];
        }
        for (int h_k{nNonNaN}; h_k >= 1; h_k--) {
          int b_x_tmp;
          b_x_tmp = (nNaNs + h_k) - 1;
          c_x[b_x_tmp] = c_x[h_k - 1];
          b_idx[b_x_tmp] = b_idx[h_k - 1];
        }
        for (int i_k{0}; i_k < nNaNs; i_k++) {
          c_x[i_k] = xwork[i_k];
          b_idx[i_k] = iwork[i_k];
        }
      }
