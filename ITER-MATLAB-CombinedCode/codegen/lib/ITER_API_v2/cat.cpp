...
void b_cat(const ::coder::array<double, 1U> &varargin_1,
           ...
           ::coder::array<double, 1U> &y)
{
  array<double, 1U> c_f1;
  array<double, 1U> f1;
  int b_loop_ub;
  int i2;
  int i3;
  int loop_ub;
  y.set_size(varargin_1.size(0) + varargin_2.size(0));
  f1.set_size(varargin_1.size(0));
  loop_ub = varargin_1.size(0);
  for (int b_i{0}; b_i < loop_ub; b_i++) {
    f1[b_i] = varargin_1[b_i];
  }
  c_f1.set_size(varargin_2.size(0));
  b_loop_ub = varargin_2.size(0);
  for (int i1{0}; i1 < b_loop_ub; i1++) {
    c_f1[i1] = varargin_2[i1];
  }
  i2 = f1.size(0);
  for (int j{0}; j < i2; j++) {
    y[j] = f1[j];
  ...
  for (int b_k{0}; b_k < i5; b_k++) {
    ...
    for (int d_k{0}; d_k < i7; d_k++) {
      y[(d_k + (y.size(0) * b_k)) + (y.size(0) * y.size(1))] =
          c_f1[d_k + (c_f1.size(0) * (subsarg_idx_1 - 1))];
    }
  }
}
