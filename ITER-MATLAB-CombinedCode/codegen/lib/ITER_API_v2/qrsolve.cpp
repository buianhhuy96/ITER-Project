             ...
             ::coder::array<double, 1U> &h_Y, int *rankA)
{
  ...
  for (int j{0}; j < c_mn; j++) {
    if (tau[j] != 0.0) {
      ...
      wj = e_B[j];
      i4 = j + 2;
      for (int e_i{i4}; e_i <= m; e_i++) {
        wj += e_A[(e_i + (e_A.size(0) * j)) - 1] * e_B[e_i - 1];
      }
      wj *= tau[j];
      if (wj != 0.0) {
        e_B[j] = e_B[j] - wj;
        for (int h_i{i4}; h_i <= m; h_i++) {
          e_B[h_i - 1] =
              e_B[h_i - 1] - (e_A[(h_i + (e_A.size(0) * j)) - 1] * wj);
        }
      }
    }
  }
  ...
  for (int b_j{assumedRank}; b_j >= 1; b_j--) {
    int i5;
    i5 = jpvt[b_j - 1];
    h_Y[i5 - 1] = h_Y[i5 - 1] / e_A[(b_j + (e_A.size(0) * (b_j - 1))) - 1];
    for (int g_i{0}; g_i <= (b_j - 2); g_i++) {
      h_Y[jpvt[g_i] - 1] =
          h_Y[jpvt[g_i] - 1] -
          (h_Y[jpvt[b_j - 1] - 1] * e_A[g_i + (e_A.size(0) * (b_j - 1))]);
    }
  }
  *rankA = b_rankA;
}
