  ...
  for (int j{0}; j < 2; j++) {
    ...
    if ((&d_A[0][0])[b_tmp + a] != 0.0) {
      ...
      if (a != 0) {
        ...
        temp = (&d_A[0][0])[j];
        (&d_A[0][0])[j] = (&d_A[0][0])[ipiv_tmp];
        (&d_A[0][0])[ipiv_tmp] = temp;
        temp = (&d_A[0][0])[j + 3];
        (&d_A[0][0])[j + 3] = (&d_A[0][0])[ipiv_tmp + 3];
        (&d_A[0][0])[ipiv_tmp + 3] = temp;
        temp = (&d_A[0][0])[j + 6];
        (&d_A[0][0])[j + 6] = (&d_A[0][0])[ipiv_tmp + 6];
        (&d_A[0][0])[ipiv_tmp + 6] = temp;
      }
      i1 = (b_tmp - j) + 3;
    ...
    for (int b_j{0}; b_j <= mmj_tmp; b_j++) {
      ...
      if (yjy_tmp != 0.0) {
        ...
        for (int ijA{i2}; ijA <= i3; ijA++) {
          (&d_A[0][0])[ijA - 1] +=
              (&d_A[0][0])[((b_tmp + ijA) - jA) - 4] * (-yjy_tmp);
        }
      }
      jA += 3;
    }
