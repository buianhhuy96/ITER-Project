                        ...
                        ::coder::array<double, 2U> &points, double boardSize[2])
{
  ...
  bool guard1{false};
  bool guard2{false};
  finalSize[0] = static_cast<double>(b_I.size(0));
  pad[0] = 14.0;
  finalSize[1] = static_cast<double>(b_I.size(1));
  pad[1] = 14.0;
  ...
  for (i = 0; i < loop_ub; i++) {
    N = I_45.size(0);
    for (b_i = 0; b_i < N; b_i++) {
      I_45[b_i + (I_45.size(0) * i)] = 16.0F * I_45[b_i + (I_45.size(0) * i)];
    }
  }
  loop_ub = r.size(1);
  for (i = 0; i < loop_ub; i++) {
    ...
    for (b_i = 0; b_i < N; b_i++) {
      r[b_i + (r.size(0) * i)] =
          6.0F * (r[b_i + (r.size(0) * i)] + a[b_i + (a.size(0) * i)]);
    }
  }
  ...
  board0 = growCheckerboard(points0, b_x, Ix2, Iy2, Ig, 0.0, &lobj_0[0]);
  points.set_size(0, 0);
  boardSize[0] = 0.0;
  boardSize[1] = 0.0;
  ...
  board45 = growCheckerboard(points0, b_x, Ix2, Iy2, Ig, 0.78539816339744828,
                             &lobj_0[3]);
  guard1 = false;
  guard2 = false;
  if (board0->isValid) {
