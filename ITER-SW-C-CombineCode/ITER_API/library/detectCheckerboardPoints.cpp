                              ...
                              double boardSize[2])
{
  ...
  finalSize[0] = static_cast<double>(p_I.size(0));
  pad[0] = 7.0;
  finalSize[1] = static_cast<double>(p_I.size(1));
  pad[1] = 7.0;
  if ((p_I.size(0) == 0) || (p_I.size(1) == 0)) {
    int c_loop_ub;
    Ig.set_size(p_I.size(0), p_I.size(1));
    c_loop_ub = p_I.size(1);
    for (int i2{0}; i2 < c_loop_ub; i2++) {
      int e_loop_ub;
      e_loop_ub = p_I.size(0);
      for (int i4{0}; i4 < e_loop_ub; i4++) {
        Ig[i4 + (Ig.size(0) * i2)] = p_I[i4 + (p_I.size(0) * i2)];
      }
    }
  } else {
    int d_loop_ub;
    int g_loop_ub;
    padImage_outSize(p_I, pad, b_a);
