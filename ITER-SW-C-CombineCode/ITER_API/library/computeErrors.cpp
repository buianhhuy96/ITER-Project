...
void computeErrors(const coder::array<double, 3U> &Hhand2base,
                   ...
                   const coder::array<double, 3U> &imgPts,
                   const coder::array<double, 2U> &Worldpts,
                   const double h_K[4][3], double errors[3])
{
  ...
  for (int i7{0}; i7 < e_loop_ub; i7++) {
    for (int i9{0}; i9 < 3; i9++) {
      PoseErrR[(3 * i9) + (9 * i7)] = 0.0;
      PoseErrR[((3 * i9) + (9 * i7)) + 1] = 0.0;
      PoseErrR[((3 * i9) + (9 * i7)) + 2] = 0.0;
    }
  }
  i8 = imgPts.size(2);
  for (int j{0}; j < i8; j++) {
  ...
  if (g_x.size(0) == 0) {
    ...
    int j_loop_ub;
    int o_loop_ub;
    b_x.set_size(g_x.size(0));
    y.set_size(g_x.size(0));
    j_loop_ub = g_x.size(0);
    ...
    for (int e_k{0}; e_k < 2; e_k++) {
      for (int f_k{0}; f_k < i28; f_k++) {
        y[f_k] = y[f_k] + g_x[f_k + (g_x.size(0) * (e_k + 1))];
      }
    }
  ...
  for (int c_j{0}; c_j < i30; c_j++) {
    ...
    for (int e_j{0}; e_j < 4; e_j++) {
      ...
      for (int k_k{3}; k_k >= 0; k_k--) {
        ...
        d10 = (&b_y[0][0])[i42];
        if (d10 != 0.0) {
          (&b_y[0][0])[i42] = d10 / (&A[0][0])[k_k + kAcol];
          for (int g_i{0}; g_i < k_k; g_i++) {
            int i47;
            i47 = g_i + jBcol;
            (&b_y[0][0])[i47] -= (&b_y[0][0])[i42] * (&A[0][0])[g_i + kAcol];
          }
        }
      }
    }
    for (int i37{0}; i37 < 4; i37++) {
      ...
      for (int i41{0}; i41 < 4; i41++) {
        ...
        for (int i46{0}; i46 < 4; i46++) {
          d11 += d_y[i46][i37] * Hbase2grid[i41][i46];
        }
        HT[i41][i37] = d11;
      }
    }
    ...
    if (i48 != 0) {
      ...
      for (int p_k{0}; p_k <= i49; p_k++) {
        ...
        h_x[3 * p_k] = e_y[3 * varargin_2] / e_y[(3 * varargin_3) + 2];
        h_x[(3 * p_k) + 1] =
    ...
    if (imgPts.size(0) == h_x.size(1)) {
      ...
      b_binary_expand_op(r1, imgPts, c_j, h_x);
    }
    if (r1.size(0) == 0) {
      r2.set_size(0);
    } else {
  ...
  errors[0] =
      coder::combineVectorElements(d_x) / (static_cast<double>(d_x.size(0)));
  errors[1] =
