...
bool testReadRobotPoses::readExpectedOutputMat(string file,
	...
	mxArray *arr = matGetVariable(pmat, "poses");
	if (arr == NULL || mxIsEmpty(arr)) {
		return false;
	} else {
		const mwSize *pose_size = mxGetDimensions(arr);
		poses.set_size(*pose_size, *(pose_size + 1), *(pose_size + 2));
		// copy data
		mwSize num = mxGetNumberOfElements(arr);
	...
	arr = matGetVariable(pmat, "extrinsics");
	if (arr == NULL || mxIsEmpty(arr)) {
		return false;
	} else {
		const mwSize *extrinsicsSize = mxGetDimensions(arr);
		extrinsics.set_size(*extrinsicsSize, *(extrinsicsSize + 1),
				*(extrinsicsSize + 2));
		// copy data
		mwSize num = mxGetNumberOfElements(arr);
		double *pr = mxGetPr(arr);
...
bool testComputeCamExtrinsics::readInputMat(string file,
	...
	if (cam_arr == NULL || mxIsEmpty(cam_arr) || img_arr == NULL
			|| mxIsEmpty(img_arr)) {
		return false;
	} else {
		const mwSize *imageDataSize = mxGetDimensions(img_arr);
		images.set_size(*imageDataSize, *(imageDataSize + 1),
				*(imageDataSize + 2), *(imageDataSize + 3));
		// copy data
		mwSize num = mxGetNumberOfElements(img_arr);
...
bool testComputeCamExtrinsics::readExpectedOutputMat(string file,
	...
	// extract the specified variable
	mxArray *image_points_arr = matGetVariable(pmat, "image_points");
	mxArray *valid_idx_arr = matGetVariable(pmat, "valid_idx");
	mxArray *cam_extrinsics_arr = matGetVariable(pmat, "cam_extrinsics");
	mxArray *camera_poses_arr = matGetVariable(pmat, "camera_poses");
	mxArray *world_points_arr = matGetVariable(pmat, "world_points");
	if (image_points_arr == NULL || mxIsEmpty(image_points_arr)
			|| valid_idx_arr == NULL || mxIsEmpty(valid_idx_arr)
			|| cam_extrinsics_arr == NULL || mxIsEmpty(cam_extrinsics_arr)
			|| camera_poses_arr == NULL || mxIsEmpty(camera_poses_arr)
			|| world_points_arr == NULL || mxIsEmpty(world_points_arr)) {
		return false;
	} else {

		const mwSize *image_points_size = mxGetDimensions(image_points_arr);
		image_points.set_size(*image_points_size, *(image_points_size + 1),
				*(image_points_size + 2));
		// copy data
		mwSize image_points_num = mxGetNumberOfElements(image_points_arr);
		mxDouble *image_points_pr = mxGetDoubles(image_points_arr);
		if (image_points_pr != NULL) {
			for (mwSize i = 0; i < image_points_num; i++) {
				image_points[i] = *(image_points_pr + i);
			}
		}

		const mwSize *valid_idx_size = mxGetDimensions(valid_idx_arr);
		valid_idx.set_size(*valid_idx_size);
		// copy data
		mwSize valid_idx_num = mxGetNumberOfElements(valid_idx_arr);
		bool *valid_idx_pr = mxGetLogicals(valid_idx_arr);
		if (valid_idx_pr != NULL) {
			for (mwSize i = 0; i < valid_idx_num; i++) {
				valid_idx[i] = *(valid_idx_pr + i);
			}
		}

		const mwSize *cam_extrinsics_size = mxGetDimensions(cam_extrinsics_arr);
		cam_extrinsics.set_size(*cam_extrinsics_size,
				*(cam_extrinsics_size + 1), *(cam_extrinsics_size + 2));
		// copy data
		mwSize cam_extrinsics_num = mxGetNumberOfElements(cam_extrinsics_arr);
		mxDouble *cam_extrinsics_pr = mxGetDoubles(cam_extrinsics_arr);
		if (cam_extrinsics_pr != NULL) {
			for (mwSize i = 0; i < cam_extrinsics_num; i++) {
				cam_extrinsics[i] = *(cam_extrinsics_pr + i);
			}
		}

		const mwSize *camera_poses_size = mxGetDimensions(camera_poses_arr);
		camera_poses.set_size(*camera_poses_size, *(camera_poses_size + 1),
				*(cam_extrinsics_size + 2));
		// copy data
		mwSize camera_poses_num = mxGetNumberOfElements(camera_poses_arr);
		mxDouble *camera_poses_pr = mxGetDoubles(camera_poses_arr);
		if (camera_poses_pr != NULL) {
			for (mwSize i = 0; i < camera_poses_num; i++) {
				camera_poses[i] = *(camera_poses_pr + i);
			}
		}

		const mwSize *world_points_size = mxGetDimensions(world_points_arr);
		world_points.set_size(*world_points_size, *(world_points_size + 1));
		// copy data
		mwSize world_points_num = mxGetNumberOfElements(world_points_arr);
		mxDouble *world_points_pr = mxGetDoubles(world_points_arr);
		if (world_points_pr != NULL) {
			for (mwSize i = 0; i < world_points_num; i++) {
				world_points[i] = *(world_points_pr + i);
			}
		}
	}
...
void testComputeCamExtrinsics::operator()() {
		...
		for (int i1 = 0; i1 < d1; i1++) {
			for (int i0 = 0; i0 < d0; i0++) {
				ASSERT_EQUAL_DELTA_DDTM(
						...
						image_points[i0 + (d0 * i1) + (d1 * d0 * i2)], 0.00015,
	...
	d2 = r_cam_extrinsics.size(2);
	d1 = r_cam_extrinsics.size(1);
	d0 = r_cam_extrinsics.size(0);
	...
	for (int i2 = 0; i2 < d2; i2++) {
		for (int i1 = 0; i1 < d1; i1++) {
			for (int i0 = 0; i0 < d0; i0++) {
				ASSERT_EQUAL_DELTA_DDTM(
						"ERROR: Trace back " + test_directory_ + ":",
						r_cam_extrinsics[i0 + (d0 * i1) + (d1 * d0 * i2)],
						cam_extrinsics[i0 + (d0 * i1) + (d1 * d0 * i2)], 0.0001,
						DDTM(" Failed at: cam_extrinsics[" + to_string(i0) + "][" + to_string(i1) + "][" + to_string(i2) + "]"));
			}
		}
	}

	d2 = r_camera_poses.size(2);
	d1 = r_camera_poses.size(1);
	d0 = r_camera_poses.size(0);
	...
	for (int i2 = 0; i2 < d2; i2++) {
		for (int i1 = 0; i1 < d1; i1++) {
			for (int i0 = 0; i0 < d0; i0++) {
				ASSERT_EQUAL_DELTA_DDTM(
						"ERROR: Trace back " + test_directory_ + ":",
						r_camera_poses[i0 + (d0 * i1) + (d1 * d0 * i2)],
						camera_poses[i0 + (d0 * i1) + (d1 * d0 * i2)], 0.0001,
						DDTM(" Failed at: camera_poses[" + to_string(i0) + "][" + to_string(i1) + "][" + to_string(i2) + "]"));
			}
		}
	}

	d1 = r_world_points.size(1);
	d0 = r_world_points.size(0);

	for (int i1 = 0; i1 < d1; i1++) {
		for (int i0 = 0; i0 < d0; i0++) {
			ASSERT_EQUAL_DELTA_DDTM(
					"ERROR: Trace back " + test_directory_ + ":",
					r_world_points[i0 + (d0 * i1)],
					world_points[i0 + (d0 * i1)], 0.0001,
					DDTM(" Failed at: world_points[" + to_string(i0) + "][" + to_string(i1) + "]"));
		}
	}
...
bool testHandeyeShah::readInputMat(string file,
	...
	// extract the specified variable
	mxArray *extrinsics_arr = matGetVariable(pmat, "extrinsics");
	mxArray *cam_extrinsics_arr = matGetVariable(pmat, "cam_extrinsics");
	if (extrinsics_arr == NULL || mxIsEmpty(extrinsics_arr)
			|| cam_extrinsics_arr == NULL || mxIsEmpty(cam_extrinsics_arr)) {
		return false;
	} else {
		const mwSize *extrinsics_arr_size = mxGetDimensions(extrinsics_arr);
		extrinsics.set_size(*extrinsics_arr_size, *(extrinsics_arr_size + 1),
				*(extrinsics_arr_size + 2));
		// copy data
		mwSize num = mxGetNumberOfElements(extrinsics_arr);
		mxDouble *extrinsics_pr = mxGetDoubles(extrinsics_arr);
		if (extrinsics_pr != NULL) {
			for (mwSize i = 0; i < num; i++) {
				extrinsics[i] = *(extrinsics_pr + i);
			}
		}

		const mwSize *cam_extrinsics_arr_size = mxGetDimensions(
				cam_extrinsics_arr);
		cam_extrinsics.set_size(*cam_extrinsics_arr_size,
				*(cam_extrinsics_arr_size + 1), *(cam_extrinsics_arr_size + 2));
		// copy data
		num = mxGetNumberOfElements(cam_extrinsics_arr);
		mxDouble *cam_extrinsics_pr = mxGetDoubles(cam_extrinsics_arr);
		if (cam_extrinsics_pr != NULL) {
			for (mwSize i = 0; i < num; i++) {
				cam_extrinsics[i] = *(cam_extrinsics_pr + i);
...
bool testComputeErrors::readInputMat(string file,
	...
	if (extrinsics_arr == NULL || mxIsEmpty(extrinsics_arr)
			...
			|| cam_extrinsics_arr == NULL || mxIsEmpty(cam_extrinsics_arr)
			|| image_points_arr == NULL || mxIsEmpty(image_points_arr)
			|| world_points_arr == NULL || mxIsEmpty(world_points_arr)
