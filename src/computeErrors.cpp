                   ...
                   const double K[4][3], double errors[3])
{
  ...
  N = 4;
  if (Hcam2grid.size(2) == 0) {
    N = Hcam2grid.size(2);
  } else if (Hcam2grid.size(2) > 4) {
    N = Hcam2grid.size(2);
  } else {
    /* no actions */
  }
  ...
  PoseErrt.set_size(N, 3);
  for (i = 0; i < 3; i++) {
    for (i1 = 0; i1 < N; i1++) {
      PoseErrt[i1 + (PoseErrt.size(0) * i)] = 0.0;
    }
  }
  i = 4;
  if (Hcam2grid.size(2) == 0) {
  ...
  N = 4;
  if (Hhand2base.size(2) == 0) {
    N = Hhand2base.size(2);
  } else if (Hhand2base.size(2) > 4) {
    N = Hhand2base.size(2);
  } else {
    /* no actions */
  }
  err.set_size(imgPts.size(0), N);
  for (i = 0; i < N; i++) {
    jp1j = imgPts.size(0);
    for (i1 = 0; i1 < jp1j; i1++) {
