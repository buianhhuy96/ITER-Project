    ...
    for (i1 = 0; i1 < 3; i1++) {
      for (num_of_valid_images = 0; num_of_valid_images < b_loop_ub;
        ...
        for (c_loop_ub = 0; c_loop_ub < loop_ub; c_loop_ub++) {
          c_images[(c_loop_ub + (c_images.size(0) * num_of_valid_images)) +
                   ((c_images.size(0) * c_images.size(1)) * i1)] =
              b_images[((c_loop_ub + (b_images.size(0) * num_of_valid_images)) +
                        ((b_images.size(0) * b_images.size(1)) * i1)) +
                       (((b_images.size(0) * b_images.size(1)) * 3) * b_i)];
        }
      }
    }
