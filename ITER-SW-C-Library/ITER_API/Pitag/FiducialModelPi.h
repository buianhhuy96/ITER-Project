...
#include "../Pitag/AbstractFiducialModel.h"
#include "../Pitag/FiducialDefines.h"
#include "../Pitag/FiducialPiParameters.h"
...
public:
   ...
   // unsigned long GetPoints(cv::Mat& image, std::vector<t_points>& vec_points, std::string parameter_directory_and_filename);
	unsigned long GetPoints(cv::Mat& image, std::vector<t_points>& vec_points, const blobDetectParams &blobDetectParams_set, const blobFilteringParams &blobFilteringParams_set, const ellipseFitParams &ellipseFitParam_set, const tagLenAngParams &tagLenAngParams_set);
	...
	unsigned long GetPose(cv::Mat& image, std::vector<t_pose>& vec_pose, std::string parameter_directory_and_filename);
	...
	unsigned long LoadParametersJSON(std::string directory_and_filename);

	unsigned long LoadParameters(std::vector<FiducialPiParameters> pi_tags);
	unsigned long LoadProcessingParametersJSON(std::string directory_and_filename, blobDetectParams &blob, blobFilteringParams &blobF, ellipseFitParams &ellipseF, tagLenAngParams &LenAngF);
...
private:
	bool TagAngLenTest(t_pi& newtag, const tagLenAngParams& tagLenAngParams_set);
	bool PtsNotUsed(std::vector<t_pi>& tag_vec, t_pi& newTag);
	bool TagIntrusionTest(std::vector<t_pi>& tag_vec, t_pi& newTag);
