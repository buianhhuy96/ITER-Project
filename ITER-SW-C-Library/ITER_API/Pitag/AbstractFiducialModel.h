...
public:
    ...
    //virtual unsigned long GetPoints(cv::Mat& image, std::vector<t_points>& vec_points, std::string parameter_directory_and_filename) = 0;
	virtual unsigned long GetPoints(cv::Mat& image, std::vector<t_points>& vec_points, const blobDetectParams &blobDetectParams_set, const blobFilteringParams &blobFilteringParams_set, const ellipseFitParams &ellipseFitParam_set, const tagLenAngParams &tagLenAngParams_set) = 0;
	...
	/// @return <code>RET_FAILED</code> if no tag could be detected
	/// <code>RET_OK</code> on success
	virtual unsigned long GetPose(cv::Mat& image, std::vector<t_pose>& vec_pose_CfromO, std::string parameter_directory_and_filename) = 0;
	*/
	...
	*************************************************************/
	...
	virtual unsigned long LoadParametersJSON(std::string directory_and_filename) = 0;
	virtual unsigned long LoadProcessingParametersJSON(std::string directory_and_filename, blobDetectParams &blob, blobFilteringParams &blobF, ellipseFitParams &ellipseF, tagLenAngParams &LenAngF) = 0;
