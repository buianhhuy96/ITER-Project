...
void b_sort(::coder::array<int, 1U> &b_x, ::coder::array<int, 1U> &idx)
{
  ...
  ::coder::array<int, 1U> b_xwork;
  ::coder::array<int, 1U> e_x;
  ::coder::array<int, 1U> iidx;
  ::coder::array<int, 1U> vwork;
  ::coder::array<signed char, 1U> iwork;
  ::coder::array<signed char, 1U> xwork;
  int c_iwork[256];
  int c_xwork[256];
  int idx4[4];
  int x4[4];
  ...
  for (int j{0}; j < vstride; j++) {
    ...
    if (vwork.size(0) != 0) {
      ...
      for (int b_j{0}; b_j < nQuartets; b_j++) {
        ...
        e_x[d_i] = x4[perm[0] - 1];
        e_x[d_i + 1] = x4[perm[1] - 1];
        e_x[d_i + 2] = x4[perm[2] - 1];
        e_x[d_i + 3] = x4[perm[3] - 1];
      }
      nDone = nQuartets * 4;
      ...
      if (vwork.size(0) > 1) {
        if (vwork.size(0) >= 256) {
          ...
          if (nLastBlock > 0) {
            b_merge_block(b_idx, e_x, tailOffset, nLastBlock, 2, b_iwork,
                          b_xwork);
          }
          preSortLevel = 8;
        }
        b_merge_block(b_idx, e_x, 0, vwork.size(0), preSortLevel, b_iwork,
                      b_xwork);
      }
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(c_idx_tmp, \
      ...
      for (int c_k = 0; c_k < c_ib; c_k++) {
        c_idx_tmp = (static_cast<int>(perm[c_k])) - 1;
        d_idx_tmp = ((c_k - nNaNs) - c_ib) + 5;
        idx[d_idx_tmp] = static_cast<int>(idx4[c_idx_tmp]);
        b_x[d_idx_tmp] = x4[c_idx_tmp];
      }
    }
  }
  m = asr_s32(nNaNs, 1U) + 5;
  for (int b_k{0}; b_k <= (m - 6); b_k++) {
    int b_itmp;
    int itmp_tmp;
    itmp_tmp = (b_k - nNaNs) + 5;
    b_itmp = idx[itmp_tmp];
    idx[itmp_tmp] = idx[4 - b_k];
    idx[4 - b_k] = b_itmp;
    b_x[itmp_tmp] = xwork[4 - b_k];
    b_x[4 - b_k] = xwork[itmp_tmp];
  }
  if ((nNaNs & 1) != 0) {
    int i1;
    i1 = m - nNaNs;
...
#pragma omp parallel for num_threads(omp_get_max_threads())
    ...
    while (b_nBlocks > 1) {
      ...
      if ((b_nBlocks & 1) != 0) {
        ...
        b_nBlocks--;
        tailOffset = bLen * b_nBlocks;
        nTail = 5 - (nNaNs + tailOffset);
        if (nTail > bLen) {
          f_merge(idx, b_x, tailOffset, bLen, nTail - bLen, iwork, xwork);
        }
      }
      bLen2 = bLen * 2;
      nPairs = asr_s32(b_nBlocks, 1U);
      for (int d_k{0}; d_k < nPairs; d_k++) {
        f_merge(idx, b_x, d_k * bLen2, bLen, bLen, iwork, xwork);
      }
      bLen = bLen2;
      b_nBlocks = nPairs;
    }
    if ((5 - nNaNs) > bLen) {
      f_merge(idx, b_x, 0, bLen, 5 - (nNaNs + bLen), iwork, xwork);
    }
  }
}
