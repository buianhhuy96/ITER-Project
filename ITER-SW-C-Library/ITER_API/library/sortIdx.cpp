...
static void d_merge_block(::coder::array<int, 1U> &idx,
                          ::coder::array<double, 1U> &b_x, int offset, int h_n,
                          int preSortLevel, ::coder::array<int, 1U> &iwork,
                          ::coder::array<double, 1U> &xwork);

static void e_merge(::coder::array<int, 1U> &idx,
                    ::coder::array<double, 1U> &b_x, int offset, int np, int nq,
                    ::coder::array<int, 1U> &iwork,
                    ::coder::array<double, 1U> &xwork);
                          ...
                          ::coder::array<double, 1U> &xwork)
{
  ...
  bLen = ((static_cast<int>(1)) << (static_cast<unsigned int>(preSortLevel)));
  while (b_nBlocks > 1) {
    int bLen2;
    int nPairs;
    if ((b_nBlocks & 1) != 0) {
                          ...
                          ::coder::array<double, 1U> &xwork)
{
  ...
  while (b_nBlocks > 1) {
    ...
    if ((b_nBlocks & 1) != 0) {
      ...
      nTail = h_n - tailOffset;
      if (nTail > bLen) {
        e_merge(idx, b_x, offset + tailOffset, bLen, nTail - bLen, iwork,
                xwork);
      }
    }
    bLen2 = bLen * 2;
    nPairs = asr_s32(b_nBlocks, 1U);
    for (int k{0}; k < nPairs; k++) {
      e_merge(idx, b_x, offset + (k * bLen2), bLen, bLen, iwork, xwork);
    }
    bLen = bLen2;
    b_nBlocks = nPairs;
  }
  if (h_n > bLen) {
    e_merge(idx, b_x, offset, bLen, h_n - bLen, iwork, xwork);
  }
}
    ...
    for (int k{0}; k < p_n; k++) {
      if (std::isnan(e_x[k])) {
        ...
        if ((c_ib + 1) == 4) {
          ...
          e_x[quartetOffset - 3] = x4[perm[0] - 1];
          e_x[quartetOffset - 2] = x4[perm[1] - 1];
          e_x[quartetOffset - 1] = x4[perm[2] - 1];
          e_x[quartetOffset] = x4[perm[3] - 1];
          c_ib = -1;
        }
      }
    }
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(e_idx_tmp, \
        ...
        for (int c_k = 0; c_k <= c_ib; c_k++) {
          ...
          e_x[f_idx_tmp] = x4[e_idx_tmp];
        }
      }
    }
    m = asr_s32(nNaNs, 1U) + 1;
    for (int b_k{0}; b_k <= (m - 2); b_k++) {
      int b_idx_tmp;
      int b_itmp;
      int itmp_tmp;
      ...
      e_x[itmp_tmp] = xwork[b_idx_tmp];
      e_x[b_idx_tmp] = xwork[itmp_tmp];
    }
    if ((nNaNs & 1) != 0) {
      int x_tmp;
      x_tmp = wOffset + m;
      e_x[x_tmp] = xwork[x_tmp];
    }
    nNonNaN = h_n - nNaNs;
    preSortLevel = 2;
    if (nNonNaN > 1) {
        ...
        if (b_nBlocks > 0) {
          ...
          for (int b{0}; b < b_nBlocks; b++) {
            ...
            for (int b_b{0}; b_b < 6; b_b++) {
              ...
              bLen =
                  ((static_cast<int>(1))
                   << (static_cast<unsigned int>(static_cast<int>(b_b + 2))));
              bLen2 = bLen * 2;
              nPairs = asr_s32(
          ...
          if (nLastBlock > 0) {
            merge_block(b_idx, e_x, tailOffset, nLastBlock, 2, iwork, xwork);
          }
          preSortLevel = 8;
        }
      }
      merge_block(b_idx, e_x, 0, nNonNaN, preSortLevel, iwork, xwork);
    }
    ...
    if (nNonNaN > 1) {
      if (h_n >= 256) {
        ...
        if (b_nBlocks > 0) {
          ...
          nLastBlock = nNonNaN - tailOffset;
          if (nLastBlock > 0) {
            c_merge_block(b_idx, e_x, tailOffset, nLastBlock, 2, iwork, xwork);
          }
          preSortLevel = 8;
        }
      }
      c_merge_block(b_idx, e_x, 0, nNonNaN, preSortLevel, iwork, xwork);
    }
    h_loop_ub = b_idx.size(0);
    if ((static_cast<int>(b_idx.size(0) < 4)) != 0) {
      for (int i5{0}; i5 < h_loop_ub; i5++) {
    ...
    if (nNonNaN > 1) {
      if (h_n >= 256) {
        ...
        if (b_nBlocks > 0) {
          ...
          nLastBlock = nNonNaN - tailOffset;
          if (nLastBlock > 0) {
            d_merge_block(b_idx, e_x, tailOffset, nLastBlock, 2, iwork, xwork);
          }
          preSortLevel = 8;
        }
      }
      d_merge_block(b_idx, e_x, 0, nNonNaN, preSortLevel, iwork, xwork);
    }
    if ((nNaNs > 0) && (nNonNaN > 0)) {
      if ((static_cast<int>(nNaNs < 4)) != 0) {
        for (int d_k{0}; d_k < nNaNs; d_k++) {
