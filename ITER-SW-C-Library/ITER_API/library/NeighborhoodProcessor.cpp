...
static double rt_remd_snf(double u0, double b_u1);
...
{
  double y;
  if (((std::isnan(u0)) || (std::isnan(b_u1))) || (std::isinf(u0))) {
    y = rtNaN;
  } else if (std::isinf(b_u1)) {
    y = u0;
  } else if ((b_u1 != 0.0) && (b_u1 != std::trunc(b_u1))) {
    double b_q;
    b_q = std::abs(u0 / b_u1);
    if (!(std::abs(b_q - std::floor(b_q + 0.5)) > (DBL_EPSILON * b_q))) {
      y = 0.0 * u0;
    } else {
      y = std::fmod(u0, b_u1);
    }
  ...
  if (y != 0) {
    ...
    for (int pind{0}; pind < 9; pind++) {
      if ((&nhConn[0][0])[pind]) {
        ...
        c_r_tmp = (static_cast<int>(rt_remd_snf(
                      ((static_cast<double>(pind)) + 1.0) - 1.0, 3.0))) +
                  1;
        weights_tmp_idx_1_tmp = static_cast<int>(static_cast<double>(
            ((static_cast<double>(static_cast<int>(pind - c_r_tmp))) + 1.0) /
            3.0));
        soffsets[0][indx] = c_r_tmp;
        soffsets[1][indx] = weights_tmp_idx_1_tmp + 1;
        linds[indx] = c_r_tmp + ((static_cast<int>(static_cast<signed char>(
                                     weights_tmp_idx_1_tmp))) *
                                 3);
        loffsets[indx] = c_r_tmp + ((static_cast<int>(static_cast<signed char>(
                                        weights_tmp_idx_1_tmp))) *
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(i1, c_k)

    for (int b_k = 0; b_k < 2; b_k++) {
      for (c_k = 0; c_k < 9; c_k++) {
        i1 = soffsets[b_k][c_k];
        soffsets[b_k][c_k] = i1 - 2;
      }
    }
  }
}
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
    ...
    loop_ub, d_i, i4, pixelInd, exitg1, i3, i2)
  ...
  for (int secondInd = lb_loop; secondInd <= ub_loop; secondInd++) {
    ...
    for (b_firstInd = b_i; b_firstInd <= i1; b_firstInd++) {
      ...
      for (d_i = 0; d_i < 9; d_i++) {
        imnhInds[d_i] = ImageNeighborLinearOffsets[d_i] + pind;
      }
      if (b_in.size(0) == 1) {
        imnh.set_size(1, 9);
        for (i4 = 0; i4 < 9; i4++) {
  ...
  if (ProcessBorder) {
    ...
    if (u0 <= b_u1) {
      ...
      firstDimExtents_idx_1_tmp = b_u1;
    }
    c_u1 = ImageSize[1];
    if (secondDimExtents_idx_1 > c_u1) {
      secondDimExtents_idx_1 = c_u1;
    }
      ...
      for (int firstInd{0}; firstInd < firstDimExtents_idx_1_tmp; firstInd++) {
        ...
        case 1: {
          ...
          for (int k_i{0}; k_i < 9; k_i++) {
            if (isInside[k_i]) {
              c_imnhInds[partialTrueCount] = b_imnhInds_[k_i];
              partialTrueCount++;
            }
          }
        } break;
        ...
        if (b_in.size(0) == 1) {
          ...
          for (int b_i8{0}; b_i8 < b_loop_ub; b_i8++) {
            c_in[b_i8] = b_in[c_imnhInds[b_i8] - 1];
          }
        ...
        if (Padding == 2.0) {
          ...
          for (int w_i{0}; w_i < 9; w_i++) {
            if (!isInside[w_i]) {
              e_r[b_partialTrueCount] = static_cast<signed char>(w_i + 1);
              b_partialTrueCount++;
            }
          }
    ...
    for (int c_secondInd{secondDimExtents_idx_0};
      ...
      for (int c_firstInd{0}; c_firstInd < firstDimExtents_idx_1_tmp;
        ...
        for (int e_i{0}; e_i < 9; e_i++) {
          b_imnhInds_[e_i] = (ImageNeighborLinearOffsets[e_i] + c_pind) + 1;
        }
        if (ImageSize[0] == 0) {
          b_r = 0;
        } else {
        ...
        if (ImageSize[0] == 0) {
          ...
          c_z = div_s32(k_x, b_y);
          k_x -= c_z * b_y;
          if ((k_x > 0) && (k_x >= (asr_s32(b_y, 1U) + (b_y & 1)))) {
            c_z++;
          }
          if ((j_x < 0) != (ImageSize[0] < 0)) {
            c_z = -c_z;
          }
        }
        pixelSub[1] = c_z + 1;
        ...
        case 1: {
          ...
          for (int eb_i{0}; eb_i < 9; eb_i++) {
            if (isInside[eb_i]) {
              d_imnhInds[c_partialTrueCount] = b_imnhInds_[eb_i];
              c_partialTrueCount++;
            }
          }
        } break;
        ...
        if (Padding == 2.0) {
          ...
          for (int jb_i{0}; jb_i < 9; jb_i++) {
            if (!isInside[jb_i]) {
              r1[d_partialTrueCount] = static_cast<signed char>(jb_i + 1);
              d_partialTrueCount++;
            }
          }
          i_loop_ub = r1.size(0);
    ...
    e_u1 = ImageSize[0];
    if (firstDimExtents_idx_1 > e_u1) {
      firstDimExtents_idx_1 = e_u1;
    }
    b_u0 = ImageSize[1];
    f_u1 = ImageSize[1];
    if (b_u0 <= f_u1) {
    ...
    for (int d_secondInd{1}; d_secondInd <= i5; d_secondInd++) {
      for (int d_firstInd{0}; d_firstInd < firstDimExtents_idx_1;
        ...
        for (int h_i{0}; h_i < 9; h_i++) {
          b_imnhInds_[h_i] = (ImageNeighborLinearOffsets[h_i] + d_pind) + 1;
        }
        if (ImageSize[0] == 0) {
          d_r = 0;
        } else {
          d_r = d_pind - (ImageSize[0] * div_s32(d_pind, ImageSize[0]));
        }
        o_x = d_pind - d_r;
        if (ImageSize[0] == 0) {
          if (o_x == 0) {
          ...
          e_z = div_s32(p_x, c_y);
          p_x -= e_z * c_y;
          if ((p_x > 0) && (p_x >= (asr_s32(c_y, 1U) + (c_y & 1)))) {
            e_z++;
          }
        ...
        for (int e_k{0}; e_k < 2; e_k++) {
          for (int g_k{0}; g_k < 9; g_k++) {
            imnhSubs[e_k][g_k] =
                NeighborSubscriptOffsets[e_k][g_k] + pixelSub[e_k];
          }
        }
        ...
        case 1: {
          ...
          for (int kb_i{0}; kb_i < 9; kb_i++) {
            if (isInside[kb_i]) {
              e_imnhInds[e_partialTrueCount] = b_imnhInds_[kb_i];
              e_partialTrueCount++;
            }
          }
        } break;
        ...
        if (Padding == 2.0) {
          ...
          for (int pb_i{0}; pb_i < 9; pb_i++) {
            if (!isInside[pb_i]) {
              r2[f_partialTrueCount] = static_cast<signed char>(pb_i + 1);
              f_partialTrueCount++;
            }
          }
          m_loop_ub = r2.size(0);
    ...
    g_u1 = ImageSize[0];
    if (firstDimExtents_idx_1 > g_u1) {
      firstDimExtents_idx_1 = g_u1;
    }
    for (int e_secondInd{0}; e_secondInd < i5; e_secondInd++) {
      for (int e_firstInd{firstDimExtents_idx_0};
        ...
        for (int t_i{0}; t_i < 9; t_i++) {
          b_imnhInds_[t_i] = (ImageNeighborLinearOffsets[t_i] + e_pind) + 1;
        }
        if (ImageSize[0] == 0) {
          f_r = 0;
        } else {
        ...
        if (ImageSize[0] == 0) {
          ...
          k_z = div_s32(s_x, d_y);
          s_x -= k_z * d_y;
          if ((s_x > 0) && (s_x >= (asr_s32(d_y, 1U) + (d_y & 1)))) {
            k_z++;
          }
          if ((q_x < 0) != (ImageSize[0] < 0)) {
            k_z = -k_z;
          }
        }
        pixelSub[1] = k_z + 1;
        pixelSub[0] = f_r + 1;
        for (int h_k{0}; h_k < 2; h_k++) {
          for (int i_k{0}; i_k < 9; i_k++) {
            imnhSubs[h_k][i_k] =
                NeighborSubscriptOffsets[h_k][i_k] + pixelSub[h_k];
          }
        }
        ...
        case 1: {
          ...
          for (int qb_i{0}; qb_i < 9; qb_i++) {
            if (isInside[qb_i]) {
              f_imnhInds[g_partialTrueCount] = b_imnhInds_[qb_i];
              g_partialTrueCount++;
            }
          }
        } break;
        ...
        if (Padding == 2.0) {
          ...
          for (int sb_i{0}; sb_i < 9; sb_i++) {
            if (!isInside[sb_i]) {
              r3[h_partialTrueCount] = static_cast<signed char>(sb_i + 1);
              h_partialTrueCount++;
            }
          }
          o_loop_ub = r3.size(0);
