...
#pragma omp parallel for num_threads(omp_get_max_threads())
  ...
  d = e_B[0][r2] - (e_B[0][r1] * j_A[0][r2]);
  b_d1 = ((e_B[0][r3] - (e_B[0][r1] * j_A[0][r3])) - (d * j_A[1][r3])) /
         j_A[2][r3];
  g_Y[0][2] = b_d1;
  d -= b_d1 * j_A[2][r2];
  d /= j_A[1][r2];
  g_Y[0][1] = d;
  g_Y[0][0] =
      ((e_B[0][r1] - (b_d1 * j_A[2][r1])) - (d * j_A[1][r1])) / j_A[0][r1];
  d = e_B[1][r2] - (e_B[1][r1] * j_A[0][r2]);
  b_d1 = ((e_B[1][r3] - (e_B[1][r1] * j_A[0][r3])) - (d * j_A[1][r3])) /
         j_A[2][r3];
  g_Y[1][2] = b_d1;
  d -= b_d1 * j_A[2][r2];
  d /= j_A[1][r2];
  g_Y[1][1] = d;
  g_Y[1][0] =
      ((e_B[1][r1] - (b_d1 * j_A[2][r1])) - (d * j_A[1][r1])) / j_A[0][r1];
  d = e_B[2][r2] - (e_B[2][r1] * j_A[0][r2]);
  b_d1 = ((e_B[2][r3] - (e_B[2][r1] * j_A[0][r3])) - (d * j_A[1][r3])) /
         j_A[2][r3];
  g_Y[2][2] = b_d1;
  d -= b_d1 * j_A[2][r2];
  d /= j_A[1][r2];
  g_Y[2][1] = d;
  g_Y[2][0] =
      ((e_B[2][r1] - (b_d1 * j_A[2][r1])) - (d * j_A[1][r1])) / j_A[0][r1];
}
...
#pragma omp parallel for num_threads(omp_get_max_threads())
  ...
  j_A[1][r3] /= j_A[1][r2];
  j_A[2][r3] -= j_A[1][r3] * j_A[2][r2];
  g_Y[1] = e_B[r2] - (e_B[r1] * j_A[0][r2]);
  g_Y[2] = (e_B[r3] - (e_B[r1] * j_A[0][r3])) - (g_Y[1] * j_A[1][r3]);
  g_Y[2] /= j_A[2][r3];
  g_Y[0] = e_B[r1] - (g_Y[2] * j_A[2][r1]);
  g_Y[1] -= g_Y[2] * j_A[2][r2];
  g_Y[1] /= j_A[1][r2];
  g_Y[0] -= g_Y[1] * j_A[1][r1];
  g_Y[0] /= j_A[0][r1];
}
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(i5,        \
    ...
    for (int i3 = 0; i3 < d_loop_ub; i3++) {
      ...
      for (i5 = 0; i5 < f_loop_ub; i5++) {
        j_A[i5 + (j_A.size(0) * i3)] = A[i5 + (A.size(0) * i3)];
      }
    }
    ipiv.set_size(1, h_n);
    ipiv[0] = 1;
      ...
      if (j_A[jj + b_a] != 0.0) {
        ...
        if (b_a != 0) {
          ...
          for (int e_k{0}; e_k < h_n; e_k++) {
            ...
            b_temp = j_A[b_temp_tmp];
            i16 = ipiv_tmp + temp_tmp;
            j_A[b_temp_tmp] = j_A[i16];
            j_A[i16] = b_temp;
          }
        }
        i12 = jj + mmj_tmp;
        for (int g_i{jp1j}; g_i <= i12; g_i++) {
          j_A[g_i - 1] = j_A[g_i - 1] / j_A[jj];
        }
      }
      b_jy = b + b_LDA;
      b_jA = jj + ldap1;
      for (int d_j{0}; d_j <= (mmj_tmp - 2); d_j++) {
        ...
        if (j_A[yjy_tmp] != 0.0) {
          ...
          for (int ijA{i15}; ijA <= i17; ijA++) {
            j_A[ijA - 1] =
                j_A[ijA - 1] + (j_A[((jj + ijA) - b_jA) - 1] * (-yjy));
          }
        }
        b_jA += b_LDA;
      }
...
#pragma omp parallel for num_threads(omp_get_max_threads())
    ...
    for (int c_k{0}; c_k < h_n; c_k++) {
      ...
      if (f_B[c_k] != 0.0) {
        ...
        for (int m_i{i13}; m_i <= h_n; m_i++) {
          f_B[m_i - 1] = f_B[m_i - 1] - (f_B[c_k] * j_A[(m_i + kAcol) - 1]);
        }
      }
    }
    for (int d_k{h_n}; d_k >= 1; d_k--) {
      ...
      d = f_B[d_k - 1];
      if (d != 0.0) {
        f_B[d_k - 1] = d / j_A[(d_k + b_kAcol) - 1];
        for (int q_i{0}; q_i <= (d_k - 2); q_i++) {
          f_B[q_i] = f_B[q_i] - (f_B[d_k - 1] * j_A[q_i + b_kAcol]);
        }
      }
    }
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(i2,        \
    ...
    for (int b_i = 0; b_i < loop_ub; b_i++) {
      ...
      for (i2 = 0; i2 < c_loop_ub; i2++) {
        j_A[i2 + (j_A.size(0) * b_i)] = A[i2 + (A.size(0) * b_i)];
      }
    }
    internal::lapack::xgeqp3(j_A, tau, jpvt);
    rankA = 0;
    ...
    if (minmn > 0) {
      ...
      while ((!exitg1) && (rankA < minmn)) {
        if (!(std::abs(j_A[rankA + (j_A.size(0) * rankA)]) <= tol)) {
          rankA++;
        } else {
          exitg1 = true;
        }
...
#pragma omp parallel for num_threads(omp_get_max_threads())
    ...
    for (int b_j{0}; b_j < b_mn; b_j++) {
      if (tau[b_j] != 0.0) {
        ...
        for (int e_i{i9}; e_i <= m; e_i++) {
          wj += j_A[(e_i + (j_A.size(0) * b_j)) - 1] * f_B[e_i - 1];
        }
        wj *= tau[b_j];
        if (wj != 0.0) {
          f_B[b_j] = f_B[b_j] - wj;
          for (int k_i{i9}; k_i <= m; k_i++) {
            f_B[k_i - 1] =
                f_B[k_i - 1] - (j_A[(k_i + (j_A.size(0) * b_j)) - 1] * wj);
          }
        }
      }
    }
    ...
    for (int c_j{rankA}; c_j >= 1; c_j--) {
      int i11;
      i11 = jpvt[c_j - 1];
      g_Y[i11 - 1] = g_Y[i11 - 1] / j_A[(c_j + (j_A.size(0) * (c_j - 1))) - 1];
      for (int h_i{0}; h_i <= (c_j - 2); h_i++) {
        g_Y[jpvt[h_i] - 1] =
            g_Y[jpvt[h_i] - 1] -
            (g_Y[jpvt[c_j - 1] - 1] * j_A[h_i + (j_A.size(0) * (c_j - 1))]);
      }
    }
  }
  if (guard1) {
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(i4)
    ...
    for (int b_i = 0; b_i < 8; b_i++) {
      for (i4 = 0; i4 < 8; i4++) {
        j_A[i4 + (j_A.size(0) * b_i)] = A[i4 + (A.size(0) * b_i)];
      }
    ...
    for (int j{0}; j < 7; j++) {
      ...
      h_n = 8 - j;
      b_a = 0;
      smax = std::abs(j_A[b_tmp]);
      for (int k{2}; k <= h_n; k++) {
        double s;
        s = std::abs(j_A[(b_tmp + k) - 1]);
        if (s > smax) {
          b_a = k - 1;
          smax = s;
        }
      }
      if (j_A[b_tmp + b_a] != 0.0) {
        int i7;
        if (b_a != 0) {
          int ipiv_tmp;
          ipiv_tmp = j + b_a;
          ...
          for (int d_k{0}; d_k < 8; d_k++) {
            ...
            j_A[b_temp_tmp] = j_A[i12];
            j_A[i12] = b_temp;
          }
        }
        i7 = (b_tmp - j) + 8;
        for (int d_i{jp1j}; d_i <= i7; d_i++) {
          j_A[d_i - 1] = j_A[d_i - 1] / j_A[b_tmp];
        }
      }
      b_jA = b_tmp;
      for (int b_j{0}; b_j <= mmj_tmp; b_j++) {
        ...
        if (j_A[yjy_tmp] != 0.0) {
          ...
          for (int ijA{i10}; ijA <= i11; ijA++) {
            j_A[ijA - 1] =
                j_A[ijA - 1] + (j_A[((b_tmp + ijA) - b_jA) - 9] * (-yjy));
          }
        }
        b_jA += 8;
      }
...
#pragma omp parallel for num_threads(omp_get_max_threads())
    ...
    for (int b_k{0}; b_k < 8; b_k++) {
      ...
      if (g_Y[b_k] != 0.0) {
        ...
        for (int e_i{i9}; e_i < 9; e_i++) {
          g_Y[e_i - 1] = g_Y[e_i - 1] - (g_Y[b_k] * j_A[(e_i + kAcol) - 1]);
        }
      }
    }
    for (int c_k{7}; c_k >= 0; c_k--) {
      ...
      b_kAcol = c_k * 8;
      if (g_Y[c_k] != 0.0) {
        g_Y[c_k] = g_Y[c_k] / j_A[c_k + b_kAcol];
        for (int h_i{0}; h_i < c_k; h_i++) {
          g_Y[h_i] = g_Y[h_i] - (g_Y[c_k] * j_A[h_i + b_kAcol]);
        }
      }
    }
  } else {
    ...
    if ((static_cast<int>((A.size(0) * 8) < 4)) != 0) {
      for (int i2{0}; i2 < 8; i2++) {
        for (i3 = 0; i3 < loop_ub; i3++) {
          j_A[i3 + (j_A.size(0) * i2)] = A[i3 + (A.size(0) * i2)];
        }
      }
    } else {
#pragma omp parallel for num_threads(omp_get_max_threads()) private(i3)
    ...
    internal::lapack::b_xgeqp3(j_A, tau, jpvt);
    rankA = 0;
    if (j_A.size(0) < 8) {
      minmn = j_A.size(0);
      maxmn = 8;
...
#pragma omp parallel for num_threads(omp_get_max_threads())
    ...
    for (int c_j{0}; c_j < b_mn; c_j++) {
      if (tau[c_j] != 0.0) {
        ...
        wj = g_Y[c_j];
        i13 = c_j + 2;
        for (int m_i{i13}; m_i <= m; m_i++) {
          wj += j_A[(m_i + (j_A.size(0) * c_j)) - 1] * g_Y[m_i - 1];
        }
        wj *= tau[c_j];
        if (wj != 0.0) {
          g_Y[c_j] = g_Y[c_j] - wj;
          for (int t_i{i13}; t_i <= m; t_i++) {
            g_Y[t_i - 1] =
                g_Y[t_i - 1] - (j_A[(t_i + (j_A.size(0) * c_j)) - 1] * wj);
          }
        }
      }
    }
    ...
    for (int d_j{rankA}; d_j >= 1; d_j--) {
      ...
      j_Y[b_Y_tmp] /= j_A[(d_j + (j_A.size(0) * (d_j - 1))) - 1];
      for (int q_i{0}; q_i <= (d_j - 2); q_i++) {
        int i15;
        i15 = jpvt[q_i];
        j_Y[i15 - 1] -= j_Y[b_Y_tmp] * j_A[q_i + (j_A.size(0) * (d_j - 1))];
      }
    }
    g_Y.set_size(8);
#pragma omp parallel for num_threads(omp_get_max_threads())
