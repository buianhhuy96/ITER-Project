...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
  ...
  for (int c_i{0}; c_i < i6; c_i++) {
    ...
    if (images.size(2) == 3) {
      ...
      c_images.set_size(images.size(0), images.size(1), 3);
      for (int i10{0}; i10 < 3; i10++) {
        for (int i12{0}; i12 < h_loop_ub; i12++) {
          for (int i14{0}; i14 < f_loop_ub; i14++) {
            c_images[(i14 + (c_images.size(0) * i12)) +
                     ((c_images.size(0) * c_images.size(1)) * i10)] =
      ...
      coder::rgb2gray(c_images, b_current_images);
      ...
      for (int i15{0}; i15 < current_images_idx_1; i15++) {
        for (int i18{0}; i18 < current_images_idx_0; i18++) {
          current_images[i18 + (current_images.size(0) * i15)] =
              b_current_images[i18 + (current_images_idx_0 * i15)];
        }
      }
    } else {
      int e_loop_ub;
    ...
    if (undistort) {
      ...
      if (5 == c_camera_params.UndistortMap.ClassOfImage.size(1)) {
        ...
        do {
          exitg2 = 0;
          if (kstr < 5) {
            if (cv1[kstr] != c_camera_params.UndistortMap.ClassOfImage[kstr]) {
              exitg2 = 1;
            } else {
              kstr++;
            }
      ...
      f_J.set_size(outputImage.size(0), outputImage.size(1),
                   outputImage.size(2));
      ...
      for (int i37{0}; i37 < eb_loop_ub; i37++) {
        ...
        for (int i39{0}; i39 < gb_loop_ub; i39++) {
          ...
          for (int i42{0}; i42 < jb_loop_ub; i42++) {
            ...
            f_J[(i42 + (f_J.size(0) * i39)) +
                ((f_J.size(0) * f_J.size(1)) * i37)] = c_u;
          }
        }
      }
      g_J.set_size(current_images.size(0), current_images.size(1),
      ...
      for (int i44{0}; i44 < ob_loop_ub; i44++) {
        for (int i45{0}; i45 < kb_loop_ub; i45++) {
          for (int i47{0}; i47 < hb_loop_ub; i47++) {
            g_J[(i47 + (g_J.size(0) * i45)) +
                ((g_J.size(0) * g_J.size(1)) * i44)] =
                f_J[(i47 + (f_J.size(0) * i45)) +
                    ((f_J.size(0) * f_J.size(1)) * i44)];
          }
        }
      }
