                      ...
                      const coder::array<double, 3U> &e_ImagePoints, int h_n)
{
  ...
  if ((static_cast<int>((b_projectedPoints.size(0) * 2) < 4)) != 0) {
...
#pragma omp parallel for num_threads(omp_get_max_threads()) private(           \
    ...
    for (int i4 = 0; i4 < 2; i4++) {
      for (i5 = 0; i5 < b_loop_ub; i5++) {
        b_varargin_1 = b_projectedPoints[i5 + (b_projectedPoints.size(0) * i4)];
        r[i5 + (r.size(0) * i4)] = b_varargin_1 * b_varargin_1;
      }
    }
  }
}
